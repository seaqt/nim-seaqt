...
template MimerSQL*(_: type QSqlDriverDbmsTypeEnum): untyped = 9
...
proc fcQSqlDriver_connectionName(self: pointer): struct_seaqt_string {.importc: "QSqlDriver_connectionName".}
...
proc connectionName*(self: gen_qsqldriver_types.QSqlDriver): string =
  let v_ms = fcQSqlDriver_connectionName(self.h)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
