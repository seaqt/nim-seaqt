...
proc fcQSqlError_new6(driverText: struct_seaqt_string, databaseText: struct_seaqt_string, typeVal: cint, nativeErrorCode: struct_seaqt_string): ptr cQSqlError {.importc: "QSqlError_new_driverText_databaseText_type_nativeErrorCode".}
...
proc create*(T: type gen_qsqlerror_types.QSqlError,
  ...
  let tmp = gen_qsqlerror_types.QSqlError(h: fcQSqlError_new3(struct_seaqt_string(data: if len(driverText) > 0: addr driverText[0] else: nil, len: csize_t(len(driverText)))), owned: true)
  tmp
proc create*(T: type gen_qsqlerror_types.QSqlError,
    driverText: openArray[char], databaseText: openArray[char]): gen_qsqlerror_types.QSqlError =
  let tmp = gen_qsqlerror_types.QSqlError(h: fcQSqlError_new4(struct_seaqt_string(data: if len(driverText) > 0: addr driverText[0] else: nil, len: csize_t(len(driverText))), struct_seaqt_string(data: if len(databaseText) > 0: addr databaseText[0] else: nil, len: csize_t(len(databaseText)))), owned: true)
  tmp
proc create*(T: type gen_qsqlerror_types.QSqlError,
    driverText: openArray[char], databaseText: openArray[char], typeVal: cint): gen_qsqlerror_types.QSqlError =
  let tmp = gen_qsqlerror_types.QSqlError(h: fcQSqlError_new5(struct_seaqt_string(data: if len(driverText) > 0: addr driverText[0] else: nil, len: csize_t(len(driverText))), struct_seaqt_string(data: if len(databaseText) > 0: addr databaseText[0] else: nil, len: csize_t(len(databaseText))), cint(typeVal)), owned: true)
  tmp
proc create*(T: type gen_qsqlerror_types.QSqlError,
    driverText: openArray[char], databaseText: openArray[char], typeVal: cint, nativeErrorCode: openArray[char]): gen_qsqlerror_types.QSqlError =
  let tmp = gen_qsqlerror_types.QSqlError(h: fcQSqlError_new6(struct_seaqt_string(data: if len(driverText) > 0: addr driverText[0] else: nil, len: csize_t(len(driverText))), struct_seaqt_string(data: if len(databaseText) > 0: addr databaseText[0] else: nil, len: csize_t(len(databaseText))), cint(typeVal), struct_seaqt_string(data: if len(nativeErrorCode) > 0: addr nativeErrorCode[0] else: nil, len: csize_t(len(nativeErrorCode)))), owned: true)
  tmp
