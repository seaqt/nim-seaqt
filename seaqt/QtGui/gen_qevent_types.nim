...
proc `=copy`(dest: var QPointerEvent, source: QPointerEvent) {.error.}
proc `=sink`(dest: var QPointerEvent, source: QPointerEvent) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned
...
proc `=copy`(dest: var QSinglePointEvent, source: QSinglePointEvent) {.error.}
proc `=sink`(dest: var QSinglePointEvent, source: QSinglePointEvent) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned
...
type QChildWindowEvent* = object of gen_qcoreevent_types.QEvent
proc `=copy`(dest: var QChildWindowEvent, source: QChildWindowEvent) {.error.}
proc `=sink`(dest: var QChildWindowEvent, source: QChildWindowEvent) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned
