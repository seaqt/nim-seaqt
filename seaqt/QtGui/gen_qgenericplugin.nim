...
proc create*(self: gen_qgenericplugin_types.QGenericPlugin, name: string, spec: string): gen_qobject_types.QObject =
  gen_qobject_types.QObject(h: fcQGenericPlugin_create(self.h, struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), struct_miqt_string(data: if len(spec) > 0: addr spec[0] else: nil, len: csize_t(len(spec)))))
