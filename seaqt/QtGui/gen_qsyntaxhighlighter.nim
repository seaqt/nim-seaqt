...
proc fcQSyntaxHighlighter_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QSyntaxHighlighter_trUtf8_s".}
...
proc fcQSyntaxHighlighter_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QSyntaxHighlighter_trUtf8_s_c".}
proc fcQSyntaxHighlighter_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QSyntaxHighlighter_trUtf8_s_c_n".}
...
type cQSyntaxHighlighterVTable {.pure.} = object
  ...
  highlightBlock*: proc(self: pointer, text: struct_seaqt_string): void {.cdecl, raises: [], gcsafe.}
...
proc trUtf8*(_: type gen_qsyntaxhighlighter_types.QSyntaxHighlighter, s: cstring): string =
  let v_ms = fcQSyntaxHighlighter_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc fcQSyntaxHighlighter_vtable_callback_highlightBlock(self: pointer, text: struct_seaqt_string): void {.cdecl.} =
  let vtbl = cast[ptr QSyntaxHighlighterVTable](fcQSyntaxHighlighter_vdata(self)[])
  let self = QSyntaxHighlighter(h: self)
  let vtext_ms = text
...
proc fcQSyntaxHighlighter_method_callback_highlightBlock(self: pointer, text: struct_seaqt_string): void {.cdecl.} =
  let inst = cast[VirtualQSyntaxHighlighter](fcQSyntaxHighlighter_vdata(self)[])
  let vtext_ms = text
  let vtextx_ret = string.fromBytes(vtext_ms)
  c_free(vtext_ms.data)
