...
type cQPictureFormatPluginVTable {.pure.} = object
  ...
  loadPicture*: proc(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl, raises: [], gcsafe.}
  savePicture*: proc(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl, raises: [], gcsafe.}
  installIOHandler*: proc(self: pointer, format: struct_seaqt_string): bool {.cdecl, raises: [], gcsafe.}
...
proc fcQPictureFormatPlugin_virtualbase_loadPicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.importc: "QPictureFormatPlugin_virtualbase_loadPicture".}
proc fcQPictureFormatPlugin_virtualbase_savePicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.importc: "QPictureFormatPlugin_virtualbase_savePicture".}
...
proc loadPicture*(self: gen_qpictureformatplugin_types.QPictureFormatPlugin, format: openArray[char], filename: openArray[char], pic: gen_qpicture_types.QPicture): bool =
  fcQPictureFormatPlugin_loadPicture(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))), struct_seaqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), pic.h)
...
proc savePicture*(self: gen_qpictureformatplugin_types.QPictureFormatPlugin, format: openArray[char], filename: openArray[char], pic: gen_qpicture_types.QPicture): bool =
  fcQPictureFormatPlugin_savePicture(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))), struct_seaqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), pic.h)
...
proc QPictureFormatPluginloadPicture*(self: gen_qpictureformatplugin_types.QPictureFormatPlugin, format: openArray[char], filename: openArray[char], pic: gen_qpicture_types.QPicture): bool =
  fcQPictureFormatPlugin_virtualbase_loadPicture(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))), struct_seaqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), pic.h)
...
proc QPictureFormatPluginsavePicture*(self: gen_qpictureformatplugin_types.QPictureFormatPlugin, format: openArray[char], filename: openArray[char], pic: gen_qpicture_types.QPicture): bool =
  fcQPictureFormatPlugin_virtualbase_savePicture(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))), struct_seaqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), pic.h)
...
proc fcQPictureFormatPlugin_vtable_callback_loadPicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QPictureFormatPluginVTable](fcQPictureFormatPlugin_vdata(self)[])
  let self = QPictureFormatPlugin(h: self)
...
proc fcQPictureFormatPlugin_vtable_callback_savePicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QPictureFormatPluginVTable](fcQPictureFormatPlugin_vdata(self)[])
  let self = QPictureFormatPlugin(h: self)
  let vformat_ms = format
  let vformatx_ret = string.fromBytes(vformat_ms)
...
proc fcQPictureFormatPlugin_vtable_callback_installIOHandler(self: pointer, format: struct_seaqt_string): bool {.cdecl.} =
  let vtbl = cast[ptr QPictureFormatPluginVTable](fcQPictureFormatPlugin_vdata(self)[])
  let self = QPictureFormatPlugin(h: self)
  let vformat_ms = format
  let vformatx_ret = string.fromBytes(vformat_ms)
...
proc fcQPictureFormatPlugin_method_callback_loadPicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl.} =
  let inst = cast[VirtualQPictureFormatPlugin](fcQPictureFormatPlugin_vdata(self)[])
  let vformat_ms = format
  let vformatx_ret = string.fromBytes(vformat_ms)
  c_free(vformat_ms.data)
...
proc fcQPictureFormatPlugin_method_callback_savePicture(self: pointer, format: struct_seaqt_string, filename: struct_seaqt_string, pic: pointer): bool {.cdecl.} =
  let inst = cast[VirtualQPictureFormatPlugin](fcQPictureFormatPlugin_vdata(self)[])
  let vformat_ms = format
  let vformatx_ret = string.fromBytes(vformat_ms)
  c_free(vformat_ms.data)
...
proc fcQPictureFormatPlugin_method_callback_installIOHandler(self: pointer, format: struct_seaqt_string): bool {.cdecl.} =
  let inst = cast[VirtualQPictureFormatPlugin](fcQPictureFormatPlugin_vdata(self)[])
  let vformat_ms = format
  let vformatx_ret = string.fromBytes(vformat_ms)
  c_free(vformat_ms.data)
...
proc create*(T: type gen_qpictureformatplugin_types.QPictureFormatPlugin,
  ...
  let tmp = gen_qpictureformatplugin_types.QPictureFormatPlugin(h: fcQPictureFormatPlugin_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQPictureFormatPlugin_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpictureformatplugin_types.QPictureFormatPlugin,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPictureFormatPlugin_vtable_callback_disconnectNotify
  let tmp = gen_qpictureformatplugin_types.QPictureFormatPlugin(h: fcQPictureFormatPlugin_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQPictureFormatPlugin_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qpictureformatplugin_types.QPictureFormatPlugin,
  ...
  inst[].h = fcQPictureFormatPlugin_new(addr(cQPictureFormatPlugin_mvtbl), csize_t(sizeof(pointer)))
  fcQPictureFormatPlugin_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpictureformatplugin_types.QPictureFormatPlugin,
    ...
    inst: VirtualQPictureFormatPlugin) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPictureFormatPlugin_new2(addr(cQPictureFormatPlugin_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQPictureFormatPlugin_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
