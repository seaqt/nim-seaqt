...
proc fcQDoubleValidator_fixup(self: pointer, input: struct_seaqt_string): void {.importc: "QDoubleValidator_fixup".}
proc fcQDoubleValidator_setRangeBottomTopDecimals(self: pointer, bottom: float64, top: float64, decimals: cint): void {.importc: "QDoubleValidator_setRange_bottom_top_decimals".}
proc fcQDoubleValidator_setRangeBottomTop(self: pointer, bottom: float64, top: float64): void {.importc: "QDoubleValidator_setRange_bottom_top".}
...
proc fcQDoubleValidator_virtualbase_fixup(self: pointer, input: struct_seaqt_string): void {.importc: "QDoubleValidator_virtualbase_fixup".}
...
proc fixup*(self: gen_qvalidator_types.QDoubleValidator, input: openArray[char]): void =
  fcQDoubleValidator_fixup(self.h, struct_seaqt_string(data: if len(input) > 0: addr input[0] else: nil, len: csize_t(len(input))))

proc setRange*(self: gen_qvalidator_types.QDoubleValidator, bottom: float64, top: float64, decimals: cint): void =
  fcQDoubleValidator_setRangeBottomTopDecimals(self.h, bottom, top, decimals)

proc setRange*(self: gen_qvalidator_types.QDoubleValidator, bottom: float64, top: float64): void =
  fcQDoubleValidator_setRangeBottomTop(self.h, bottom, top)
...
proc QDoubleValidatorfixup*(self: gen_qvalidator_types.QDoubleValidator, input: openArray[char]): void =
  fcQDoubleValidator_virtualbase_fixup(self.h, struct_seaqt_string(data: if len(input) > 0: addr input[0] else: nil, len: csize_t(len(input))))
...
proc fcQDoubleValidator_vtable_callback_fixup(self: pointer, input: struct_seaqt_string): void {.cdecl.} =
  let vtbl = cast[ptr QDoubleValidatorVTable](fcQDoubleValidator_vdata(self)[])
  let self = QDoubleValidator(h: self)
  let vinput_ms = input
  let vinputx_ret = string.fromBytes(vinput_ms)
...
proc fcQDoubleValidator_method_callback_fixup(self: pointer, input: struct_seaqt_string): void {.cdecl.} =
  let inst = cast[VirtualQDoubleValidator](fcQDoubleValidator_vdata(self)[])
  let vinput_ms = input
  let vinputx_ret = string.fromBytes(vinput_ms)
  c_free(vinput_ms.data)
