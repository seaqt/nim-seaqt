...
proc fcQPixmapCache_find_QString_QPixmap(key: struct_seaqt_string, pixmap: pointer): bool {.importc: "QPixmapCache_find_QString_QPixmap".}
proc fcQPixmapCache_find_QPixmapCache_Key_QPixmap(key: pointer, pixmap: pointer): bool {.importc: "QPixmapCache_find_QPixmapCache_Key_QPixmap".}
...
proc find*(_: type gen_qpixmapcache_types.QPixmapCache, key: openArray[char], pixmap: gen_qpixmap_types.QPixmap): bool =
  fcQPixmapCache_find_QString_QPixmap(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), pixmap.h)
...
proc find*(_: type gen_qpixmapcache_types.QPixmapCache, key: gen_qpixmapcache_types.QPixmapCacheKey, pixmap: gen_qpixmap_types.QPixmap): bool =
  fcQPixmapCache_find_QPixmapCache_Key_QPixmap(key.h, pixmap.h)
