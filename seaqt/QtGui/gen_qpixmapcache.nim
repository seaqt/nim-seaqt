...
proc find*(_: type gen_qpixmapcache_types.QPixmapCache, key: openArray[char]): gen_qpixmap_types.QPixmap =
  gen_qpixmap_types.QPixmap(h: fcQPixmapCache_find(struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: false)

proc find*(_: type gen_qpixmapcache_types.QPixmapCache, key: openArray[char], pixmap: gen_qpixmap_types.QPixmap): bool =
  fcQPixmapCache_find2(struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), pixmap.h)

proc find2*(_: type gen_qpixmapcache_types.QPixmapCache, key: openArray[char], pixmap: gen_qpixmap_types.QPixmap): bool =
  fcQPixmapCache_find3(struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), pixmap.h)
...
proc create*(T: type gen_qpixmapcache_types.QPixmapCacheKey): gen_qpixmapcache_types.QPixmapCacheKey =
  gen_qpixmapcache_types.QPixmapCacheKey(h: fcQPixmapCacheKey_new(), owned: true)
...
proc create*(T: type gen_qpixmapcache_types.QPixmapCacheKey,
  ...
  gen_qpixmapcache_types.QPixmapCacheKey(h: fcQPixmapCacheKey_new2(other.h), owned: true)
