...
proc fcQPixmapCache_find(key: struct_seaqt_string, pixmap: pointer): bool {.importc: "QPixmapCache_find".}
...
proc find*(_: type gen_qpixmapcache_types.QPixmapCache, key: openArray[char], pixmap: gen_qpixmap_types.QPixmap): bool =
  fcQPixmapCache_find(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), pixmap.h)
