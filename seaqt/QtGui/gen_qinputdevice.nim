...
proc fcQInputDevice_seatName(self: pointer): struct_seaqt_string {.importc: "QInputDevice_seatName".}
proc fcQInputDevice_availableVirtualGeometry(self: pointer): pointer {.importc: "QInputDevice_availableVirtualGeometry".}
proc fcQInputDevice_seatNames(): struct_seaqt_array {.importc: "QInputDevice_seatNames".}
proc fcQInputDevice_devices(): struct_seaqt_array {.importc: "QInputDevice_devices".}
...
proc fcQInputDevice_capabilitiesChanged(self: pointer, capabilities: cint): void {.importc: "QInputDevice_capabilitiesChanged".}
proc fcQInputDevice_connect_capabilitiesChanged(self: pointer, slot: int, callback: proc (slot: int, capabilities: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QInputDevice_connect_capabilitiesChanged".}
...
proc capabilitiesChanged*(self: gen_qinputdevice_types.QInputDevice, capabilities: cint): void =
  fcQInputDevice_capabilitiesChanged(self.h, cint(capabilities))

type QInputDevicecapabilitiesChangedSlot* = proc(capabilities: cint)
proc fcQInputDevice_slot_callback_capabilitiesChanged(slot: int, capabilities: cint) {.cdecl.} =
  let nimfunc = cast[ptr QInputDevicecapabilitiesChangedSlot](cast[pointer](slot))
  let slotval1 = cint(capabilities)

  nimfunc[](slotval1)

proc fcQInputDevice_slot_callback_capabilitiesChanged_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QInputDevicecapabilitiesChangedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onCapabilitiesChanged*(self: gen_qinputdevice_types.QInputDevice, slot: QInputDevicecapabilitiesChangedSlot) =
  var tmp = new QInputDevicecapabilitiesChangedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQInputDevice_connect_capabilitiesChanged(self.h, cast[int](addr tmp[]), fcQInputDevice_slot_callback_capabilitiesChanged, fcQInputDevice_slot_callback_capabilitiesChanged_release)
...
proc create*(T: type gen_qinputdevice_types.QInputDevice,
  ...
  let tmp = gen_qinputdevice_types.QInputDevice(h: fcQInputDevice_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qinputdevice_types.QInputDevice,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQInputDevice_vtable_callback_disconnectNotify
  let tmp = gen_qinputdevice_types.QInputDevice(h: fcQInputDevice_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal)), owned: true)
  fcQInputDevice_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qinputdevice_types.QInputDevice,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQInputDevice_vtable_callback_disconnectNotify
  let tmp = gen_qinputdevice_types.QInputDevice(h: fcQInputDevice_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQInputDevice_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qinputdevice_types.QInputDevice,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQInputDevice_vtable_callback_disconnectNotify
  let tmp = gen_qinputdevice_types.QInputDevice(h: fcQInputDevice_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal), struct_seaqt_string(data: if len(seatName) > 0: addr seatName[0] else: nil, len: csize_t(len(seatName)))), owned: true)
  fcQInputDevice_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qinputdevice_types.QInputDevice,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQInputDevice_vtable_callback_disconnectNotify
  let tmp = gen_qinputdevice_types.QInputDevice(h: fcQInputDevice_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal), struct_seaqt_string(data: if len(seatName) > 0: addr seatName[0] else: nil, len: csize_t(len(seatName))), parent.h), owned: true)
  fcQInputDevice_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qinputdevice_types.QInputDevice,
    ...
    inst: VirtualQInputDevice) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQInputDevice_new2(addr(cQInputDevice_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal))
  fcQInputDevice_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qinputdevice_types.QInputDevice,
    ...
    inst: VirtualQInputDevice) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQInputDevice_new4(addr(cQInputDevice_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal), struct_seaqt_string(data: if len(seatName) > 0: addr seatName[0] else: nil, len: csize_t(len(seatName))))
  fcQInputDevice_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qinputdevice_types.QInputDevice,
    ...
    inst: VirtualQInputDevice) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQInputDevice_new5(addr(cQInputDevice_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), systemId, cint(typeVal), struct_seaqt_string(data: if len(seatName) > 0: addr seatName[0] else: nil, len: csize_t(len(seatName))), parent.h)
  fcQInputDevice_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
