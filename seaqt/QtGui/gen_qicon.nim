...
proc fromTheme*(_: type gen_qicon_types.QIcon, name: string): gen_qicon_types.QIcon =
  gen_qicon_types.QIcon(h: fcQIcon_fromTheme(struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))))

proc fromTheme*(_: type gen_qicon_types.QIcon, name: string, fallback: gen_qicon_types.QIcon): gen_qicon_types.QIcon =
  gen_qicon_types.QIcon(h: fcQIcon_fromTheme2(struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), fallback.h))
...
proc create*(T: type gen_qicon_types.QIcon,
  ...
  let tmp = gen_qicon_types.QIcon(h: fcQIcon_new4(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))))
  tmp
proc create*(T: type gen_qicon_types.QIcon,
