...
proc fcQTextTable_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QTextTable_trUtf8_s".}
...
proc fcQTextTable_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QTextTable_trUtf8_s_c".}
proc fcQTextTable_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QTextTable_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qtexttable_types.QTextTable, s: cstring): string =
  let v_ms = fcQTextTable_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qtexttable_types.QTextTable,
  ...
  let tmp = gen_qtexttable_types.QTextTable(h: fcQTextTable_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), doc.h), owned: true)
