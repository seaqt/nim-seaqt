...
proc fcQRasterWindow_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QRasterWindow_trUtf8_s".}
...
proc fcQRasterWindow_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QRasterWindow_trUtf8_s_c".}
proc fcQRasterWindow_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QRasterWindow_trUtf8_s_c_n".}
...
proc fcQRasterWindow_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QRasterWindow_virtualbase_nativeEvent".}
...
proc trUtf8*(_: type gen_qrasterwindow_types.QRasterWindow, s: cstring): string =
  let v_ms = fcQRasterWindow_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
type QRasterWindownativeEventProc* = proc(self: QRasterWindow, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc fcQRasterWindow_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QRasterWindowVTable](fcQRasterWindow_vdata(self)[])
  let self = QRasterWindow(h: self)
...
proc fcQRasterWindow_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQRasterWindow](fcQRasterWindow_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
