...
proc fcQRasterWindow_virtualbase_resizeEvent(self: pointer, event: pointer): void {.importc: "QRasterWindow_virtualbase_resizeEvent".}
...
proc fcQRasterWindow_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QRasterWindow_virtualbase_nativeEvent".}
...
proc fcQRasterWindow_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QRasterWindow_protectedbase_getDecodedMetricF".}
...
type QRasterWindowresizeEventProc* = proc(self: QRasterWindow, event: gen_qevent_types.QResizeEvent): void {.raises: [], gcsafe.}
...
proc QRasterWindowresizeEvent*(self: gen_qrasterwindow_types.QRasterWindow, event: gen_qevent_types.QResizeEvent): void =
  fcQRasterWindow_virtualbase_resizeEvent(self.h, event.h)
...
proc QRasterWindownativeEvent*(self: gen_qrasterwindow_types.QRasterWindow, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQRasterWindow_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQRasterWindow_vtable_callback_resizeEvent(self: pointer, event: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QRasterWindowVTable](fcQRasterWindow_vdata(self)[])
  let self = QRasterWindow(h: self)
...
proc fcQRasterWindow_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QRasterWindowVTable](fcQRasterWindow_vdata(self)[])
  let self = QRasterWindow(h: self)
...
method resizeEvent*(self: VirtualQRasterWindow, event: gen_qevent_types.QResizeEvent): void {.base.} =
  QRasterWindowresizeEvent(self[], event)
...
proc fcQRasterWindow_method_callback_resizeEvent(self: pointer, event: pointer): void {.cdecl.} =
  let inst = cast[VirtualQRasterWindow](fcQRasterWindow_vdata(self)[])
  let slotval1 = gen_qevent_types.QResizeEvent(h: event, owned: false)
...
proc fcQRasterWindow_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQRasterWindow](fcQRasterWindow_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qrasterwindow_types.QRasterWindow, metricA: cint, metricB: cint): float64 =
  fcQRasterWindow_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qrasterwindow_types.QRasterWindow,
  ...
  let tmp = gen_qrasterwindow_types.QRasterWindow(h: fcQRasterWindow_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQRasterWindow_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qrasterwindow_types.QRasterWindow,
  ...
  if not isNil(vtbl[].sharedPainter):
    vtbl[].vtbl.sharedPainter = fcQRasterWindow_vtable_callback_sharedPainter
  let tmp = gen_qrasterwindow_types.QRasterWindow(h: fcQRasterWindow_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQRasterWindow_vdata(tmp.h)[] = addr(vtbl[])
  tmp
