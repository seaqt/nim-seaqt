...
proc cQImageIOHandler_vtable_callback_name(self: pointer): struct_miqt_string {.cdecl.} =
...
proc cQImageIOHandler_method_callback_name(self: pointer): struct_miqt_string {.cdecl.} =
...
proc create*(T: type gen_qimageiohandler_types.QImageIOHandler,
  ...
  if not isNil(vtbl[].name):
    vtbl[].vtbl.name = cQImageIOHandler_vtable_callback_name
...
const cQImageIOHandler_mvtbl = cQImageIOHandlerVTable(
  destructor: proc(self: pointer) {.cdecl.} =
    ...
    inst[].h = nil
    inst[].owned = false,
  name: cQImageIOHandler_method_callback_name,
