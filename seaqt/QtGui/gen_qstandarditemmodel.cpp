...
VirtualQStandardItem* QStandardItem_new(const QStandardItem_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQStandardItem>());
	return _mem_ ? new (_mem_)VirtualQStandardItem(vtbl) : nullptr;
}

VirtualQStandardItem* QStandardItem_new2(const QStandardItem_VTable* vtbl, size_t vdata, struct miqt_string text) {
	QString text_QString = QString::fromUtf8(text.data, text.len);
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQStandardItem>());
	return _mem_ ? new (_mem_)VirtualQStandardItem(vtbl, text_QString) : nullptr;
}

VirtualQStandardItem* QStandardItem_new3(const QStandardItem_VTable* vtbl, size_t vdata, QIcon* icon, struct miqt_string text) {
	QString text_QString = QString::fromUtf8(text.data, text.len);
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQStandardItem>());
...
VirtualQStandardItemModel* QStandardItemModel_new(const QStandardItemModel_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQStandardItemModel>());
