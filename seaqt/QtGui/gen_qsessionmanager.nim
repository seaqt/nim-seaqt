...
{.compile("gen_qsessionmanager.cpp", QtGuiCFlags).}
...
proc fcQSessionManager_setRestartCommand(self: pointer, restartCommand: struct_seaqt_array): void {.importc: "QSessionManager_setRestartCommand".}
...
proc setRestartCommand*(self: gen_qsessionmanager_types.QSessionManager, restartCommand: openArray[string]): void =
  var restartCommand_CArray = newSeq[struct_seaqt_string](len(restartCommand))
  for i in 0..<len(restartCommand):
    restartCommand_CArray[i] = struct_seaqt_string(data: if len(restartCommand[i]) > 0: addr restartCommand[i][0] else: nil, len: csize_t(len(restartCommand[i])))

  fcQSessionManager_setRestartCommand(self.h, struct_seaqt_array(len: csize_t(len(restartCommand)), data: if len(restartCommand) == 0: nil else: addr(restartCommand_CArray[0])))
...
proc setDiscardCommand*(self: gen_qsessionmanager_types.QSessionManager, discardCommand: openArray[string]): void =
  var discardCommand_CArray = newSeq[struct_seaqt_string](len(discardCommand))
  for i in 0..<len(discardCommand):
    discardCommand_CArray[i] = struct_seaqt_string(data: if len(discardCommand[i]) > 0: addr discardCommand[i][0] else: nil, len: csize_t(len(discardCommand[i])))

  fcQSessionManager_setDiscardCommand(self.h, struct_seaqt_array(len: csize_t(len(discardCommand)), data: if len(discardCommand) == 0: nil else: addr(discardCommand_CArray[0])))
