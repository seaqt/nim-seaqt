...
proc fcQStandardItem_new2(vtbl: pointer, vdata: csize_t, text: struct_miqt_string): ptr cQStandardItem {.importc: "QStandardItem_new2".}
proc fcQStandardItem_new3(vtbl: pointer, vdata: csize_t, icon: pointer, text: struct_miqt_string): ptr cQStandardItem {.importc: "QStandardItem_new3".}
...
proc create*(T: type gen_qstandarditemmodel_types.QStandardItem,
  ...
  if not isNil(vtbl[].operatorLesser):
    vtbl[].vtbl.operatorLesser = fcQStandardItem_vtable_callback_operatorLesser
  let tmp = gen_qstandarditemmodel_types.QStandardItem(h: fcQStandardItem_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQStandardItem_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstandarditemmodel_types.QStandardItem,
  ...
  if not isNil(vtbl[].operatorLesser):
    vtbl[].vtbl.operatorLesser = fcQStandardItem_vtable_callback_operatorLesser
  let tmp = gen_qstandarditemmodel_types.QStandardItem(h: fcQStandardItem_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), icon.h, struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQStandardItem_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstandarditemmodel_types.QStandardItem,
    rows: cint,
...
proc create*(T: type gen_qstandarditemmodel_types.QStandardItem,
    ...
    inst: VirtualQStandardItem) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQStandardItem_new2(addr(cQStandardItem_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQStandardItem_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qstandarditemmodel_types.QStandardItem,
    ...
    inst: VirtualQStandardItem) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQStandardItem_new3(addr(cQStandardItem_mvtbl), csize_t(sizeof(pointer)), icon.h, struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQStandardItem_vdata(inst[].h)[] = addr inst[]
...
proc setItemRoleNames*(self: gen_qstandarditemmodel_types.QStandardItemModel, roleNames: Table[cint,seq[byte]]): void =
  ...
  for roleNames_v in roleNames.values():
    roleNames_Values_CArray[roleNames_ctr] = struct_miqt_string(data: if len(roleNames_v) > 0: addr roleNames_v[0] else: nil, len: csize_t(len(roleNames_v)))
    roleNames_ctr += 1
