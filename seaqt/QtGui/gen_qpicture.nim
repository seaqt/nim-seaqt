...
proc create*(T: type gen_qpicture_types.QPictureIO): gen_qpicture_types.QPictureIO =
  let tmp = gen_qpicture_types.QPictureIO(h: fcQPictureIO_new())
  tmp
proc create*(T: type gen_qpicture_types.QPictureIO,
  ...
  let tmp = gen_qpicture_types.QPictureIO(h: fcQPictureIO_new2(ioDevice.h, format))
  tmp
proc create*(T: type gen_qpicture_types.QPictureIO,
    fileName: openArray[char], format: cstring): gen_qpicture_types.QPictureIO =
  let tmp = gen_qpicture_types.QPictureIO(h: fcQPictureIO_new3(struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), format))
  tmp
