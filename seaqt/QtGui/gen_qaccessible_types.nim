...
type QAccessibleSelectionInterface* {.inheritable.} = object
  h*: pointer
  owned*: bool

proc fcQAccessibleSelectionInterface_delete(self: pointer) {.importc: "QAccessibleSelectionInterface_delete".}
proc `=destroy`(self: var QAccessibleSelectionInterface) =
  if self.owned: fcQAccessibleSelectionInterface_delete(self.h)

proc `=sink`(dest: var QAccessibleSelectionInterface, source: QAccessibleSelectionInterface) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QAccessibleSelectionInterface, source: QAccessibleSelectionInterface) {.error.}
proc delete*(self: sink QAccessibleSelectionInterface) =
  let h = self.h
  wasMoved(self)
  fcQAccessibleSelectionInterface_delete(h)

type QAccessibleAttributesInterface* {.inheritable.} = object
  h*: pointer
  owned*: bool

proc fcQAccessibleAttributesInterface_delete(self: pointer) {.importc: "QAccessibleAttributesInterface_delete".}
proc `=destroy`(self: var QAccessibleAttributesInterface) =
  if self.owned: fcQAccessibleAttributesInterface_delete(self.h)

proc `=sink`(dest: var QAccessibleAttributesInterface, source: QAccessibleAttributesInterface) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QAccessibleAttributesInterface, source: QAccessibleAttributesInterface) {.error.}
proc delete*(self: sink QAccessibleAttributesInterface) =
  let h = self.h
  wasMoved(self)
  fcQAccessibleAttributesInterface_delete(h)
...
type QAccessibleAnnouncementEvent* = object of QAccessibleEvent
proc `=copy`(dest: var QAccessibleAnnouncementEvent, source: QAccessibleAnnouncementEvent) {.error.}
proc `=sink`(dest: var QAccessibleAnnouncementEvent, source: QAccessibleAnnouncementEvent) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned
