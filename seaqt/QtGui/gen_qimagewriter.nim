...
proc fcQImageWriter_setFileName(self: pointer, fileName: struct_seaqt_string): void {.importc: "QImageWriter_setFileName".}
proc fcQImageWriter_fileName(self: pointer): struct_seaqt_string {.importc: "QImageWriter_fileName".}
...
proc fcQImageWriter_setSubType(self: pointer, typeVal: struct_seaqt_string): void {.importc: "QImageWriter_setSubType".}
proc fcQImageWriter_subType(self: pointer): struct_seaqt_string {.importc: "QImageWriter_subType".}
proc fcQImageWriter_supportedSubTypes(self: pointer): struct_seaqt_array {.importc: "QImageWriter_supportedSubTypes".}
...
proc fcQImageWriter_setDescription(self: pointer, description: struct_seaqt_string): void {.importc: "QImageWriter_setDescription".}
proc fcQImageWriter_description(self: pointer): struct_seaqt_string {.importc: "QImageWriter_description".}
proc fcQImageWriter_setText(self: pointer, key: struct_seaqt_string, text: struct_seaqt_string): void {.importc: "QImageWriter_setText".}
...
proc fcQImageWriter_errorString(self: pointer): struct_seaqt_string {.importc: "QImageWriter_errorString".}
...
proc setFormat*(self: gen_qimagewriter_types.QImageWriter, format: openArray[byte]): void =
  fcQImageWriter_setFormat(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))))
...
proc setFileName*(self: gen_qimagewriter_types.QImageWriter, fileName: openArray[char]): void =
  fcQImageWriter_setFileName(self.h, struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
...
proc setSubType*(self: gen_qimagewriter_types.QImageWriter, typeVal: openArray[byte]): void =
  fcQImageWriter_setSubType(self.h, struct_seaqt_string(data: if len(typeVal) > 0: addr typeVal[0] else: nil, len: csize_t(len(typeVal))))
...
proc setDescription*(self: gen_qimagewriter_types.QImageWriter, description: openArray[char]): void =
  fcQImageWriter_setDescription(self.h, struct_seaqt_string(data: if len(description) > 0: addr description[0] else: nil, len: csize_t(len(description))))
...
proc setText*(self: gen_qimagewriter_types.QImageWriter, key: openArray[char], text: openArray[char]): void =
  fcQImageWriter_setText(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc imageFormatsForMimeType*(_: type gen_qimagewriter_types.QImageWriter, mimeType: openArray[byte]): seq[seq[byte]] =
  var v_ma = fcQImageWriter_imageFormatsForMimeType(struct_seaqt_string(data: if len(mimeType) > 0: addr mimeType[0] else: nil, len: csize_t(len(mimeType))))
  var vx_ret = newSeq[seq[byte]](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    var vx_lv_bytearray = v_outCast[i]
...
proc create*(T: type gen_qimagewriter_types.QImageWriter,
  ...
  let tmp = gen_qimagewriter_types.QImageWriter(h: fcQImageWriter_new2(device.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))), owned: true)
  tmp
proc create*(T: type gen_qimagewriter_types.QImageWriter,
    fileName: openArray[char]): gen_qimagewriter_types.QImageWriter =
  let tmp = gen_qimagewriter_types.QImageWriter(h: fcQImageWriter_new3(struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))), owned: true)
  tmp
proc create*(T: type gen_qimagewriter_types.QImageWriter,
    fileName: openArray[char], format: openArray[byte]): gen_qimagewriter_types.QImageWriter =
  let tmp = gen_qimagewriter_types.QImageWriter(h: fcQImageWriter_new4(struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))), owned: true)
  tmp
