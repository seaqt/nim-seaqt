...
proc fcQGlyphRun_glyphIndexes(self: pointer): struct_seaqt_array {.importc: "QGlyphRun_glyphIndexes".}
proc fcQGlyphRun_setGlyphIndexes(self: pointer, glyphIndexes: struct_seaqt_array): void {.importc: "QGlyphRun_setGlyphIndexes".}
proc fcQGlyphRun_positions(self: pointer): struct_seaqt_array {.importc: "QGlyphRun_positions".}
proc fcQGlyphRun_setPositions(self: pointer, positions: struct_seaqt_array): void {.importc: "QGlyphRun_setPositions".}
...
proc fcQGlyphRun_stringIndexes(self: pointer): struct_seaqt_array {.importc: "QGlyphRun_stringIndexes".}
proc fcQGlyphRun_setStringIndexes(self: pointer, stringIndexes: struct_seaqt_array): void {.importc: "QGlyphRun_setStringIndexes".}
proc fcQGlyphRun_setSourceString(self: pointer, sourceString: struct_seaqt_string): void {.importc: "QGlyphRun_setSourceString".}
proc fcQGlyphRun_sourceString(self: pointer): struct_seaqt_string {.importc: "QGlyphRun_sourceString".}
...
proc setGlyphIndexes*(self: gen_qglyphrun_types.QGlyphRun, glyphIndexes: openArray[cuint]): void =
  ...
  fcQGlyphRun_setGlyphIndexes(self.h, struct_seaqt_array(len: csize_t(len(glyphIndexes)), data: if len(glyphIndexes) == 0: nil else: addr(glyphIndexes_CArray[0])))
...
proc setPositions*(self: gen_qglyphrun_types.QGlyphRun, positions: openArray[gen_qpoint_types.QPointF]): void =
  ...
  fcQGlyphRun_setPositions(self.h, struct_seaqt_array(len: csize_t(len(positions)), data: if len(positions) == 0: nil else: addr(positions_CArray[0])))
...
proc stringIndexes*(self: gen_qglyphrun_types.QGlyphRun): seq[int64] =
  var v_ma = fcQGlyphRun_stringIndexes(self.h)
  var vx_ret = newSeq[int64](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[int64]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = v_outCast[i]
  c_free(v_ma.data)
  vx_ret

proc setStringIndexes*(self: gen_qglyphrun_types.QGlyphRun, stringIndexes: openArray[int64]): void =
  var stringIndexes_CArray = newSeq[int64](len(stringIndexes))
  for i in 0..<len(stringIndexes):
    stringIndexes_CArray[i] = stringIndexes[i]

  fcQGlyphRun_setStringIndexes(self.h, struct_seaqt_array(len: csize_t(len(stringIndexes)), data: if len(stringIndexes) == 0: nil else: addr(stringIndexes_CArray[0])))

proc setSourceString*(self: gen_qglyphrun_types.QGlyphRun, sourceString: openArray[char]): void =
  fcQGlyphRun_setSourceString(self.h, struct_seaqt_string(data: if len(sourceString) > 0: addr sourceString[0] else: nil, len: csize_t(len(sourceString))))

proc sourceString*(self: gen_qglyphrun_types.QGlyphRun): string =
  let v_ms = fcQGlyphRun_sourceString(self.h)
