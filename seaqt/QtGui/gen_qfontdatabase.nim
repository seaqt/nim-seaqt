...
proc fcQFontDatabase_hasFamily(self: pointer, family: struct_seaqt_string): bool {.importc: "QFontDatabase_hasFamily".}
proc fcQFontDatabase_isPrivateFamily(self: pointer, family: struct_seaqt_string): bool {.importc: "QFontDatabase_isPrivateFamily".}
...
proc styles*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char]): seq[string] =
  var v_ma = fcQFontDatabase_styles(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))))
  var vx_ret = newSeq[string](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    let vx_lv_ms = v_outCast[i]
...
proc smoothSizes*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char], style: openArray[char]): seq[cint] =
  var v_ma = fcQFontDatabase_smoothSizes(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))), struct_seaqt_string(data: if len(style) > 0: addr style[0] else: nil, len: csize_t(len(style))))
  var vx_ret = newSeq[cint](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[cint]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = v_outCast[i]
...
proc font*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char], style: openArray[char], pointSize: cint): gen_qfont_types.QFont =
  gen_qfont_types.QFont(h: fcQFontDatabase_font(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))), struct_seaqt_string(data: if len(style) > 0: addr style[0] else: nil, len: csize_t(len(style))), pointSize))
...
proc bold*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char], style: openArray[char]): bool =
  fcQFontDatabase_bold(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))), struct_seaqt_string(data: if len(style) > 0: addr style[0] else: nil, len: csize_t(len(style))))

proc weight*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char], style: openArray[char]): cint =
  fcQFontDatabase_weight(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))), struct_seaqt_string(data: if len(style) > 0: addr style[0] else: nil, len: csize_t(len(style))))

proc hasFamily*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char]): bool =
  fcQFontDatabase_hasFamily(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))))

proc isPrivateFamily*(self: gen_qfontdatabase_types.QFontDatabase, family: openArray[char]): bool =
  fcQFontDatabase_isPrivateFamily(self.h, struct_seaqt_string(data: if len(family) > 0: addr family[0] else: nil, len: csize_t(len(family))))
