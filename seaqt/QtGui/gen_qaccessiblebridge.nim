...
proc create*(self: gen_qaccessiblebridge_types.QAccessibleBridgePlugin, key: string): gen_qaccessiblebridge_types.QAccessibleBridge =
  gen_qaccessiblebridge_types.QAccessibleBridge(h: fcQAccessibleBridgePlugin_create(self.h, struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))))
...
proc fcQAccessibleBridgePlugin_vtable_callback_create(self: pointer, key: struct_miqt_string): pointer {.cdecl.} =
  let vtbl = cast[ptr QAccessibleBridgePluginVTable](fcQAccessibleBridgePlugin_vdata(self)[])
  let self = QAccessibleBridgePlugin(h: self)
  let vkey_ms = key
...
proc create*(T: type gen_qaccessiblebridge_types.QAccessibleBridgePlugin,
  ...
  if not isNil(vtbl[].create):
    vtbl[].vtbl.create = fcQAccessibleBridgePlugin_vtable_callback_create
...
const cQAccessibleBridgePlugin_mvtbl = cQAccessibleBridgePluginVTable(
  ...
  create: fcQAccessibleBridgePlugin_method_callback_create,
