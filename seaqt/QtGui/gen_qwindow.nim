...
proc fcQWindow_safeAreaMargins(self: pointer): pointer {.importc: "QWindow_safeAreaMargins".}
...
proc fcQWindow_setTitle(self: pointer, title: struct_seaqt_string): void {.importc: "QWindow_setTitle".}
...
proc fcQWindow_flagsChanged(self: pointer, flags: cint): void {.importc: "QWindow_flagsChanged".}
proc fcQWindow_connect_flagsChanged(self: pointer, slot: int, callback: proc (slot: int, flags: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QWindow_connect_flagsChanged".}
...
proc fcQWindow_windowTitleChanged(self: pointer, title: struct_seaqt_string): void {.importc: "QWindow_windowTitleChanged".}
proc fcQWindow_connect_windowTitleChanged(self: pointer, slot: int, callback: proc (slot: int, title: struct_seaqt_string) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QWindow_connect_windowTitleChanged".}
...
proc fcQWindow_safeAreaMarginsChanged(self: pointer, arg: pointer): void {.importc: "QWindow_safeAreaMarginsChanged".}
proc fcQWindow_connect_safeAreaMarginsChanged(self: pointer, slot: int, callback: proc (slot: int, arg: pointer) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QWindow_connect_safeAreaMarginsChanged".}
...
proc fcQWindow_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QWindow_virtualbase_nativeEvent".}
...
proc safeAreaMargins*(self: gen_qwindow_types.QWindow): gen_qmargins_types.QMargins =
  gen_qmargins_types.QMargins(h: fcQWindow_safeAreaMargins(self.h), owned: true)
...
proc setTitle*(self: gen_qwindow_types.QWindow, title: openArray[char]): void =
  fcQWindow_setTitle(self.h, struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))))
...
proc flagsChanged*(self: gen_qwindow_types.QWindow, flags: cint): void =
  fcQWindow_flagsChanged(self.h, cint(flags))

type QWindowflagsChangedSlot* = proc(flags: cint)
proc fcQWindow_slot_callback_flagsChanged(slot: int, flags: cint) {.cdecl.} =
  let nimfunc = cast[ptr QWindowflagsChangedSlot](cast[pointer](slot))
  let slotval1 = cint(flags)

  nimfunc[](slotval1)

proc fcQWindow_slot_callback_flagsChanged_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QWindowflagsChangedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onFlagsChanged*(self: gen_qwindow_types.QWindow, slot: QWindowflagsChangedSlot) =
  var tmp = new QWindowflagsChangedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQWindow_connect_flagsChanged(self.h, cast[int](addr tmp[]), fcQWindow_slot_callback_flagsChanged, fcQWindow_slot_callback_flagsChanged_release)
...
proc windowTitleChanged*(self: gen_qwindow_types.QWindow, title: openArray[char]): void =
  fcQWindow_windowTitleChanged(self.h, struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))))

type QWindowwindowTitleChangedSlot* = proc(title: openArray[char])
proc fcQWindow_slot_callback_windowTitleChanged(slot: int, title: struct_seaqt_string) {.cdecl.} =
...
proc safeAreaMarginsChanged*(self: gen_qwindow_types.QWindow, arg: gen_qmargins_types.QMargins): void =
  fcQWindow_safeAreaMarginsChanged(self.h, arg.h)

type QWindowsafeAreaMarginsChangedSlot* = proc(arg: gen_qmargins_types.QMargins)
proc fcQWindow_slot_callback_safeAreaMarginsChanged(slot: int, arg: pointer) {.cdecl.} =
  let nimfunc = cast[ptr QWindowsafeAreaMarginsChangedSlot](cast[pointer](slot))
  let slotval1 = gen_qmargins_types.QMargins(h: arg, owned: true)

  nimfunc[](slotval1)

proc fcQWindow_slot_callback_safeAreaMarginsChanged_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QWindowsafeAreaMarginsChangedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onSafeAreaMarginsChanged*(self: gen_qwindow_types.QWindow, slot: QWindowsafeAreaMarginsChangedSlot) =
  var tmp = new QWindowsafeAreaMarginsChangedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQWindow_connect_safeAreaMarginsChanged(self.h, cast[int](addr tmp[]), fcQWindow_slot_callback_safeAreaMarginsChanged, fcQWindow_slot_callback_safeAreaMarginsChanged_release)
...
proc QWindownativeEvent*(self: gen_qwindow_types.QWindow, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQWindow_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQWindow_vtable_callback_paintEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QWindowVTable](fcQWindow_vdata(self)[])
  let self = QWindow(h: self)
...
proc fcQWindow_vtable_callback_closeEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QWindowVTable](fcQWindow_vdata(self)[])
  let self = QWindow(h: self)
...
proc fcQWindow_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QWindowVTable](fcQWindow_vdata(self)[])
  let self = QWindow(h: self)
...
proc fcQWindow_method_callback_paintEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let inst = cast[VirtualQWindow](fcQWindow_vdata(self)[])
  let slotval1 = gen_qevent_types.QPaintEvent(h: param1, owned: false)
...
proc fcQWindow_method_callback_closeEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let inst = cast[VirtualQWindow](fcQWindow_vdata(self)[])
  let slotval1 = gen_qevent_types.QCloseEvent(h: param1, owned: false)
...
proc fcQWindow_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQWindow](fcQWindow_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qwindow_types.QWindow,
  ...
  if not isNil(vtbl[].paintEvent):
    vtbl[].vtbl.paintEvent = fcQWindow_vtable_callback_paintEvent
  ...
  if not isNil(vtbl[].closeEvent):
    vtbl[].vtbl.closeEvent = fcQWindow_vtable_callback_closeEvent
  ...
  let tmp = gen_qwindow_types.QWindow(h: fcQWindow_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQWindow_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qwindow_types.QWindow,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWindow_vtable_callback_disconnectNotify
  let tmp = gen_qwindow_types.QWindow(h: fcQWindow_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQWindow_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qwindow_types.QWindow,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWindow_vtable_callback_disconnectNotify
  let tmp = gen_qwindow_types.QWindow(h: fcQWindow_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), screen.h), owned: true)
  fcQWindow_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQWindow_mvtbl = cQWindowVTable(
  ...
  paintEvent: fcQWindow_method_callback_paintEvent,
  ...
  closeEvent: fcQWindow_method_callback_closeEvent,
