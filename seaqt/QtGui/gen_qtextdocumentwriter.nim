...
proc create*(T: type gen_qtextdocumentwriter_types.QTextDocumentWriter,
  ...
  let tmp = gen_qtextdocumentwriter_types.QTextDocumentWriter(h: fcQTextDocumentWriter_new2(device.h, struct_miqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))))
  tmp
proc create*(T: type gen_qtextdocumentwriter_types.QTextDocumentWriter,
    fileName: string): gen_qtextdocumentwriter_types.QTextDocumentWriter =
  let tmp = gen_qtextdocumentwriter_types.QTextDocumentWriter(h: fcQTextDocumentWriter_new3(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))))
  tmp
proc create*(T: type gen_qtextdocumentwriter_types.QTextDocumentWriter,
    fileName: string, format: seq[byte]): gen_qtextdocumentwriter_types.QTextDocumentWriter =
  let tmp = gen_qtextdocumentwriter_types.QTextDocumentWriter(h: fcQTextDocumentWriter_new4(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_miqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))))
  tmp
