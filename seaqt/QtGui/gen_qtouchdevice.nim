...
proc fcQTouchDevice_devices(): struct_seaqt_array {.importc: "QTouchDevice_devices".}
proc fcQTouchDevice_name(self: pointer): struct_seaqt_string {.importc: "QTouchDevice_name".}
...
proc fcQTouchDevice_setName(self: pointer, name: struct_seaqt_string): void {.importc: "QTouchDevice_setName".}
...
proc setName*(self: gen_qtouchdevice_types.QTouchDevice, name: openArray[char]): void =
  fcQTouchDevice_setName(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc create*(T: type gen_qtouchdevice_types.QTouchDevice): gen_qtouchdevice_types.QTouchDevice =
  let tmp = gen_qtouchdevice_types.QTouchDevice(h: fcQTouchDevice_new(), owned: true)
  tmp
