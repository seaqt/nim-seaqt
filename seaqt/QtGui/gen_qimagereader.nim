...
proc fcQImageReader_trUtf8SourceText(sourceText: cstring): struct_seaqt_string {.importc: "QImageReader_trUtf8_sourceText".}
...
proc fcQImageReader_setFileName(self: pointer, fileName: struct_seaqt_string): void {.importc: "QImageReader_setFileName".}
proc fcQImageReader_fileName(self: pointer): struct_seaqt_string {.importc: "QImageReader_fileName".}
...
proc fcQImageReader_textKeys(self: pointer): struct_seaqt_array {.importc: "QImageReader_textKeys".}
proc fcQImageReader_text(self: pointer, key: struct_seaqt_string): struct_seaqt_string {.importc: "QImageReader_text".}
...
proc fcQImageReader_subType(self: pointer): struct_seaqt_string {.importc: "QImageReader_subType".}
...
proc fcQImageReader_trUtf8SourceTextDisambiguation(sourceText: cstring, disambiguation: cstring): struct_seaqt_string {.importc: "QImageReader_trUtf8_sourceText_disambiguation".}
proc fcQImageReader_trUtf8SourceTextDisambiguationN(sourceText: cstring, disambiguation: cstring, n: cint): struct_seaqt_string {.importc: "QImageReader_trUtf8_sourceText_disambiguation_n".}
...
proc trUtf8*(_: type gen_qimagereader_types.QImageReader, sourceText: cstring): string =
  let v_ms = fcQImageReader_trUtf8SourceText(sourceText)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc setFormat*(self: gen_qimagereader_types.QImageReader, format: openArray[byte]): void =
  fcQImageReader_setFormat(self.h, struct_seaqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format))))
...
proc text*(self: gen_qimagereader_types.QImageReader, key: openArray[char]): string =
  let v_ms = fcQImageReader_text(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc imageFormatsForMimeType*(_: type gen_qimagereader_types.QImageReader, mimeType: openArray[byte]): seq[seq[byte]] =
  var v_ma = fcQImageReader_imageFormatsForMimeType(struct_seaqt_string(data: if len(mimeType) > 0: addr mimeType[0] else: nil, len: csize_t(len(mimeType))))
  var vx_ret = newSeq[seq[byte]](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    var vx_lv_bytearray = v_outCast[i]
...
proc trUtf8*(_: type gen_qimagereader_types.QImageReader, sourceText: cstring, disambiguation: cstring): string =
  let v_ms = fcQImageReader_trUtf8SourceTextDisambiguation(sourceText, disambiguation)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc trUtf8*(_: type gen_qimagereader_types.QImageReader, sourceText: cstring, disambiguation: cstring, n: cint): string =
  let v_ms = fcQImageReader_trUtf8SourceTextDisambiguationN(sourceText, disambiguation, n)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
