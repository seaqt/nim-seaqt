...
proc create*(T: type gen_qimagereader_types.QImageReader,
  ...
  let tmp = gen_qimagereader_types.QImageReader(h: fcQImageReader_new3(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))))
  tmp
proc create*(T: type gen_qimagereader_types.QImageReader,
    device: gen_qiodevice_types.QIODevice, format: seq[byte]): gen_qimagereader_types.QImageReader =
  let tmp = gen_qimagereader_types.QImageReader(h: fcQImageReader_new4(device.h, struct_miqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))))
  tmp
proc create*(T: type gen_qimagereader_types.QImageReader,
    fileName: string, format: seq[byte]): gen_qimagereader_types.QImageReader =
  let tmp = gen_qimagereader_types.QImageReader(h: fcQImageReader_new5(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_miqt_string(data: if len(format) > 0: addr format[0] else: nil, len: csize_t(len(format)))))
  tmp
