...
proc fcQPdfWriter_new(vtbl: pointer, vdata: csize_t, filename: struct_miqt_string): ptr cQPdfWriter {.importc: "QPdfWriter_new".}
...
proc create*(T: type gen_qpdfwriter_types.QPdfWriter,
  ...
  let tmp = gen_qpdfwriter_types.QPdfWriter(h: fcQPdfWriter_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename)))), owned: true)
