...
proc create*(T: type gen_qbitmap_types.QBitmap,
  ...
  if not isNil(vtbl[].sharedPainter):
    vtbl[].vtbl.sharedPainter = fcQBitmap_vtable_callback_sharedPainter
  let tmp = gen_qbitmap_types.QBitmap(h: fcQBitmap_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))))
  fcQBitmap_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qbitmap_types.QBitmap,
  ...
  if not isNil(vtbl[].sharedPainter):
    vtbl[].vtbl.sharedPainter = fcQBitmap_vtable_callback_sharedPainter
  let tmp = gen_qbitmap_types.QBitmap(h: fcQBitmap_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), param1.h))
  fcQBitmap_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qbitmap_types.QBitmap,
  ...
  if not isNil(vtbl[].sharedPainter):
    vtbl[].vtbl.sharedPainter = fcQBitmap_vtable_callback_sharedPainter
  let tmp = gen_qbitmap_types.QBitmap(h: fcQBitmap_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), format))
  fcQBitmap_vdata(tmp.h)[] = addr(vtbl[])
  tmp
