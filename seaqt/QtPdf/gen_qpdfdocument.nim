...
type QPdfDocumentErrorEnum* = distinct cint
template None*(_: type QPdfDocumentErrorEnum): untyped = 0
template Unknown*(_: type QPdfDocumentErrorEnum): untyped = 1
template DataNotYetAvailable*(_: type QPdfDocumentErrorEnum): untyped = 2
template FileNotFound*(_: type QPdfDocumentErrorEnum): untyped = 3
template InvalidFileFormat*(_: type QPdfDocumentErrorEnum): untyped = 4
template IncorrectPassword*(_: type QPdfDocumentErrorEnum): untyped = 5
template UnsupportedSecurityScheme*(_: type QPdfDocumentErrorEnum): untyped = 6
...
type QPdfDocumentPageModelRoleEnum* = distinct cint
template Label*(_: type QPdfDocumentPageModelRoleEnum): untyped = 256
template PointSize*(_: type QPdfDocumentPageModelRoleEnum): untyped = 257
template NRoles*(_: type QPdfDocumentPageModelRoleEnum): untyped = 258
...
proc fcQPdfDocument_pageIndexForLabel(self: pointer, label: struct_seaqt_string): cint {.importc: "QPdfDocument_pageIndexForLabel".}
...
proc pagePointSize*(self: gen_qpdfdocument_types.QPdfDocument, page: cint): gen_qsize_types.QSizeF =
  gen_qsize_types.QSizeF(h: fcQPdfDocument_pagePointSize(self.h, page), owned: true)

proc pageLabel*(self: gen_qpdfdocument_types.QPdfDocument, page: cint): string =
  let v_ms = fcQPdfDocument_pageLabel(self.h, page)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc pageIndexForLabel*(self: gen_qpdfdocument_types.QPdfDocument, label: openArray[char]): cint =
  fcQPdfDocument_pageIndexForLabel(self.h, struct_seaqt_string(data: if len(label) > 0: addr label[0] else: nil, len: csize_t(len(label))))

proc pageModel*(self: gen_qpdfdocument_types.QPdfDocument): gen_qabstractitemmodel_types.QAbstractListModel =
...
proc pageModelChanged*(self: gen_qpdfdocument_types.QPdfDocument): void =
  fcQPdfDocument_pageModelChanged(self.h)

type QPdfDocumentpageModelChangedSlot* = proc()
proc fcQPdfDocument_slot_callback_pageModelChanged(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QPdfDocumentpageModelChangedSlot](cast[pointer](slot))
  nimfunc[]()

proc fcQPdfDocument_slot_callback_pageModelChanged_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QPdfDocumentpageModelChangedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onPageModelChanged*(self: gen_qpdfdocument_types.QPdfDocument, slot: QPdfDocumentpageModelChangedSlot) =
  var tmp = new QPdfDocumentpageModelChangedSlot
  tmp[] = slot
  GC_ref(tmp)
