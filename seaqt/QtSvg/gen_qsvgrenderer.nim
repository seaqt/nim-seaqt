...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename)))), owned: true)
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents)))), owned: true)
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), parent.h), owned: true)
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents))), parent.h), owned: true)
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
