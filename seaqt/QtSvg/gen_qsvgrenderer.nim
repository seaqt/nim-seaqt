...
proc fcQSvgRenderer_new2(vtbl: pointer, vdata: csize_t, filename: struct_miqt_string): ptr cQSvgRenderer {.importc: "QSvgRenderer_new2".}
proc fcQSvgRenderer_new3(vtbl: pointer, vdata: csize_t, contents: struct_miqt_string): ptr cQSvgRenderer {.importc: "QSvgRenderer_new3".}
...
proc fcQSvgRenderer_new6(vtbl: pointer, vdata: csize_t, filename: struct_miqt_string, parent: pointer): ptr cQSvgRenderer {.importc: "QSvgRenderer_new6".}
proc fcQSvgRenderer_new7(vtbl: pointer, vdata: csize_t, contents: struct_miqt_string, parent: pointer): ptr cQSvgRenderer {.importc: "QSvgRenderer_new7".}
...
proc boundsOnElement*(self: gen_qsvgrenderer_types.QSvgRenderer, id: string): gen_qrect_types.QRectF =
  gen_qrect_types.QRectF(h: fcQSvgRenderer_boundsOnElement(self.h, struct_miqt_string(data: if len(id) > 0: addr id[0] else: nil, len: csize_t(len(id)))))
...
proc transformForElement*(self: gen_qsvgrenderer_types.QSvgRenderer, id: string): gen_qtransform_types.QTransform =
  gen_qtransform_types.QTransform(h: fcQSvgRenderer_transformForElement(self.h, struct_miqt_string(data: if len(id) > 0: addr id[0] else: nil, len: csize_t(len(id)))))
...
proc load*(self: gen_qsvgrenderer_types.QSvgRenderer, contents: seq[byte]): bool =
  fcQSvgRenderer_loadWithContents(self.h, struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents))))
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename)))))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
    contents: seq[byte],
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents)))))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), contents.h))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), parent.h))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents))), parent.h))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgRenderer_vtable_callback_disconnectNotify
  let tmp = gen_qsvgrenderer_types.QSvgRenderer(h: fcQSvgRenderer_new8(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), contents.h, parent.h))
  fcQSvgRenderer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
    ...
    inst: VirtualQSvgRenderer) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgRenderer_new2(addr(cQSvgRenderer_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))))
  fcQSvgRenderer_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
    ...
    inst: VirtualQSvgRenderer) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgRenderer_new3(addr(cQSvgRenderer_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents))))
  fcQSvgRenderer_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
    ...
    inst: VirtualQSvgRenderer) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgRenderer_new6(addr(cQSvgRenderer_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(filename) > 0: addr filename[0] else: nil, len: csize_t(len(filename))), parent.h)
  fcQSvgRenderer_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qsvgrenderer_types.QSvgRenderer,
    ...
    inst: VirtualQSvgRenderer) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgRenderer_new7(addr(cQSvgRenderer_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(contents) > 0: addr contents[0] else: nil, len: csize_t(len(contents))), parent.h)
  fcQSvgRenderer_vdata(inst[].h)[] = addr inst[]
