...
proc fcQWebEngineScript_sourceCode(self: pointer): struct_seaqt_string {.importc: "QWebEngineScript_sourceCode".}
proc fcQWebEngineScript_setSourceCode(self: pointer, sourceCode: struct_seaqt_string): void {.importc: "QWebEngineScript_setSourceCode".}
...
proc setSourceCode*(self: gen_qwebenginescript_types.QWebEngineScript, sourceCode: openArray[char]): void =
  fcQWebEngineScript_setSourceCode(self.h, struct_seaqt_string(data: if len(sourceCode) > 0: addr sourceCode[0] else: nil, len: csize_t(len(sourceCode))))
