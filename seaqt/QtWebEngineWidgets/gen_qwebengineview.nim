...
proc fcQWebEngineView_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QWebEngineView_virtualbase_nativeEvent".}
...
proc fcQWebEngineView_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQWebEngineView {.importc: "QWebEngineView_new_parent".}
...
type QWebEngineViewnativeEventProc* = proc(self: QWebEngineView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc QWebEngineViewnativeEvent*(self: gen_qwebengineview_types.QWebEngineView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
...
proc fcQWebEngineView_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QWebEngineViewVTable](fcQWebEngineView_vdata(self)[])
  let self = QWebEngineView(h: self)
...
method enterEvent*(self: VirtualQWebEngineView, event: gen_qcoreevent_types.QEvent): void {.base.} =
...
method nativeEvent*(self: VirtualQWebEngineView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
...
proc fcQWebEngineView_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQWebEngineView](fcQWebEngineView_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qwebengineview_types.QWebEngineView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWebEngineView_vtable_callback_disconnectNotify
  let tmp = gen_qwebengineview_types.QWebEngineView(h: fcQWebEngineView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQWebEngineView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qwebengineview_types.QWebEngineView,
    ...
    inst: VirtualQWebEngineView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQWebEngineView_new2(addr(cQWebEngineView_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQWebEngineView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
