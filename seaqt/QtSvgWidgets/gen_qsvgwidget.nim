...
{.compile("gen_qsvgwidget.cpp", QtSvgWidgetsCFlags).}
...
proc fcQSvgWidget_options(self: pointer): cint {.importc: "QSvgWidget_options".}
proc fcQSvgWidget_setOptions(self: pointer, options: cint): void {.importc: "QSvgWidget_setOptions".}
...
proc fcQSvgWidget_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QSvgWidget_virtualbase_nativeEvent".}
...
proc fcQSvgWidget_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QSvgWidget_protectedbase_getDecodedMetricF".}
...
proc options*(self: gen_qsvgwidget_types.QSvgWidget): cint =
  cint(fcQSvgWidget_options(self.h))

proc setOptions*(self: gen_qsvgwidget_types.QSvgWidget, options: cint): void =
  fcQSvgWidget_setOptions(self.h, cint(options))
...
proc QSvgWidgetnativeEvent*(self: gen_qsvgwidget_types.QSvgWidget, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQSvgWidget_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQSvgWidget_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QSvgWidgetVTable](fcQSvgWidget_vdata(self)[])
  let self = QSvgWidget(h: self)
...
proc fcQSvgWidget_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQSvgWidget](fcQSvgWidget_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qsvgwidget_types.QSvgWidget, metricA: cint, metricB: cint): float64 =
  fcQSvgWidget_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file)))), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))), parent.h), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  inst[].h = fcQSvgWidget_new(addr(cQSvgWidget_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
    ...
    inst: VirtualQSvgWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgWidget_new2(addr(cQSvgWidget_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))))
  fcQSvgWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
    ...
    inst: VirtualQSvgWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgWidget_new3(addr(cQSvgWidget_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQSvgWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
    ...
    inst: VirtualQSvgWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgWidget_new4(addr(cQSvgWidget_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))), parent.h)
  fcQSvgWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
