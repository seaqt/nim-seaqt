...
proc fcQSvgWidget_new3(vtbl: pointer, vdata: csize_t, file: struct_miqt_string): ptr cQSvgWidget {.importc: "QSvgWidget_new3".}
proc fcQSvgWidget_new4(vtbl: pointer, vdata: csize_t, file: struct_miqt_string, parent: pointer): ptr cQSvgWidget {.importc: "QSvgWidget_new4".}
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file)))), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))), parent.h), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
