...
proc fcQSvgWidget_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQSvgWidget {.importc: "QSvgWidget_new_parent".}
proc fcQSvgWidget_new2(vtbl: pointer, vdata: csize_t): ptr cQSvgWidget {.importc: "QSvgWidget_new".}
proc fcQSvgWidget_new3(vtbl: pointer, vdata: csize_t, file: struct_seaqt_string): ptr cQSvgWidget {.importc: "QSvgWidget_new_file".}
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
    file: openArray[char],
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSvgWidget_vtable_callback_disconnectNotify
  let tmp = gen_qsvgwidget_types.QSvgWidget(h: fcQSvgWidget_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file)))), owned: true)
  fcQSvgWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
...
proc create*(T: type gen_qsvgwidget_types.QSvgWidget,
    ...
    inst: VirtualQSvgWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSvgWidget_new3(addr(cQSvgWidget_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))))
  fcQSvgWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
