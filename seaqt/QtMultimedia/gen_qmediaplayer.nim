...
proc fcQMediaPlayer_supportedAudioRoles(self: pointer): struct_seaqt_array {.importc: "QMediaPlayer_supportedAudioRoles".}
proc fcQMediaPlayer_customAudioRole(self: pointer): struct_seaqt_string {.importc: "QMediaPlayer_customAudioRole".}
proc fcQMediaPlayer_setCustomAudioRole(self: pointer, audioRole: struct_seaqt_string): void {.importc: "QMediaPlayer_setCustomAudioRole".}
proc fcQMediaPlayer_supportedCustomAudioRoles(self: pointer): struct_seaqt_array {.importc: "QMediaPlayer_supportedCustomAudioRoles".}
...
proc fcQMediaPlayer_protectedbase_addPropertyWatch(self: pointer, name: struct_seaqt_string): void {.importc: "QMediaPlayer_protectedbase_addPropertyWatch".}
proc fcQMediaPlayer_protectedbase_removePropertyWatch(self: pointer, name: struct_seaqt_string): void {.importc: "QMediaPlayer_protectedbase_removePropertyWatch".}
...
proc setCustomAudioRole*(self: gen_qmediaplayer_types.QMediaPlayer, audioRole: openArray[char]): void =
  fcQMediaPlayer_setCustomAudioRole(self.h, struct_seaqt_string(data: if len(audioRole) > 0: addr audioRole[0] else: nil, len: csize_t(len(audioRole))))
...
proc setNetworkConfigurations*(self: gen_qmediaplayer_types.QMediaPlayer, configurations: openArray[gen_qnetworkconfiguration_types.QNetworkConfiguration]): void =
  ...
  fcQMediaPlayer_setNetworkConfigurations(self.h, struct_seaqt_array(len: csize_t(len(configurations)), data: if len(configurations) == 0: nil else: addr(configurations_CArray[0])))
...
proc customAudioRoleChanged*(self: gen_qmediaplayer_types.QMediaPlayer, role: openArray[char]): void =
  fcQMediaPlayer_customAudioRoleChanged(self.h, struct_seaqt_string(data: if len(role) > 0: addr role[0] else: nil, len: csize_t(len(role))))

type QMediaPlayercustomAudioRoleChangedSlot* = proc(role: openArray[char])
proc fcQMediaPlayer_slot_callback_customAudioRoleChanged(slot: int, role: struct_seaqt_string) {.cdecl.} =
...
proc addPropertyWatch*(self: gen_qmediaplayer_types.QMediaPlayer, name: openArray[byte]): void =
  fcQMediaPlayer_protectedbase_addPropertyWatch(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

proc removePropertyWatch*(self: gen_qmediaplayer_types.QMediaPlayer, name: openArray[byte]): void =
  fcQMediaPlayer_protectedbase_removePropertyWatch(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc create*(T: type gen_qmediaplayer_types.QMediaPlayer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMediaPlayer_vtable_callback_disconnectNotify
  let tmp = gen_qmediaplayer_types.QMediaPlayer(h: fcQMediaPlayer_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h, cint(flags)), owned: true)
  fcQMediaPlayer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qmediaplayer_types.QMediaPlayer,
    ...
    inst: VirtualQMediaPlayer) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQMediaPlayer_new3(addr(cQMediaPlayer_mvtbl), csize_t(sizeof(pointer)), parent.h, cint(flags))
  fcQMediaPlayer_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
