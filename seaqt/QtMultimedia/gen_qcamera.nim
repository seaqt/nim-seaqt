...
proc fcQCamera_setViewfinder(self: pointer, surface: pointer): void {.importc: "QCamera_setViewfinder".}
...
proc fcQCamera_supportedViewfinderSettings(self: pointer): struct_seaqt_array {.importc: "QCamera_supportedViewfinderSettings".}
proc fcQCamera_supportedViewfinderResolutions(self: pointer): struct_seaqt_array {.importc: "QCamera_supportedViewfinderResolutions".}
proc fcQCamera_supportedViewfinderFrameRateRanges(self: pointer): struct_seaqt_array {.importc: "QCamera_supportedViewfinderFrameRateRanges".}
proc fcQCamera_supportedViewfinderPixelFormats(self: pointer): struct_seaqt_array {.importc: "QCamera_supportedViewfinderPixelFormats".}
...
proc fcQCamera_protectedbase_addPropertyWatch(self: pointer, name: struct_seaqt_string): void {.importc: "QCamera_protectedbase_addPropertyWatch".}
proc fcQCamera_protectedbase_removePropertyWatch(self: pointer, name: struct_seaqt_string): void {.importc: "QCamera_protectedbase_removePropertyWatch".}
...
proc deviceDescription*(_: type gen_qcamera_types.QCamera, device: openArray[byte]): string =
  let v_ms = fcQCamera_deviceDescription(struct_seaqt_string(data: if len(device) > 0: addr device[0] else: nil, len: csize_t(len(device))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setViewfinder*(self: gen_qcamera_types.QCamera, surface: gen_qabstractvideosurface_types.QAbstractVideoSurface): void =
  fcQCamera_setViewfinder(self.h, surface.h)
...
proc fcQCamera_slot_callback_stateChanged(slot: int, state: cint) {.cdecl.} =
  let nimfunc = cast[ptr QCamerastateChangedSlot](cast[pointer](slot))
  let slotval1 = cint(state)

  nimfunc[](slotval1)

proc fcQCamera_slot_callback_stateChanged_release(slot: int) {.cdecl.} =
...
proc onStateChanged*(self: gen_qcamera_types.QCamera, slot: QCamerastateChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQCamera_connect_stateChanged(self.h, cast[int](addr tmp[]), fcQCamera_slot_callback_stateChanged, fcQCamera_slot_callback_stateChanged_release)
...
proc fcQCamera_slot_callback_captureModeChanged(slot: int, param1: cint) {.cdecl.} =
  let nimfunc = cast[ptr QCameracaptureModeChangedSlot](cast[pointer](slot))
  let slotval1 = cint(param1)

  nimfunc[](slotval1)

proc fcQCamera_slot_callback_captureModeChanged_release(slot: int) {.cdecl.} =
...
proc onCaptureModeChanged*(self: gen_qcamera_types.QCamera, slot: QCameracaptureModeChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQCamera_connect_captureModeChanged(self.h, cast[int](addr tmp[]), fcQCamera_slot_callback_captureModeChanged, fcQCamera_slot_callback_captureModeChanged_release)
...
proc fcQCamera_slot_callback_statusChanged(slot: int, status: cint) {.cdecl.} =
  let nimfunc = cast[ptr QCamerastatusChangedSlot](cast[pointer](slot))
  let slotval1 = cint(status)

  nimfunc[](slotval1)

proc fcQCamera_slot_callback_statusChanged_release(slot: int) {.cdecl.} =
...
proc onStatusChanged*(self: gen_qcamera_types.QCamera, slot: QCamerastatusChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQCamera_connect_statusChanged(self.h, cast[int](addr tmp[]), fcQCamera_slot_callback_statusChanged, fcQCamera_slot_callback_statusChanged_release)

proc locked*(self: gen_qcamera_types.QCamera): void =
  fcQCamera_locked(self.h)

type QCameralockedSlot* = proc()
proc fcQCamera_slot_callback_locked(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QCameralockedSlot](cast[pointer](slot))
  nimfunc[]()

proc fcQCamera_slot_callback_locked_release(slot: int) {.cdecl.} =
...
proc onLocked*(self: gen_qcamera_types.QCamera, slot: QCameralockedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQCamera_connect_locked(self.h, cast[int](addr tmp[]), fcQCamera_slot_callback_locked, fcQCamera_slot_callback_locked_release)

proc lockFailed*(self: gen_qcamera_types.QCamera): void =
  fcQCamera_lockFailed(self.h)

type QCameralockFailedSlot* = proc()
proc fcQCamera_slot_callback_lockFailed(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QCameralockFailedSlot](cast[pointer](slot))
  nimfunc[]()

proc fcQCamera_slot_callback_lockFailed_release(slot: int) {.cdecl.} =
...
proc fcQCamera_slot_callback_errorOccurred(slot: int, param1: cint) {.cdecl.} =
...
proc fcQCamera_vtable_callback_availability(self: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QCameraVTable](fcQCamera_vdata(self)[])
  let self = QCamera(h: self)
  var virtualReturn = vtbl[].availability(self)
  cint(virtualReturn)

proc fcQCamera_vtable_callback_isAvailable(self: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QCameraVTable](fcQCamera_vdata(self)[])
  let self = QCamera(h: self)
  var virtualReturn = vtbl[].isAvailable(self)
  virtualReturn

proc fcQCamera_vtable_callback_service(self: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QCameraVTable](fcQCamera_vdata(self)[])
  let self = QCamera(h: self)
  var virtualReturn = vtbl[].service(self)
  virtualReturn.h

proc fcQCamera_vtable_callback_bindX(self: pointer, param1: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QCameraVTable](fcQCamera_vdata(self)[])
  let self = QCamera(h: self)
...
proc fcQCamera_vtable_callback_unbind(self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QCameraVTable](fcQCamera_vdata(self)[])
  let self = QCamera(h: self)
  let slotval1 = gen_qobject_types.QObject(h: param1)
...
proc fcQCamera_method_callback_availability(self: pointer): cint {.cdecl.} =
  let inst = cast[VirtualQCamera](fcQCamera_vdata(self)[])
  var virtualReturn = inst.availability()
  cint(virtualReturn)

proc fcQCamera_method_callback_isAvailable(self: pointer): bool {.cdecl.} =
  let inst = cast[VirtualQCamera](fcQCamera_vdata(self)[])
  var virtualReturn = inst.isAvailable()
  virtualReturn

proc fcQCamera_method_callback_service(self: pointer): pointer {.cdecl.} =
  let inst = cast[VirtualQCamera](fcQCamera_vdata(self)[])
  var virtualReturn = inst.service()
  virtualReturn.h

proc fcQCamera_method_callback_bindX(self: pointer, param1: pointer): bool {.cdecl.} =
  let inst = cast[VirtualQCamera](fcQCamera_vdata(self)[])
  let slotval1 = gen_qobject_types.QObject(h: param1)
  var virtualReturn = inst.bindX(slotval1)
  virtualReturn

proc fcQCamera_method_callback_unbind(self: pointer, param1: pointer): void {.cdecl.} =
  let inst = cast[VirtualQCamera](fcQCamera_vdata(self)[])
  let slotval1 = gen_qobject_types.QObject(h: param1)
...
proc addPropertyWatch*(self: gen_qcamera_types.QCamera, name: openArray[byte]): void =
  fcQCamera_protectedbase_addPropertyWatch(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

proc removePropertyWatch*(self: gen_qcamera_types.QCamera, name: openArray[byte]): void =
  fcQCamera_protectedbase_removePropertyWatch(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].availability):
    vtbl[].vtbl.availability = fcQCamera_vtable_callback_availability
  if not isNil(vtbl[].isAvailable):
    vtbl[].vtbl.isAvailable = fcQCamera_vtable_callback_isAvailable
  if not isNil(vtbl[].service):
    vtbl[].vtbl.service = fcQCamera_vtable_callback_service
  if not isNil(vtbl[].bindX):
    vtbl[].vtbl.bindX = fcQCamera_vtable_callback_bindX
  if not isNil(vtbl[].unbind):
    vtbl[].vtbl.unbind = fcQCamera_vtable_callback_unbind
...
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(deviceName) > 0: addr deviceName[0] else: nil, len: csize_t(len(deviceName)))))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cameraInfo.h))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
    position: cint,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(position)))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(deviceName) > 0: addr deviceName[0] else: nil, len: csize_t(len(deviceName))), parent.h))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cameraInfo.h, parent.h))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcamera_types.QCamera,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCamera_vtable_callback_disconnectNotify
  let tmp = gen_qcamera_types.QCamera(h: fcQCamera_new8(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(position), parent.h))
  fcQCamera_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQCamera_mvtbl = cQCameraVTable(
  ...
  availability: fcQCamera_method_callback_availability,
  isAvailable: fcQCamera_method_callback_isAvailable,
  service: fcQCamera_method_callback_service,
  bindX: fcQCamera_method_callback_bindX,
  unbind: fcQCamera_method_callback_unbind,
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new2(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(deviceName) > 0: addr deviceName[0] else: nil, len: csize_t(len(deviceName))))
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new3(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), cameraInfo.h)
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new4(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), cint(position))
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new5(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new6(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(deviceName) > 0: addr deviceName[0] else: nil, len: csize_t(len(deviceName))), parent.h)
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new7(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), cameraInfo.h, parent.h)
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcamera_types.QCamera,
    ...
    inst: VirtualQCamera) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCamera_new8(addr(cQCamera_mvtbl), csize_t(sizeof(pointer)), cint(position), parent.h)
  fcQCamera_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc minimumFrameRate*(self: gen_qcamera_types.QCameraFrameRateRange): float64 =
  fcQCameraFrameRateRange_minimumFrameRate(self.h)

proc setMinimumFrameRate*(self: gen_qcamera_types.QCameraFrameRateRange, minimumFrameRate: float64): void =
  fcQCameraFrameRateRange_setMinimumFrameRate(self.h, minimumFrameRate)

proc maximumFrameRate*(self: gen_qcamera_types.QCameraFrameRateRange): float64 =
  fcQCameraFrameRateRange_maximumFrameRate(self.h)

proc setMaximumFrameRate*(self: gen_qcamera_types.QCameraFrameRateRange, maximumFrameRate: float64): void =
  fcQCameraFrameRateRange_setMaximumFrameRate(self.h, maximumFrameRate)

proc create*(T: type gen_qcamera_types.QCameraFrameRateRange): gen_qcamera_types.QCameraFrameRateRange =
  let tmp = gen_qcamera_types.QCameraFrameRateRange(h: fcQCameraFrameRateRange_new())
  tmp
proc create*(T: type gen_qcamera_types.QCameraFrameRateRange,
  ...
  let tmp = gen_qcamera_types.QCameraFrameRateRange(h: fcQCameraFrameRateRange_new2(minimum, maximum))
  tmp
proc create*(T: type gen_qcamera_types.QCameraFrameRateRange,
    param1: gen_qcamera_types.QCameraFrameRateRange): gen_qcamera_types.QCameraFrameRateRange =
  let tmp = gen_qcamera_types.QCameraFrameRateRange(h: fcQCameraFrameRateRange_new3(param1.h))
  tmp
