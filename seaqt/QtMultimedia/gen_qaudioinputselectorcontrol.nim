...
{.compile("gen_qaudioinputselectorcontrol.cpp", QtMultimediaCFlags).}
...
proc fcQAudioInputSelectorControl_activeInput(self: pointer): struct_seaqt_string {.importc: "QAudioInputSelectorControl_activeInput".}
proc fcQAudioInputSelectorControl_setActiveInput(self: pointer, name: struct_seaqt_string): void {.importc: "QAudioInputSelectorControl_setActiveInput".}
proc fcQAudioInputSelectorControl_activeInputChanged(self: pointer, name: struct_seaqt_string): void {.importc: "QAudioInputSelectorControl_activeInputChanged".}
...
proc inputDescription*(self: gen_qaudioinputselectorcontrol_types.QAudioInputSelectorControl, name: openArray[char]): string =
  let v_ms = fcQAudioInputSelectorControl_inputDescription(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setActiveInput*(self: gen_qaudioinputselectorcontrol_types.QAudioInputSelectorControl, name: openArray[char]): void =
  fcQAudioInputSelectorControl_setActiveInput(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

proc activeInputChanged*(self: gen_qaudioinputselectorcontrol_types.QAudioInputSelectorControl, name: openArray[char]): void =
  fcQAudioInputSelectorControl_activeInputChanged(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

type QAudioInputSelectorControlactiveInputChangedSlot* = proc(name: openArray[char])
proc fcQAudioInputSelectorControl_slot_callback_activeInputChanged(slot: int, name: struct_seaqt_string) {.cdecl.} =
...
proc fcQAudioInputSelectorControl_slot_callback_activeInputChanged_release(slot: int) {.cdecl.} =
...
proc onActiveInputChanged*(self: gen_qaudioinputselectorcontrol_types.QAudioInputSelectorControl, slot: QAudioInputSelectorControlactiveInputChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQAudioInputSelectorControl_connect_activeInputChanged(self.h, cast[int](addr tmp[]), fcQAudioInputSelectorControl_slot_callback_activeInputChanged, fcQAudioInputSelectorControl_slot_callback_activeInputChanged_release)
...
proc fcQAudioInputSelectorControl_slot_callback_availableInputsChanged(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QAudioInputSelectorControlavailableInputsChangedSlot](cast[pointer](slot))
  nimfunc[]()

proc fcQAudioInputSelectorControl_slot_callback_availableInputsChanged_release(slot: int) {.cdecl.} =
