...
{.compile("gen_qimageencodercontrol.cpp", QtMultimediaCFlags).}
...
proc imageCodecDescription*(self: gen_qimageencodercontrol_types.QImageEncoderControl, codec: openArray[char]): string =
  let v_ms = fcQImageEncoderControl_imageCodecDescription(self.h, struct_seaqt_string(data: if len(codec) > 0: addr codec[0] else: nil, len: csize_t(len(codec))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
