...
proc fcQImageCapture_errorString(self: pointer): struct_seaqt_string {.importc: "QImageCapture_errorString".}
...
proc fcQImageCapture_supportedFormats(): struct_seaqt_array {.importc: "QImageCapture_supportedFormats".}
...
proc fcQImageCapture_errorOccurred(self: pointer, id: cint, error: cint, errorString: struct_seaqt_string): void {.importc: "QImageCapture_errorOccurred".}
proc fcQImageCapture_connect_errorOccurred(self: pointer, slot: int, callback: proc (slot: int, id: cint, error: cint, errorString: struct_seaqt_string) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QImageCapture_connect_errorOccurred".}
...
proc errorOccurred*(self: gen_qimagecapture_types.QImageCapture, id: cint, error: cint, errorString: openArray[char]): void =
  fcQImageCapture_errorOccurred(self.h, id, cint(error), struct_seaqt_string(data: if len(errorString) > 0: addr errorString[0] else: nil, len: csize_t(len(errorString))))

type QImageCaptureerrorOccurredSlot* = proc(id: cint, error: cint, errorString: openArray[char])
proc fcQImageCapture_slot_callback_errorOccurred(slot: int, id: cint, error: cint, errorString: struct_seaqt_string) {.cdecl.} =
...
proc imageSaved*(self: gen_qimagecapture_types.QImageCapture, id: cint, fileName: openArray[char]): void =
  fcQImageCapture_imageSaved(self.h, id, struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))

type QImageCaptureimageSavedSlot* = proc(id: cint, fileName: openArray[char])
proc fcQImageCapture_slot_callback_imageSaved(slot: int, id: cint, fileName: struct_seaqt_string) {.cdecl.} =
...
proc create*(T: type gen_qimagecapture_types.QImageCapture,
  ...
  let tmp = gen_qimagecapture_types.QImageCapture(h: fcQImageCapture_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQImageCapture_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qimagecapture_types.QImageCapture,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQImageCapture_vtable_callback_disconnectNotify
  let tmp = gen_qimagecapture_types.QImageCapture(h: fcQImageCapture_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQImageCapture_vdata(tmp.h)[] = addr(vtbl[])
  tmp
