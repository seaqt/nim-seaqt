...
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
  ...
  let tmp = gen_qmediaplaylist_types.QMediaPlaylist(h: fcQMediaPlaylist_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMediaPlaylist_vtable_callback_disconnectNotify
  let tmp = gen_qmediaplaylist_types.QMediaPlaylist(h: fcQMediaPlaylist_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQMediaPlaylist_vdata(tmp.h)[] = addr(vtbl[])
  tmp
