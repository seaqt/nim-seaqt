...
{.compile("gen_qmediaplaylist.cpp", QtMultimediaCFlags).}
...
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
  ...
  let tmp = gen_qmediaplaylist_types.QMediaPlaylist(h: fcQMediaPlaylist_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQMediaPlaylist_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMediaPlaylist_vtable_callback_disconnectNotify
  let tmp = gen_qmediaplaylist_types.QMediaPlaylist(h: fcQMediaPlaylist_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQMediaPlaylist_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQMediaPlaylist_mvtbl = cQMediaPlaylistVTable(
  destructor: proc(self: pointer) {.cdecl.} =
    let inst = cast[ptr typeof(VirtualQMediaPlaylist()[])](self.fcQMediaPlaylist_vdata()[])
    inst[].h = nil
    inst[].owned = false,
...
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
  ...
  inst[].h = fcQMediaPlaylist_new(addr(cQMediaPlaylist_mvtbl), csize_t(sizeof(pointer)))
  fcQMediaPlaylist_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qmediaplaylist_types.QMediaPlaylist,
    ...
    inst: VirtualQMediaPlaylist) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQMediaPlaylist_new2(addr(cQMediaPlaylist_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQMediaPlaylist_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
