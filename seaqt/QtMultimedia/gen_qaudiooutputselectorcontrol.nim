...
{.compile("gen_qaudiooutputselectorcontrol.cpp", QtMultimediaCFlags).}
...
proc fcQAudioOutputSelectorControl_activeOutput(self: pointer): struct_seaqt_string {.importc: "QAudioOutputSelectorControl_activeOutput".}
proc fcQAudioOutputSelectorControl_setActiveOutput(self: pointer, name: struct_seaqt_string): void {.importc: "QAudioOutputSelectorControl_setActiveOutput".}
proc fcQAudioOutputSelectorControl_activeOutputChanged(self: pointer, name: struct_seaqt_string): void {.importc: "QAudioOutputSelectorControl_activeOutputChanged".}
...
proc outputDescription*(self: gen_qaudiooutputselectorcontrol_types.QAudioOutputSelectorControl, name: openArray[char]): string =
  let v_ms = fcQAudioOutputSelectorControl_outputDescription(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setActiveOutput*(self: gen_qaudiooutputselectorcontrol_types.QAudioOutputSelectorControl, name: openArray[char]): void =
  fcQAudioOutputSelectorControl_setActiveOutput(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

proc activeOutputChanged*(self: gen_qaudiooutputselectorcontrol_types.QAudioOutputSelectorControl, name: openArray[char]): void =
  fcQAudioOutputSelectorControl_activeOutputChanged(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

type QAudioOutputSelectorControlactiveOutputChangedSlot* = proc(name: openArray[char])
proc fcQAudioOutputSelectorControl_slot_callback_activeOutputChanged(slot: int, name: struct_seaqt_string) {.cdecl.} =
...
proc fcQAudioOutputSelectorControl_slot_callback_activeOutputChanged_release(slot: int) {.cdecl.} =
...
proc onActiveOutputChanged*(self: gen_qaudiooutputselectorcontrol_types.QAudioOutputSelectorControl, slot: QAudioOutputSelectorControlactiveOutputChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQAudioOutputSelectorControl_connect_activeOutputChanged(self.h, cast[int](addr tmp[]), fcQAudioOutputSelectorControl_slot_callback_activeOutputChanged, fcQAudioOutputSelectorControl_slot_callback_activeOutputChanged_release)
...
proc fcQAudioOutputSelectorControl_slot_callback_availableOutputsChanged(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QAudioOutputSelectorControlavailableOutputsChangedSlot](cast[pointer](slot))
  nimfunc[]()

proc fcQAudioOutputSelectorControl_slot_callback_availableOutputsChanged_release(slot: int) {.cdecl.} =
