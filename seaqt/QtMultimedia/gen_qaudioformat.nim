...
proc fcQAudioFormat_setCodec(self: pointer, codec: struct_seaqt_string): void {.importc: "QAudioFormat_setCodec".}
proc fcQAudioFormat_codec(self: pointer): struct_seaqt_string {.importc: "QAudioFormat_codec".}
...
proc setCodec*(self: gen_qaudioformat_types.QAudioFormat, codec: openArray[char]): void =
  fcQAudioFormat_setCodec(self.h, struct_seaqt_string(data: if len(codec) > 0: addr codec[0] else: nil, len: csize_t(len(codec))))
