...
proc cQSoundEffect_slot_callback_audioDeviceChanged(slot: int) {.cdecl.} =
  let nimfunc = cast[ptr QSoundEffectaudioDeviceChangedSlot](cast[pointer](slot))
  nimfunc[]()

proc cQSoundEffect_slot_callback_audioDeviceChanged_release(slot: int) {.cdecl.} =
...
proc onaudioDeviceChanged*(self: gen_qsoundeffect_types.QSoundEffect, slot: QSoundEffectaudioDeviceChangedSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQSoundEffect_connect_audioDeviceChanged(self.h, cast[int](addr tmp[]), cQSoundEffect_slot_callback_audioDeviceChanged, cQSoundEffect_slot_callback_audioDeviceChanged_release)
