...
{.compile("gen_qmedianetworkaccesscontrol.cpp", QtMultimediaCFlags).}
...
proc setConfigurations*(self: gen_qmedianetworkaccesscontrol_types.QMediaNetworkAccessControl, configuration: openArray[gen_qnetworkconfiguration_types.QNetworkConfiguration]): void =
  ...
  fcQMediaNetworkAccessControl_setConfigurations(self.h, struct_seaqt_array(len: csize_t(len(configuration)), data: if len(configuration) == 0: nil else: addr(configuration_CArray[0])))
