...
proc create*(T: type gen_qwebview_types.QWebView,
  ...
  let tmp = gen_qwebview_types.QWebView(h: fcQWebView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qwebview_types.QWebView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWebView_vtable_callback_disconnectNotify
  let tmp = gen_qwebview_types.QWebView(h: fcQWebView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQWebView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qwebview_types.QWebView,
  ...
  inst[].h = fcQWebView_new(addr(cQWebView_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qwebview_types.QWebView,
    ...
    inst: VirtualQWebView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQWebView_new2(addr(cQWebView_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQWebView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
