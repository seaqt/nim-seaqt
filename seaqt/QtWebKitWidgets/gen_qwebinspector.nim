...
proc fcQWebInspector_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QWebInspector_virtualbase_nativeEvent".}
...
proc QWebInspectornativeEvent*(self: gen_qwebinspector_types.QWebInspector, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQWebInspector_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc create*(T: type gen_qwebinspector_types.QWebInspector,
  ...
  let tmp = gen_qwebinspector_types.QWebInspector(h: fcQWebInspector_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qwebinspector_types.QWebInspector,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWebInspector_vtable_callback_disconnectNotify
  let tmp = gen_qwebinspector_types.QWebInspector(h: fcQWebInspector_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQWebInspector_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qwebinspector_types.QWebInspector,
  ...
  inst[].h = fcQWebInspector_new(addr(cQWebInspector_mvtbl), csize_t(sizeof(pointer)))
  fcQWebInspector_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qwebinspector_types.QWebInspector,
    ...
    inst: VirtualQWebInspector) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQWebInspector_new2(addr(cQWebInspector_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQWebInspector_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
