...
proc fcQCameraViewfinder_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QCameraViewfinder_virtualbase_nativeEvent".}
...
proc fcQCameraViewfinder_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQCameraViewfinder {.importc: "QCameraViewfinder_new_parent".}
proc fcQCameraViewfinder_new2(vtbl: pointer, vdata: csize_t): ptr cQCameraViewfinder {.importc: "QCameraViewfinder_new".}
...
proc QCameraViewfindernativeEvent*(self: gen_qcameraviewfinder_types.QCameraViewfinder, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQCameraViewfinder_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQCameraViewfinder_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QCameraViewfinderVTable](fcQCameraViewfinder_vdata(self)[])
  let self = QCameraViewfinder(h: self)
...
proc fcQCameraViewfinder_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQCameraViewfinder](fcQCameraViewfinder_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qcameraviewfinder_types.QCameraViewfinder,
  ...
  let tmp = gen_qcameraviewfinder_types.QCameraViewfinder(h: fcQCameraViewfinder_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQCameraViewfinder_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcameraviewfinder_types.QCameraViewfinder,
    vtbl: ref QCameraViewfinderVTable = nil): gen_qcameraviewfinder_types.QCameraViewfinder =
  let vtbl = if vtbl == nil: new QCameraViewfinderVTable else: vtbl
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCameraViewfinder_vtable_callback_disconnectNotify
  let tmp = gen_qcameraviewfinder_types.QCameraViewfinder(h: fcQCameraViewfinder_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQCameraViewfinder_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qcameraviewfinder_types.QCameraViewfinder,
  ...
  inst[].h = fcQCameraViewfinder_new(addr(cQCameraViewfinder_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQCameraViewfinder_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true

proc create*(T: type gen_qcameraviewfinder_types.QCameraViewfinder,
    inst: VirtualQCameraViewfinder) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCameraViewfinder_new2(addr(cQCameraViewfinder_mvtbl), csize_t(sizeof(pointer)))
  fcQCameraViewfinder_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
