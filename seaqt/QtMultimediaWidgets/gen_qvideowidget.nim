...
proc fcQVideoWidget_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QVideoWidget_virtualbase_nativeEvent".}
...
proc fcQVideoWidget_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QVideoWidget_protectedbase_getDecodedMetricF".}
...
proc fcQVideoWidget_slot_callback_aspectRatioModeChanged(slot: int, mode: cint) {.cdecl.} =
  let nimfunc = cast[ptr QVideoWidgetaspectRatioModeChangedSlot](cast[pointer](slot))
  let slotval1 = cint(mode)

  nimfunc[](slotval1)

proc fcQVideoWidget_slot_callback_aspectRatioModeChanged_release(slot: int) {.cdecl.} =
...
proc fcQVideoWidget_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QVideoWidgetVTable](fcQVideoWidget_vdata(self)[])
  let self = QVideoWidget(h: self)
...
proc fcQVideoWidget_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQVideoWidget](fcQVideoWidget_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qvideowidget_types.QVideoWidget, metricA: cint, metricB: cint): float64 =
  fcQVideoWidget_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
