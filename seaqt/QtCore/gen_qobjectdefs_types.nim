...
type QMetaMethodArgument* {.inheritable.} = object
  h*: pointer
  owned*: bool

proc fcQMetaMethodArgument_delete(self: pointer) {.importc: "QMetaMethodArgument_delete".}
proc `=destroy`(self: var QMetaMethodArgument) =
  if self.owned: fcQMetaMethodArgument_delete(self.h)

proc `=sink`(dest: var QMetaMethodArgument, source: QMetaMethodArgument) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QMetaMethodArgument, source: QMetaMethodArgument) {.error.}
proc delete*(self: sink QMetaMethodArgument) =
  let h = self.h
  wasMoved(self)
  fcQMetaMethodArgument_delete(h)

type QMetaMethodReturnArgument* {.inheritable.} = object
  h*: pointer
  owned*: bool

proc fcQMetaMethodReturnArgument_delete(self: pointer) {.importc: "QMetaMethodReturnArgument_delete".}
proc `=destroy`(self: var QMetaMethodReturnArgument) =
  if self.owned: fcQMetaMethodReturnArgument_delete(self.h)

proc `=sink`(dest: var QMetaMethodReturnArgument, source: QMetaMethodReturnArgument) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QMetaMethodReturnArgument, source: QMetaMethodReturnArgument) {.error.}
proc delete*(self: sink QMetaMethodReturnArgument) =
  let h = self.h
  wasMoved(self)
  fcQMetaMethodReturnArgument_delete(h)
