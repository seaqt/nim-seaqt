...
proc fcQVariantAnimation_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QVariantAnimation_trUtf8_s".}
...
proc fcQVariantAnimation_keyValues(self: pointer): struct_seaqt_array {.importc: "QVariantAnimation_keyValues".}
proc fcQVariantAnimation_setKeyValues(self: pointer, values: struct_seaqt_array): void {.importc: "QVariantAnimation_setKeyValues".}
...
proc fcQVariantAnimation_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QVariantAnimation_trUtf8_s_c".}
proc fcQVariantAnimation_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QVariantAnimation_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qvariantanimation_types.QVariantAnimation, s: cstring): string =
  let v_ms = fcQVariantAnimation_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setKeyValues*(self: gen_qvariantanimation_types.QVariantAnimation, values: openArray[tuple[first: float64, second: gen_qvariant_types.QVariant]]): void =
  var values_CArray = newSeq[struct_seaqt_map](len(values))
  for i in 0..<len(values):
    ...
    values_CArray[i] = struct_seaqt_map(len: 1,keys: addr(values_i_CArray_First),values: addr(values_i_CArray_Second),)

  fcQVariantAnimation_setKeyValues(self.h, struct_seaqt_array(len: csize_t(len(values)), data: if len(values) == 0: nil else: addr(values_CArray[0])))
...
proc create*(T: type gen_qvariantanimation_types.QVariantAnimation,
  ...
  let tmp = gen_qvariantanimation_types.QVariantAnimation(h: fcQVariantAnimation_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQVariantAnimation_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qvariantanimation_types.QVariantAnimation,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQVariantAnimation_vtable_callback_disconnectNotify
  let tmp = gen_qvariantanimation_types.QVariantAnimation(h: fcQVariantAnimation_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQVariantAnimation_vdata(tmp.h)[] = addr(vtbl[])
  tmp
