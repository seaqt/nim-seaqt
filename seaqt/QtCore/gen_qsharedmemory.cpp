...
VirtualQSharedMemory* QSharedMemory_new(const QSharedMemory_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQSharedMemory>());
	return _mem_ ? new (_mem_)VirtualQSharedMemory(vtbl) : nullptr;
}

VirtualQSharedMemory* QSharedMemory_new2(const QSharedMemory_VTable* vtbl, size_t vdata, struct miqt_string key) {
...
VirtualQSharedMemory* QSharedMemory_new4(const QSharedMemory_VTable* vtbl, size_t vdata, struct miqt_string key, QObject* parent) {
	QString key_QString = QString::fromUtf8(key.data, key.len);
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQSharedMemory>());
