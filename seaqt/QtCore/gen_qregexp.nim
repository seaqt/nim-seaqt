...
proc fcQRegExp_indexIn(self: pointer, str: struct_seaqt_string): cint {.importc: "QRegExp_indexIn".}
proc fcQRegExp_lastIndexIn(self: pointer, str: struct_seaqt_string): cint {.importc: "QRegExp_lastIndexIn".}
...
proc fcQRegExp_capturedTexts2(self: pointer): struct_seaqt_array {.importc: "QRegExp_capturedTexts2".}
proc fcQRegExp_cap(self: pointer): struct_seaqt_string {.importc: "QRegExp_cap".}
proc fcQRegExp_cap2(self: pointer): struct_seaqt_string {.importc: "QRegExp_cap2".}
...
proc fcQRegExp_errorString2(self: pointer): struct_seaqt_string {.importc: "QRegExp_errorString2".}
proc fcQRegExp_escape(str: struct_seaqt_string): struct_seaqt_string {.importc: "QRegExp_escape".}
proc fcQRegExp_indexIn2(self: pointer, str: struct_seaqt_string, offset: cint): cint {.importc: "QRegExp_indexIn2".}
proc fcQRegExp_indexIn3(self: pointer, str: struct_seaqt_string, offset: cint, caretMode: cint): cint {.importc: "QRegExp_indexIn3".}
proc fcQRegExp_lastIndexIn2(self: pointer, str: struct_seaqt_string, offset: cint): cint {.importc: "QRegExp_lastIndexIn2".}
proc fcQRegExp_lastIndexIn3(self: pointer, str: struct_seaqt_string, offset: cint, caretMode: cint): cint {.importc: "QRegExp_lastIndexIn3".}
proc fcQRegExp_capWithNth(self: pointer, nth: cint): struct_seaqt_string {.importc: "QRegExp_capWithNth".}
proc fcQRegExp_cap3(self: pointer, nth: cint): struct_seaqt_string {.importc: "QRegExp_cap3".}
proc fcQRegExp_posWithNth(self: pointer, nth: cint): cint {.importc: "QRegExp_posWithNth".}
proc fcQRegExp_pos3(self: pointer, nth: cint): cint {.importc: "QRegExp_pos3".}
proc fcQRegExp_new(): ptr cQRegExp {.importc: "QRegExp_new".}
proc fcQRegExp_new2(pattern: struct_seaqt_string): ptr cQRegExp {.importc: "QRegExp_new2".}
...
proc indexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char]): cint =
  fcQRegExp_indexIn(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))))

proc lastIndexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char]): cint =
  fcQRegExp_lastIndexIn(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))))
...
proc indexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char], offset: cint): cint =
  fcQRegExp_indexIn2(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))), offset)

proc indexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char], offset: cint, caretMode: cint): cint =
  fcQRegExp_indexIn3(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))), offset, cint(caretMode))

proc lastIndexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char], offset: cint): cint =
  fcQRegExp_lastIndexIn2(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))), offset)

proc lastIndexIn*(self: gen_qregexp_types.QRegExp, str: openArray[char], offset: cint, caretMode: cint): cint =
  fcQRegExp_lastIndexIn3(self.h, struct_seaqt_string(data: if len(str) > 0: addr str[0] else: nil, len: csize_t(len(str))), offset, cint(caretMode))

proc cap*(self: gen_qregexp_types.QRegExp, nth: cint): string =
  let v_ms = fcQRegExp_capWithNth(self.h, nth)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc cap2*(self: gen_qregexp_types.QRegExp, nth: cint): string =
  let v_ms = fcQRegExp_cap3(self.h, nth)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc pos*(self: gen_qregexp_types.QRegExp, nth: cint): cint =
  fcQRegExp_posWithNth(self.h, nth)
