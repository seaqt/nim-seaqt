...
proc fcQUrl_fromEncodedInput(input: struct_seaqt_string): pointer {.importc: "QUrl_fromEncoded_input".}
...
proc fcQUrl_fromEncodedInputMode(input: struct_seaqt_string, mode: cint): pointer {.importc: "QUrl_fromEncoded_input_mode".}
...
proc fromEncoded*(_: type gen_qurl_types.QUrl, input: openArray[byte]): gen_qurl_types.QUrl =
  gen_qurl_types.QUrl(h: fcQUrl_fromEncodedInput(struct_seaqt_string(data: if len(input) > 0: addr input[0] else: nil, len: csize_t(len(input)))), owned: true)
...
proc fromEncoded*(_: type gen_qurl_types.QUrl, input: openArray[byte], mode: cint): gen_qurl_types.QUrl =
  gen_qurl_types.QUrl(h: fcQUrl_fromEncodedInputMode(struct_seaqt_string(data: if len(input) > 0: addr input[0] else: nil, len: csize_t(len(input))), cint(mode)), owned: true)
