...
proc create*(T: type gen_qstringlistmodel_types.QStringListModel,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQStringListModel_vtable_callback_disconnectNotify
  let tmp = gen_qstringlistmodel_types.QStringListModel(h: fcQStringListModel_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_array(len: csize_t(len(strings)), data: if len(strings) == 0: nil else: addr(strings_CArray[0]))))
  fcQStringListModel_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstringlistmodel_types.QStringListModel,
...
proc create*(T: type gen_qstringlistmodel_types.QStringListModel,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQStringListModel_vtable_callback_disconnectNotify
  let tmp = gen_qstringlistmodel_types.QStringListModel(h: fcQStringListModel_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_array(len: csize_t(len(strings)), data: if len(strings) == 0: nil else: addr(strings_CArray[0])), parent.h))
  fcQStringListModel_vdata(tmp.h)[] = addr(vtbl[])
  tmp
