...
proc cQMimeData_vtable_callback_retrieveData(self: pointer, mimetype: struct_miqt_string, preferredType: cint): pointer {.cdecl.} =
  let vtbl = cast[ptr QMimeDataVTable](fcQMimeData_vdata(self))
  let self = QMimeData(h: self)
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
...
proc cQMimeData_method_callback_retrieveData(self: pointer, mimetype: struct_miqt_string, preferredType: cint): pointer {.cdecl.} =
  let inst = cast[VirtualQMimeData](fcQMimeData_vdata(self))
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
  c_free(vmimetype_ms.data)
