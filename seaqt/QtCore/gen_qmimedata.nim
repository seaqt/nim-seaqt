...
type cQMimeDataVTable {.pure.} = object
  ...
  retrieveData*: proc(self: pointer, mimetype: struct_seaqt_string, preferredType: pointer): pointer {.cdecl, raises: [], gcsafe.}
...
proc fcQMimeData_virtualbase_retrieveData(self: pointer, mimetype: struct_seaqt_string, preferredType: pointer): pointer {.importc: "QMimeData_virtualbase_retrieveData".}
...
proc fcQMimeData_vtable_callback_retrieveData(self: pointer, mimetype: struct_seaqt_string, preferredType: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QMimeDataVTable](fcQMimeData_vdata(self)[])
  let self = QMimeData(h: self)
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
...
proc fcQMimeData_method_callback_retrieveData(self: pointer, mimetype: struct_seaqt_string, preferredType: pointer): pointer {.cdecl.} =
  let inst = cast[VirtualQMimeData](fcQMimeData_vdata(self)[])
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
  c_free(vmimetype_ms.data)
