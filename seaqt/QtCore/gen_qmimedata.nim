...
type cQMimeDataVTable {.pure.} = object
  ...
  retrieveData*: proc(self: pointer, mimetype: struct_miqt_string, preferredType: cint): pointer {.cdecl, raises: [], gcsafe.}
...
proc QMimeDataretrieveData*(self: gen_qmimedata_types.QMimeData, mimetype: openArray[char], preferredType: cint): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQMimeData_virtualbase_retrieveData(self.h, struct_miqt_string(data: if len(mimetype) > 0: addr mimetype[0] else: nil, len: csize_t(len(mimetype))), cint(preferredType)), owned: true)
...
proc fcQMimeData_vtable_callback_hasFormat(self: pointer, mimetype: struct_miqt_string): bool {.cdecl.} =
  let vtbl = cast[ptr QMimeDataVTable](fcQMimeData_vdata(self))
  let self = QMimeData(h: self)
...
proc fcQMimeData_vtable_callback_formats(self: pointer): struct_miqt_array {.cdecl.} =
  let vtbl = cast[ptr QMimeDataVTable](fcQMimeData_vdata(self))
  let self = QMimeData(h: self)
...
proc fcQMimeData_vtable_callback_retrieveData(self: pointer, mimetype: struct_miqt_string, preferredType: cint): pointer {.cdecl.} =
  let vtbl = cast[ptr QMimeDataVTable](fcQMimeData_vdata(self))
  let self = QMimeData(h: self)
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
...
proc fcQMimeData_method_callback_hasFormat(self: pointer, mimetype: struct_miqt_string): bool {.cdecl.} =
  let inst = cast[VirtualQMimeData](fcQMimeData_vdata(self))
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
  c_free(vmimetype_ms.data)
...
proc fcQMimeData_method_callback_formats(self: pointer): struct_miqt_array {.cdecl.} =
  let inst = cast[VirtualQMimeData](fcQMimeData_vdata(self))
...
proc fcQMimeData_method_callback_retrieveData(self: pointer, mimetype: struct_miqt_string, preferredType: cint): pointer {.cdecl.} =
  let inst = cast[VirtualQMimeData](fcQMimeData_vdata(self))
  let vmimetype_ms = mimetype
  let vmimetypex_ret = string.fromBytes(vmimetype_ms)
  c_free(vmimetype_ms.data)
