...
proc QMimeDataretrieveData*(self: gen_qmimedata_types.QMimeData, mimetype: openArray[char], preferredType: cint): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQMimeData_virtualbase_retrieveData(self.h, struct_seaqt_string(data: if len(mimetype) > 0: addr mimetype[0] else: nil, len: csize_t(len(mimetype))), cint(preferredType)))
