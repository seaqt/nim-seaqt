...
proc addPath*(self: gen_qfilesystemwatcher_types.QFileSystemWatcher, file: openArray[char]): bool =
  fcQFileSystemWatcher_addPath(self.h, struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))))
...
proc addPaths*(self: gen_qfilesystemwatcher_types.QFileSystemWatcher, files: openArray[string]): seq[string] =
  ...
  var v_ma = fcQFileSystemWatcher_addPaths(self.h, struct_seaqt_array(len: csize_t(len(files)), data: if len(files) == 0: nil else: addr(files_CArray[0])))
...
proc removePath*(self: gen_qfilesystemwatcher_types.QFileSystemWatcher, file: openArray[char]): bool =
  fcQFileSystemWatcher_removePath(self.h, struct_seaqt_string(data: if len(file) > 0: addr file[0] else: nil, len: csize_t(len(file))))
...
proc removePaths*(self: gen_qfilesystemwatcher_types.QFileSystemWatcher, files: openArray[string]): seq[string] =
  ...
  var v_ma = fcQFileSystemWatcher_removePaths(self.h, struct_seaqt_array(len: csize_t(len(files)), data: if len(files) == 0: nil else: addr(files_CArray[0])))
  var vx_ret = newSeq[string](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    let vx_lv_ms = v_outCast[i]
...
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  let tmp = gen_qfilesystemwatcher_types.QFileSystemWatcher(h: fcQFileSystemWatcher_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQFileSystemWatcher_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQFileSystemWatcher_vtable_callback_disconnectNotify
  let tmp = gen_qfilesystemwatcher_types.QFileSystemWatcher(h: fcQFileSystemWatcher_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(paths)), data: if len(paths) == 0: nil else: addr(paths_CArray[0]))), owned: true)
  fcQFileSystemWatcher_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQFileSystemWatcher_vtable_callback_disconnectNotify
  let tmp = gen_qfilesystemwatcher_types.QFileSystemWatcher(h: fcQFileSystemWatcher_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQFileSystemWatcher_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQFileSystemWatcher_vtable_callback_disconnectNotify
  let tmp = gen_qfilesystemwatcher_types.QFileSystemWatcher(h: fcQFileSystemWatcher_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(paths)), data: if len(paths) == 0: nil else: addr(paths_CArray[0])), parent.h), owned: true)
  fcQFileSystemWatcher_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  inst[].h = fcQFileSystemWatcher_new(addr(cQFileSystemWatcher_mvtbl), csize_t(sizeof(pointer)))
  fcQFileSystemWatcher_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQFileSystemWatcher_new2(addr(cQFileSystemWatcher_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(paths)), data: if len(paths) == 0: nil else: addr(paths_CArray[0])))
  fcQFileSystemWatcher_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
    ...
    inst: VirtualQFileSystemWatcher) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQFileSystemWatcher_new3(addr(cQFileSystemWatcher_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQFileSystemWatcher_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qfilesystemwatcher_types.QFileSystemWatcher,
  ...
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQFileSystemWatcher_new4(addr(cQFileSystemWatcher_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(paths)), data: if len(paths) == 0: nil else: addr(paths_CArray[0])), parent.h)
  fcQFileSystemWatcher_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
