type QScopedPropertyUpdateGroup* {.inheritable.} = object
  h*: pointer
  owned*: bool
...
proc fcQScopedPropertyUpdateGroup_delete(self: pointer) {.importc: "QScopedPropertyUpdateGroup_delete".}
proc `=destroy`(self: var QScopedPropertyUpdateGroup) =
  if self.owned: fcQScopedPropertyUpdateGroup_delete(self.h)

proc `=sink`(dest: var QScopedPropertyUpdateGroup, source: QScopedPropertyUpdateGroup) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QScopedPropertyUpdateGroup, source: QScopedPropertyUpdateGroup) {.error.}
proc delete*(self: sink QScopedPropertyUpdateGroup) =
  let h = self.h
  wasMoved(self)
  fcQScopedPropertyUpdateGroup_delete(h)
