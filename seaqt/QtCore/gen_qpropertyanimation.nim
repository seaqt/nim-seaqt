...
proc fcQPropertyAnimation_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QPropertyAnimation_trUtf8_s".}
...
proc fcQPropertyAnimation_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QPropertyAnimation_trUtf8_s_c".}
proc fcQPropertyAnimation_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QPropertyAnimation_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qpropertyanimation_types.QPropertyAnimation, s: cstring): string =
  let v_ms = fcQPropertyAnimation_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
  ...
  let tmp = gen_qpropertyanimation_types.QPropertyAnimation(h: fcQPropertyAnimation_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPropertyAnimation_vtable_callback_disconnectNotify
  let tmp = gen_qpropertyanimation_types.QPropertyAnimation(h: fcQPropertyAnimation_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), target.h, struct_seaqt_string(data: if len(propertyName) > 0: addr propertyName[0] else: nil, len: csize_t(len(propertyName)))), owned: true)
  fcQPropertyAnimation_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPropertyAnimation_vtable_callback_disconnectNotify
  let tmp = gen_qpropertyanimation_types.QPropertyAnimation(h: fcQPropertyAnimation_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQPropertyAnimation_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPropertyAnimation_vtable_callback_disconnectNotify
  let tmp = gen_qpropertyanimation_types.QPropertyAnimation(h: fcQPropertyAnimation_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), target.h, struct_seaqt_string(data: if len(propertyName) > 0: addr propertyName[0] else: nil, len: csize_t(len(propertyName))), parent.h), owned: true)
  fcQPropertyAnimation_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
    ...
    inst: VirtualQPropertyAnimation) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPropertyAnimation_new2(addr(cQPropertyAnimation_mvtbl), csize_t(sizeof(pointer)), target.h, struct_seaqt_string(data: if len(propertyName) > 0: addr propertyName[0] else: nil, len: csize_t(len(propertyName))))
  fcQPropertyAnimation_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpropertyanimation_types.QPropertyAnimation,
    ...
    inst: VirtualQPropertyAnimation) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPropertyAnimation_new4(addr(cQPropertyAnimation_mvtbl), csize_t(sizeof(pointer)), target.h, struct_seaqt_string(data: if len(propertyName) > 0: addr propertyName[0] else: nil, len: csize_t(len(propertyName))), parent.h)
  fcQPropertyAnimation_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
