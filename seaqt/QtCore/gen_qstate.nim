...
proc fcQState_trUtf8(s: cstring): struct_seaqt_string {.importc: "QState_trUtf8".}
...
proc fcQState_trUtf82(s: cstring, c: cstring): struct_seaqt_string {.importc: "QState_trUtf82".}
proc fcQState_trUtf83(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QState_trUtf83".}
...
proc create*(T: type gen_qstate_types.QState,
  ...
  let tmp = gen_qstate_types.QState(h: fcQState_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQState_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstate_types.QState,
    childMode: cint,
    vtbl: ref QStateVTable = nil): gen_qstate_types.QState =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQState_vtable_callback_disconnectNotify
  let tmp = gen_qstate_types.QState(h: fcQState_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(childMode)), owned: true)
  fcQState_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstate_types.QState,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQState_vtable_callback_disconnectNotify
  let tmp = gen_qstate_types.QState(h: fcQState_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQState_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstate_types.QState,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQState_vtable_callback_disconnectNotify
  let tmp = gen_qstate_types.QState(h: fcQState_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(childMode), parent.h), owned: true)
  fcQState_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qstate_types.QState,
  ...
  inst[].h = fcQState_new(addr(cQState_mvtbl), csize_t(sizeof(pointer)))
  fcQState_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qstate_types.QState,
    ...
    inst: VirtualQState) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQState_new2(addr(cQState_mvtbl), csize_t(sizeof(pointer)), cint(childMode))
  fcQState_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qstate_types.QState,
    ...
    inst: VirtualQState) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQState_new3(addr(cQState_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQState_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qstate_types.QState,
    ...
    inst: VirtualQState) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQState_new4(addr(cQState_mvtbl), csize_t(sizeof(pointer)), cint(childMode), parent.h)
  fcQState_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
