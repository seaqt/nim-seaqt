...
proc operatorSubscript*(self: gen_qjsonvalue_types.QJsonValue, key: string): gen_qjsonvalue_types.QJsonValue =
  gen_qjsonvalue_types.QJsonValue(h: fcQJsonValue_operatorSubscript(self.h, struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))))
...
proc create*(T: type gen_qjsonvalue_types.QJsonValue,
  ...
  let tmp = gen_qjsonvalue_types.QJsonValue(h: fcQJsonValue_new6(struct_miqt_string(data: if len(s) > 0: addr s[0] else: nil, len: csize_t(len(s)))))
  tmp
proc create*(T: type gen_qjsonvalue_types.QJsonValue,
