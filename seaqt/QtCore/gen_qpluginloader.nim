...
proc fcQPluginLoader_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QPluginLoader_trUtf8_s".}
...
proc fcQPluginLoader_staticPlugins(): struct_seaqt_array {.importc: "QPluginLoader_staticPlugins".}
...
proc fcQPluginLoader_setFileName(self: pointer, fileName: struct_seaqt_string): void {.importc: "QPluginLoader_setFileName".}
proc fcQPluginLoader_fileName(self: pointer): struct_seaqt_string {.importc: "QPluginLoader_fileName".}
...
proc fcQPluginLoader_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QPluginLoader_trUtf8_s_c".}
proc fcQPluginLoader_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QPluginLoader_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qpluginloader_types.QPluginLoader, s: cstring): string =
  let v_ms = fcQPluginLoader_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setFileName*(self: gen_qpluginloader_types.QPluginLoader, fileName: openArray[char]): void =
  fcQPluginLoader_setFileName(self.h, struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
...
proc fcQPluginLoader_vtable_callback_metacast(self: pointer, param1: cstring): pointer {.cdecl.} =
  let vtbl = cast[ptr QPluginLoaderVTable](fcQPluginLoader_vdata(self)[])
  let self = QPluginLoader(h: self)
  let slotval1 = (param1)
...
proc fcQPluginLoader_vtable_callback_metacall(self: pointer, param1: cint, param2: cint, param3: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QPluginLoaderVTable](fcQPluginLoader_vdata(self)[])
  let self = QPluginLoader(h: self)
...
proc fcQPluginLoader_method_callback_metaObject(self: pointer): pointer {.cdecl.} =
...
proc fcQPluginLoader_method_callback_metacast(self: pointer, param1: cstring): pointer {.cdecl.} =
  let inst = cast[VirtualQPluginLoader](fcQPluginLoader_vdata(self)[])
  let slotval1 = (param1)
  var virtualReturn = inst.metacast(slotval1)
  virtualReturn

proc fcQPluginLoader_method_callback_metacall(self: pointer, param1: cint, param2: cint, param3: pointer): cint {.cdecl.} =
  let inst = cast[VirtualQPluginLoader](fcQPluginLoader_vdata(self)[])
  let slotval1 = cint(param1)
  let slotval2 = param2
  let slotval3 = param3
...
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
  ...
  if not isNil(vtbl[].metaObject):
    vtbl[].vtbl.metaObject = fcQPluginLoader_vtable_callback_metaObject
  if not isNil(vtbl[].metacast):
    vtbl[].vtbl.metacast = fcQPluginLoader_vtable_callback_metacast
  if not isNil(vtbl[].metacall):
    vtbl[].vtbl.metacall = fcQPluginLoader_vtable_callback_metacall
  if not isNil(vtbl[].event):
    vtbl[].vtbl.event = fcQPluginLoader_vtable_callback_event
  if not isNil(vtbl[].eventFilter):
    vtbl[].vtbl.eventFilter = fcQPluginLoader_vtable_callback_eventFilter
  if not isNil(vtbl[].timerEvent):
    vtbl[].vtbl.timerEvent = fcQPluginLoader_vtable_callback_timerEvent
  if not isNil(vtbl[].childEvent):
    vtbl[].vtbl.childEvent = fcQPluginLoader_vtable_callback_childEvent
  if not isNil(vtbl[].customEvent):
    vtbl[].vtbl.customEvent = fcQPluginLoader_vtable_callback_customEvent
...
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPluginLoader_vtable_callback_disconnectNotify
  let tmp = gen_qpluginloader_types.QPluginLoader(h: fcQPluginLoader_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))), owned: true)
  fcQPluginLoader_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
...
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPluginLoader_vtable_callback_disconnectNotify
  let tmp = gen_qpluginloader_types.QPluginLoader(h: fcQPluginLoader_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), parent.h), owned: true)
  fcQPluginLoader_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQPluginLoader_mvtbl = cQPluginLoaderVTable(
  ...
  metacall: fcQPluginLoader_method_callback_metacall,
  event: fcQPluginLoader_method_callback_event,
  eventFilter: fcQPluginLoader_method_callback_eventFilter,
  timerEvent: fcQPluginLoader_method_callback_timerEvent,
  childEvent: fcQPluginLoader_method_callback_childEvent,
  customEvent: fcQPluginLoader_method_callback_customEvent,
  connectNotify: fcQPluginLoader_method_callback_connectNotify,
  disconnectNotify: fcQPluginLoader_method_callback_disconnectNotify,
)
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
...
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
    ...
    inst: VirtualQPluginLoader) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPluginLoader_new2(addr(cQPluginLoader_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
  fcQPluginLoader_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpluginloader_types.QPluginLoader,
    ...
    inst: VirtualQPluginLoader) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPluginLoader_new4(addr(cQPluginLoader_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), parent.h)
  fcQPluginLoader_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
