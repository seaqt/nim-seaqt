...
proc fcQBasicTimer_id(self: pointer): cint {.importc: "QBasicTimer_id".}
...
proc id*(self: gen_qbasictimer_types.QBasicTimer): cint =
  cint(fcQBasicTimer_id(self.h))
...
proc create*(T: type gen_qbasictimer_types.QBasicTimer): gen_qbasictimer_types.QBasicTimer =
  let tmp = gen_qbasictimer_types.QBasicTimer(h: fcQBasicTimer_new(), owned: true)
  tmp
