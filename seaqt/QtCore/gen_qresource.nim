...
proc fcQResource_setFileName(self: pointer, file: struct_seaqt_string): void {.importc: "QResource_setFileName".}
proc fcQResource_fileName(self: pointer): struct_seaqt_string {.importc: "QResource_fileName".}
proc fcQResource_absoluteFilePath(self: pointer): struct_seaqt_string {.importc: "QResource_absoluteFilePath".}
...
proc fcQResource_uncompressedData(self: pointer): struct_seaqt_string {.importc: "QResource_uncompressedData".}
...
proc addSearchPath*(_: type gen_qresource_types.QResource, path: openArray[char]): void =
  fcQResource_addSearchPath(struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
