...
proc fcQSharedMemory_setNativeKey_QNativeIpcKey(self: pointer, key: pointer): void {.importc: "QSharedMemory_setNativeKey_QNativeIpcKey".}
proc fcQSharedMemory_setNativeKey_QString(self: pointer, key: struct_seaqt_string): void {.importc: "QSharedMemory_setNativeKey_QString".}
proc fcQSharedMemory_nativeKey(self: pointer): struct_seaqt_string {.importc: "QSharedMemory_nativeKey".}
proc fcQSharedMemory_nativeIpcKey(self: pointer): pointer {.importc: "QSharedMemory_nativeIpcKey".}
proc fcQSharedMemory_createSize(self: pointer, size: int64): bool {.importc: "QSharedMemory_create_size".}
...
proc fcQSharedMemory_isKeyTypeSupported(typeVal: cint): bool {.importc: "QSharedMemory_isKeyTypeSupported".}
proc fcQSharedMemory_platformSafeKeyKey(key: struct_seaqt_string): pointer {.importc: "QSharedMemory_platformSafeKey_key".}
proc fcQSharedMemory_legacyNativeKeyKey(key: struct_seaqt_string): pointer {.importc: "QSharedMemory_legacyNativeKey_key".}
...
proc fcQSharedMemory_setNativeKey_QString_QNativeIpcKey_Type(self: pointer, key: struct_seaqt_string, typeVal: cint): void {.importc: "QSharedMemory_setNativeKey_QString_QNativeIpcKey_Type".}
proc fcQSharedMemory_createSizeMode(self: pointer, size: int64, mode: cint): bool {.importc: "QSharedMemory_create_size_mode".}
proc fcQSharedMemory_attachMode(self: pointer, mode: cint): bool {.importc: "QSharedMemory_attach_mode".}
proc fcQSharedMemory_platformSafeKeyKeyType(key: struct_seaqt_string, typeVal: cint): pointer {.importc: "QSharedMemory_platformSafeKey_key_type".}
proc fcQSharedMemory_legacyNativeKeyKeyType(key: struct_seaqt_string, typeVal: cint): pointer {.importc: "QSharedMemory_legacyNativeKey_key_type".}
...
proc fcQSharedMemory_new2(vtbl: pointer, vdata: csize_t, key: pointer): ptr cQSharedMemory {.importc: "QSharedMemory_new_QNativeIpcKey".}
proc fcQSharedMemory_new3(vtbl: pointer, vdata: csize_t, key: struct_seaqt_string): ptr cQSharedMemory {.importc: "QSharedMemory_new_QString".}
proc fcQSharedMemory_new4(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQSharedMemory {.importc: "QSharedMemory_new_QObject".}
proc fcQSharedMemory_new5(vtbl: pointer, vdata: csize_t, key: pointer, parent: pointer): ptr cQSharedMemory {.importc: "QSharedMemory_new_QNativeIpcKey_QObject".}
proc fcQSharedMemory_new6(vtbl: pointer, vdata: csize_t, key: struct_seaqt_string, parent: pointer): ptr cQSharedMemory {.importc: "QSharedMemory_new_QString_QObject".}
...
proc setKey*(self: gen_qsharedmemory_types.QSharedMemory, key: openArray[char]): void =
  fcQSharedMemory_setKey(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
...
proc setNativeKey*(self: gen_qsharedmemory_types.QSharedMemory, key: gen_qtipccommon_types.QNativeIpcKey): void =
  fcQSharedMemory_setNativeKey_QNativeIpcKey(self.h, key.h)

proc setNativeKey*(self: gen_qsharedmemory_types.QSharedMemory, key: openArray[char]): void =
  fcQSharedMemory_setNativeKey_QString(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
...
proc nativeIpcKey*(self: gen_qsharedmemory_types.QSharedMemory): gen_qtipccommon_types.QNativeIpcKey =
  gen_qtipccommon_types.QNativeIpcKey(h: fcQSharedMemory_nativeIpcKey(self.h), owned: true)

proc createX*(self: gen_qsharedmemory_types.QSharedMemory, size: int64): bool =
...
proc isKeyTypeSupported*(_: type gen_qsharedmemory_types.QSharedMemory, typeVal: cint): bool =
  fcQSharedMemory_isKeyTypeSupported(cint(typeVal))

proc platformSafeKey*(_: type gen_qsharedmemory_types.QSharedMemory, key: openArray[char]): gen_qtipccommon_types.QNativeIpcKey =
  gen_qtipccommon_types.QNativeIpcKey(h: fcQSharedMemory_platformSafeKeyKey(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: true)

proc legacyNativeKey*(_: type gen_qsharedmemory_types.QSharedMemory, key: openArray[char]): gen_qtipccommon_types.QNativeIpcKey =
  gen_qtipccommon_types.QNativeIpcKey(h: fcQSharedMemory_legacyNativeKeyKey(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: true)
...
proc setNativeKey*(self: gen_qsharedmemory_types.QSharedMemory, key: openArray[char], typeVal: cint): void =
  fcQSharedMemory_setNativeKey_QString_QNativeIpcKey_Type(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), cint(typeVal))

proc createX*(self: gen_qsharedmemory_types.QSharedMemory, size: int64, mode: cint): bool =
...
proc platformSafeKey*(_: type gen_qsharedmemory_types.QSharedMemory, key: openArray[char], typeVal: cint): gen_qtipccommon_types.QNativeIpcKey =
  gen_qtipccommon_types.QNativeIpcKey(h: fcQSharedMemory_platformSafeKeyKeyType(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), cint(typeVal)), owned: true)

proc legacyNativeKey*(_: type gen_qsharedmemory_types.QSharedMemory, key: openArray[char], typeVal: cint): gen_qtipccommon_types.QNativeIpcKey =
  gen_qtipccommon_types.QNativeIpcKey(h: fcQSharedMemory_legacyNativeKeyKeyType(struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), cint(typeVal)), owned: true)
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    key: gen_qtipccommon_types.QNativeIpcKey,
    vtbl: ref QSharedMemoryVTable = nil): gen_qsharedmemory_types.QSharedMemory =
  let vtbl = if vtbl == nil: new QSharedMemoryVTable else: vtbl
  GC_ref(vtbl)
  vtbl[].vtbl.destructor = proc(self: pointer) {.cdecl.} =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), key.h), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    key: openArray[char],
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    key: gen_qtipccommon_types.QNativeIpcKey, parent: gen_qobject_types.QObject,
    vtbl: ref QSharedMemoryVTable = nil): gen_qsharedmemory_types.QSharedMemory =
  let vtbl = if vtbl == nil: new QSharedMemoryVTable else: vtbl
  GC_ref(vtbl)
  vtbl[].vtbl.destructor = proc(self: pointer) {.cdecl.} =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), key.h, parent.h), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), parent.h), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  inst[].h = fcQSharedMemory_new(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)))
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new2(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), key.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new3(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new4(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new5(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), key.h, parent.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new6(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), parent.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
