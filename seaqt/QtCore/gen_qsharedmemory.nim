...
proc setNativeKey*(self: gen_qsharedmemory_types.QSharedMemory, key: openArray[char]): void =
  fcQSharedMemory_setNativeKey(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSharedMemory_vtable_callback_disconnectNotify
  let tmp = gen_qsharedmemory_types.QSharedMemory(h: fcQSharedMemory_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), parent.h), owned: true)
  fcQSharedMemory_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new2(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new3(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsharedmemory_types.QSharedMemory,
    ...
    inst: VirtualQSharedMemory) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSharedMemory_new4(addr(cQSharedMemory_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), parent.h)
  fcQSharedMemory_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
