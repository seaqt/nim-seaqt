...
proc create*(T: type gen_qproperty_types.QPropertyBindingError,
  ...
  let tmp = gen_qproperty_types.QPropertyBindingError(h: fcQPropertyBindingError_new4(cint(typeVal), struct_miqt_string(data: if len(description) > 0: addr description[0] else: nil, len: csize_t(len(description)))))
  tmp
...
proc create*(T: type gen_qproperty_types.QUntypedPropertyBinding,
  ...
  let tmp = gen_qproperty_types.QUntypedPropertyBinding(h: fcQUntypedPropertyBinding_new2(other.h))
  tmp
