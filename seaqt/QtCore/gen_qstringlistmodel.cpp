...
VirtualQStringListModel* QStringListModel_new(const QStringListModel_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQStringListModel>());
	return _mem_ ? new (_mem_)VirtualQStringListModel(vtbl) : nullptr;
}

VirtualQStringListModel* QStringListModel_new2(const QStringListModel_VTable* vtbl, size_t vdata, struct miqt_array /* of struct miqt_string */  strings) {
...
VirtualQStringListModel* QStringListModel_new4(const QStringListModel_VTable* vtbl, size_t vdata, struct miqt_array /* of struct miqt_string */  strings, QObject* parent) {
	QStringList strings_QList;
	strings_QList.reserve(strings.len);
	struct miqt_string* strings_arr = static_cast<struct miqt_string*>(strings.data);
	for(size_t i = 0; i < strings.len; ++i) {
