...
proc create*(T: type gen_qtextboundaryfinder_types.QTextBoundaryFinder,
  ...
  let tmp = gen_qtextboundaryfinder_types.QTextBoundaryFinder(h: fcQTextBoundaryFinder_new3(cint(typeVal), struct_miqt_string(data: if len(stringVal) > 0: addr stringVal[0] else: nil, len: csize_t(len(stringVal)))), owned: true)
  tmp
proc create*(T: type gen_qtextboundaryfinder_types.QTextBoundaryFinder,
