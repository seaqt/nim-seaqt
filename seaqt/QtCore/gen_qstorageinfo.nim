...
proc fcQStorageInfo_device(self: pointer): struct_seaqt_string {.importc: "QStorageInfo_device".}
proc fcQStorageInfo_subvolume(self: pointer): struct_seaqt_string {.importc: "QStorageInfo_subvolume".}
proc fcQStorageInfo_fileSystemType(self: pointer): struct_seaqt_string {.importc: "QStorageInfo_fileSystemType".}
proc fcQStorageInfo_name(self: pointer): struct_seaqt_string {.importc: "QStorageInfo_name".}
proc fcQStorageInfo_displayName(self: pointer): struct_seaqt_string {.importc: "QStorageInfo_displayName".}
...
proc setPath*(self: gen_qstorageinfo_types.QStorageInfo, path: openArray[char]): void =
  fcQStorageInfo_setPath(self.h, struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
