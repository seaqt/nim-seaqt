...
proc fcQProcessEnvironment_new2(fromVal: pointer): ptr cQProcessEnvironment {.importc: "QProcessEnvironment_new_from".}
...
proc fcQProcess_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QProcess_trUtf8_s".}
...
proc fcQProcess_startCommand(self: pointer, command: struct_seaqt_string): void {.importc: "QProcess_start_command".}
...
proc fcQProcess_executeCommand(command: struct_seaqt_string): cint {.importc: "QProcess_execute_command".}
...
proc fcQProcess_startDetachedCommand(command: struct_seaqt_string): bool {.importc: "QProcess_startDetached_command".}
...
proc fcQProcess_errorError(self: pointer, error: cint): void {.importc: "QProcess_error_error".}
proc fcQProcess_connect_errorError(self: pointer, slot: int, callback: proc (slot: int, error: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QProcess_connect_error_error".}
...
proc fcQProcess_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QProcess_trUtf8_s_c".}
proc fcQProcess_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QProcess_trUtf8_s_c_n".}
...
proc fcQProcess_startCommandMode(self: pointer, command: struct_seaqt_string, mode: cint): void {.importc: "QProcess_start_command_mode".}
...
proc create*(T: type gen_qprocess_types.QProcessEnvironment,
  ...
  let tmp = gen_qprocess_types.QProcessEnvironment(h: fcQProcessEnvironment_new2(fromVal.h), owned: true)
  tmp
...
proc execute*(_: type gen_qprocess_types.QProcess, command: openArray[char]): cint =
  fcQProcess_executeCommand(struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))))
...
proc startDetached*(_: type gen_qprocess_types.QProcess, command: openArray[char]): bool =
  fcQProcess_startDetachedCommand(struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))))
...
proc error*(self: gen_qprocess_types.QProcess, error: cint): void =
  fcQProcess_errorError(self.h, cint(error))

type QProcesserrorErrorSlot* = proc(error: cint)
proc fcQProcess_slot_callback_errorError(slot: int, error: cint) {.cdecl.} =
  let nimfunc = cast[ptr QProcesserrorErrorSlot](cast[pointer](slot))
  let slotval1 = cint(error)

  nimfunc[](slotval1)

proc fcQProcess_slot_callback_errorError_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QProcesserrorErrorSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onError*(self: gen_qprocess_types.QProcess, slot: QProcesserrorErrorSlot) =
  var tmp = new QProcesserrorErrorSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQProcess_connect_errorError(self.h, cast[int](addr tmp[]), fcQProcess_slot_callback_errorError, fcQProcess_slot_callback_errorError_release)
...
proc start*(self: gen_qprocess_types.QProcess, command: openArray[char], mode: cint): void =
  fcQProcess_startCommandMode(self.h, struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))), cint(mode))
