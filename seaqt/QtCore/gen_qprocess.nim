...
proc fcQProcess_trUtf8(s: cstring): struct_seaqt_string {.importc: "QProcess_trUtf8".}
proc fcQProcess_start(self: pointer, program: struct_seaqt_string, arguments: struct_seaqt_array): void {.importc: "QProcess_start".}
proc fcQProcess_startWithCommand(self: pointer, command: struct_seaqt_string): void {.importc: "QProcess_startWithCommand".}
...
proc fcQProcess_setProgram(self: pointer, program: struct_seaqt_string): void {.importc: "QProcess_setProgram".}
proc fcQProcess_arguments(self: pointer): struct_seaqt_array {.importc: "QProcess_arguments".}
proc fcQProcess_setArguments(self: pointer, arguments: struct_seaqt_array): void {.importc: "QProcess_setArguments".}
...
proc fcQProcess_setEnvironment(self: pointer, environment: struct_seaqt_array): void {.importc: "QProcess_setEnvironment".}
proc fcQProcess_environment(self: pointer): struct_seaqt_array {.importc: "QProcess_environment".}
...
proc fcQProcess_readAllStandardOutput(self: pointer): struct_seaqt_string {.importc: "QProcess_readAllStandardOutput".}
proc fcQProcess_readAllStandardError(self: pointer): struct_seaqt_string {.importc: "QProcess_readAllStandardError".}
...
proc fcQProcess_execute(program: struct_seaqt_string, arguments: struct_seaqt_array): cint {.importc: "QProcess_execute".}
proc fcQProcess_executeWithCommand(command: struct_seaqt_string): cint {.importc: "QProcess_executeWithCommand".}
proc fcQProcess_startDetached2(program: struct_seaqt_string, arguments: struct_seaqt_array, workingDirectory: struct_seaqt_string): bool {.importc: "QProcess_startDetached2".}
proc fcQProcess_startDetached3(program: struct_seaqt_string, arguments: struct_seaqt_array): bool {.importc: "QProcess_startDetached3".}
proc fcQProcess_startDetachedWithCommand(command: struct_seaqt_string): bool {.importc: "QProcess_startDetachedWithCommand".}
...
proc fcQProcess_trUtf82(s: cstring, c: cstring): struct_seaqt_string {.importc: "QProcess_trUtf82".}
proc fcQProcess_trUtf83(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QProcess_trUtf83".}
proc fcQProcess_start3(self: pointer, program: struct_seaqt_string, arguments: struct_seaqt_array, mode: cint): void {.importc: "QProcess_start3".}
proc fcQProcess_start4(self: pointer, command: struct_seaqt_string, mode: cint): void {.importc: "QProcess_start4".}
...
proc fcQProcess_protectedbase_setErrorString(self: pointer, errorString: struct_seaqt_string): void {.importc: "QProcess_protectedbase_setErrorString".}
...
proc create*(T: type gen_qprocess_types.QProcessEnvironment,
  ...
  let tmp = gen_qprocess_types.QProcessEnvironment(h: fcQProcessEnvironment_new2(other.h), owned: true)
  tmp
...
proc start*(self: gen_qprocess_types.QProcess, program: openArray[char], arguments: openArray[string]): void =
  ...
  fcQProcess_start(self.h, struct_seaqt_string(data: if len(program) > 0: addr program[0] else: nil, len: csize_t(len(program))), struct_seaqt_array(len: csize_t(len(arguments)), data: if len(arguments) == 0: nil else: addr(arguments_CArray[0])))

proc start*(self: gen_qprocess_types.QProcess, command: openArray[char]): void =
  fcQProcess_startWithCommand(self.h, struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))))
...
proc setProgram*(self: gen_qprocess_types.QProcess, program: openArray[char]): void =
  fcQProcess_setProgram(self.h, struct_seaqt_string(data: if len(program) > 0: addr program[0] else: nil, len: csize_t(len(program))))
...
proc setArguments*(self: gen_qprocess_types.QProcess, arguments: openArray[string]): void =
  ...
  fcQProcess_setArguments(self.h, struct_seaqt_array(len: csize_t(len(arguments)), data: if len(arguments) == 0: nil else: addr(arguments_CArray[0])))
...
proc setWorkingDirectory*(self: gen_qprocess_types.QProcess, dir: openArray[char]): void =
  fcQProcess_setWorkingDirectory(self.h, struct_seaqt_string(data: if len(dir) > 0: addr dir[0] else: nil, len: csize_t(len(dir))))

proc setEnvironment*(self: gen_qprocess_types.QProcess, environment: openArray[string]): void =
  var environment_CArray = newSeq[struct_seaqt_string](len(environment))
  for i in 0..<len(environment):
    environment_CArray[i] = struct_seaqt_string(data: if len(environment[i]) > 0: addr environment[i][0] else: nil, len: csize_t(len(environment[i])))

  fcQProcess_setEnvironment(self.h, struct_seaqt_array(len: csize_t(len(environment)), data: if len(environment) == 0: nil else: addr(environment_CArray[0])))
...
proc execute*(_: type gen_qprocess_types.QProcess, program: openArray[char], arguments: openArray[string]): cint =
  ...
  fcQProcess_execute(struct_seaqt_string(data: if len(program) > 0: addr program[0] else: nil, len: csize_t(len(program))), struct_seaqt_array(len: csize_t(len(arguments)), data: if len(arguments) == 0: nil else: addr(arguments_CArray[0])))

proc execute*(_: type gen_qprocess_types.QProcess, command: openArray[char]): cint =
  fcQProcess_executeWithCommand(struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))))
...
proc startDetached*(_: type gen_qprocess_types.QProcess, program: openArray[char], arguments: openArray[string]): bool =
  ...
  fcQProcess_startDetached3(struct_seaqt_string(data: if len(program) > 0: addr program[0] else: nil, len: csize_t(len(program))), struct_seaqt_array(len: csize_t(len(arguments)), data: if len(arguments) == 0: nil else: addr(arguments_CArray[0])))

proc startDetached*(_: type gen_qprocess_types.QProcess, command: openArray[char]): bool =
  fcQProcess_startDetachedWithCommand(struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))))
...
proc fcQProcess_slot_callback_errorWithError(slot: int, error: cint) {.cdecl.} =
  let nimfunc = cast[ptr QProcesserrorWithErrorSlot](cast[pointer](slot))
  let slotval1 = cint(error)

  nimfunc[](slotval1)

proc fcQProcess_slot_callback_errorWithError_release(slot: int) {.cdecl.} =
...
proc onError*(self: gen_qprocess_types.QProcess, slot: QProcesserrorWithErrorSlot) =
  ...
  tmp[] = slot
  GC_ref(tmp)
  fcQProcess_connect_errorWithError(self.h, cast[int](addr tmp[]), fcQProcess_slot_callback_errorWithError, fcQProcess_slot_callback_errorWithError_release)
...
proc start*(self: gen_qprocess_types.QProcess, program: openArray[char], arguments: openArray[string], mode: cint): void =
  ...
  fcQProcess_start3(self.h, struct_seaqt_string(data: if len(program) > 0: addr program[0] else: nil, len: csize_t(len(program))), struct_seaqt_array(len: csize_t(len(arguments)), data: if len(arguments) == 0: nil else: addr(arguments_CArray[0])), cint(mode))

proc start*(self: gen_qprocess_types.QProcess, command: openArray[char], mode: cint): void =
  fcQProcess_start4(self.h, struct_seaqt_string(data: if len(command) > 0: addr command[0] else: nil, len: csize_t(len(command))), cint(mode))
...
proc fcQProcess_vtable_callback_setupChildProcess(self: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QProcessVTable](fcQProcess_vdata(self)[])
  let self = QProcess(h: self)
...
proc fcQProcess_method_callback_setupChildProcess(self: pointer): void {.cdecl.} =
  let inst = cast[VirtualQProcess](fcQProcess_vdata(self)[])
...
proc setErrorString*(self: gen_qprocess_types.QProcess, errorString: openArray[char]): void =
  fcQProcess_protectedbase_setErrorString(self.h, struct_seaqt_string(data: if len(errorString) > 0: addr errorString[0] else: nil, len: csize_t(len(errorString))))
...
proc create*(T: type gen_qprocess_types.QProcess,
  ...
  if not isNil(vtbl[].setupChildProcess):
    vtbl[].vtbl.setupChildProcess = fcQProcess_vtable_callback_setupChildProcess
...
const cQProcess_mvtbl = cQProcessVTable(
  ...
  setupChildProcess: fcQProcess_method_callback_setupChildProcess,
...
proc create*(T: type gen_qprocess_types.QProcess,
  ...
  inst[].h = fcQProcess_new(addr(cQProcess_mvtbl), csize_t(sizeof(pointer)))
  fcQProcess_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qprocess_types.QProcess,
    ...
    inst: VirtualQProcess) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQProcess_new2(addr(cQProcess_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQProcess_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
