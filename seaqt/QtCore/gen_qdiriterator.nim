...
proc create*(T: type gen_qdiriterator_types.QDirIterator,
  ...
  let tmp = gen_qdiriterator_types.QDirIterator(h: fcQDirIterator_new4(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))), struct_miqt_array(len: csize_t(len(nameFilters)), data: if len(nameFilters) == 0: nil else: addr(nameFilters_CArray[0]))), owned: true)
  tmp
proc create*(T: type gen_qdiriterator_types.QDirIterator,
...
proc create*(T: type gen_qdiriterator_types.QDirIterator,
  ...
  let tmp = gen_qdiriterator_types.QDirIterator(h: fcQDirIterator_new9(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))), struct_miqt_array(len: csize_t(len(nameFilters)), data: if len(nameFilters) == 0: nil else: addr(nameFilters_CArray[0])), cint(filters), cint(flags)), owned: true)
  tmp
