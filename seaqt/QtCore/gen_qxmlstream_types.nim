...
proc `=destroy`(self: var QXmlStreamStringRef) =
  if self.owned: fcQXmlStreamStringRef_delete(self.h)

proc `=sink`(dest: var QXmlStreamStringRef, source: QXmlStreamStringRef) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QXmlStreamStringRef, source: QXmlStreamStringRef) {.error.}
proc delete*(self: sink QXmlStreamStringRef) =
  let h = self.h
  wasMoved(self)
