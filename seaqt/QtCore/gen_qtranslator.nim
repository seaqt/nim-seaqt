...
proc fcQTranslator_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QTranslator_trUtf8_s".}
...
proc fcQTranslator_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QTranslator_trUtf8_s_c".}
proc fcQTranslator_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QTranslator_trUtf8_s_c_n".}
...
type cQTranslatorVTable {.pure.} = object
  ...
  translate*: proc(self: pointer, context: cstring, sourceText: cstring, disambiguation: cstring, n: cint): struct_seaqt_string {.cdecl, raises: [], gcsafe.}
...
proc fcQTranslator_virtualbase_translate(self: pointer, context: cstring, sourceText: cstring, disambiguation: cstring, n: cint): struct_seaqt_string {.importc: "QTranslator_virtualbase_translate".}
...
proc trUtf8*(_: type gen_qtranslator_types.QTranslator, s: cstring): string =
  let v_ms = fcQTranslator_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc fcQTranslator_vtable_callback_translate(self: pointer, context: cstring, sourceText: cstring, disambiguation: cstring, n: cint): struct_seaqt_string {.cdecl.} =
  let vtbl = cast[ptr QTranslatorVTable](fcQTranslator_vdata(self)[])
  let self = QTranslator(h: self)
...
proc fcQTranslator_method_callback_translate(self: pointer, context: cstring, sourceText: cstring, disambiguation: cstring, n: cint): struct_seaqt_string {.cdecl.} =
  let inst = cast[VirtualQTranslator](fcQTranslator_vdata(self)[])
  let slotval1 = (context)
  let slotval2 = (sourceText)
  let slotval3 = (disambiguation)
...
proc create*(T: type gen_qtranslator_types.QTranslator,
  ...
  let tmp = gen_qtranslator_types.QTranslator(h: fcQTranslator_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQTranslator_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtranslator_types.QTranslator,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTranslator_vtable_callback_disconnectNotify
  let tmp = gen_qtranslator_types.QTranslator(h: fcQTranslator_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQTranslator_vdata(tmp.h)[] = addr(vtbl[])
  tmp
