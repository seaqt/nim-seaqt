...
type QOperatingSystemVersionUnexported* = object of QOperatingSystemVersionBase
proc `=copy`(dest: var QOperatingSystemVersionUnexported, source: QOperatingSystemVersionUnexported) {.error.}
proc `=sink`(dest: var QOperatingSystemVersionUnexported, source: QOperatingSystemVersionUnexported) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

type QOperatingSystemVersion* = object of QOperatingSystemVersionUnexported
