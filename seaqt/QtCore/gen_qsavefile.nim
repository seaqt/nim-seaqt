...
proc cQSaveFile_vtable_callback_skipData(self: pointer, maxSize: clonglong): clonglong {.cdecl.} =
  let vtbl = cast[ptr QSaveFileVTable](fcQSaveFile_vdata(self))
  let self = QSaveFile(h: self)
...
proc cQSaveFile_method_callback_skipData(self: pointer, maxSize: clonglong): clonglong {.cdecl.} =
  let inst = cast[VirtualQSaveFile](fcQSaveFile_vdata(self))
  let slotval1 = maxSize
...
proc create*(T: type gen_qsavefile_types.QSaveFile,
  ...
  if not isNil(vtbl[].skipData):
    vtbl[].vtbl.skipData = cQSaveFile_vtable_callback_skipData
...
const cQSaveFile_mvtbl = cQSaveFileVTable(
  ...
  skipData: cQSaveFile_method_callback_skipData,
