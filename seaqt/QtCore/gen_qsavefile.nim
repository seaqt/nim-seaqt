...
proc fcQSaveFile_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QSaveFile_trUtf8_s".}
...
proc fcQSaveFile_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QSaveFile_trUtf8_s_c".}
proc fcQSaveFile_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QSaveFile_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qsavefile_types.QSaveFile, s: cstring): string =
  let v_ms = fcQSaveFile_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qsavefile_types.QSaveFile,
  ...
  let tmp = gen_qsavefile_types.QSaveFile(h: fcQSaveFile_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))), owned: true)
...
proc create*(T: type gen_qsavefile_types.QSaveFile,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSaveFile_vtable_callback_disconnectNotify
  let tmp = gen_qsavefile_types.QSaveFile(h: fcQSaveFile_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), parent.h), owned: true)
  fcQSaveFile_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsavefile_types.QSaveFile,
...
proc create*(T: type gen_qsavefile_types.QSaveFile,
  ...
  inst[].h = fcQSaveFile_new(addr(cQSaveFile_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc create*(T: type gen_qsavefile_types.QSaveFile,
    ...
    inst: VirtualQSaveFile) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSaveFile_new3(addr(cQSaveFile_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), parent.h)
  fcQSaveFile_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
