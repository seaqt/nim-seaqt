...
proc QTransposeProxyModelmimeData*(self: gen_qtransposeproxymodel_types.QTransposeProxyModel, indexes: openArray[gen_qabstractitemmodel_types.QModelIndex]): gen_qmimedata_types.QMimeData =
  ...
  gen_qmimedata_types.QMimeData(h: fcQTransposeProxyModel_virtualbase_mimeData(self.h, struct_seaqt_array(len: csize_t(len(indexes)), data: if len(indexes) == 0: nil else: addr(indexes_CArray[0]))))
...
proc create*(T: type gen_qtransposeproxymodel_types.QTransposeProxyModel,
  ...
  let tmp = gen_qtransposeproxymodel_types.QTransposeProxyModel(h: fcQTransposeProxyModel_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qtransposeproxymodel_types.QTransposeProxyModel,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTransposeProxyModel_vtable_callback_disconnectNotify
  let tmp = gen_qtransposeproxymodel_types.QTransposeProxyModel(h: fcQTransposeProxyModel_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQTransposeProxyModel_vdata(tmp.h)[] = addr(vtbl[])
  tmp
