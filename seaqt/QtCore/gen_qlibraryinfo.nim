...
proc fcQLibraryInfo_isSharedBuild(): bool {.importc: "QLibraryInfo_isSharedBuild".}
...
proc fcQLibraryInfo_paths(p: cint): struct_seaqt_array {.importc: "QLibraryInfo_paths".}
...
proc isSharedBuild*(_: type gen_qlibraryinfo_types.QLibraryInfo): bool =
  fcQLibraryInfo_isSharedBuild()
...
proc paths*(_: type gen_qlibraryinfo_types.QLibraryInfo, p: cint): seq[string] =
  var v_ma = fcQLibraryInfo_paths(cint(p))
...
proc platformPluginArguments*(_: type gen_qlibraryinfo_types.QLibraryInfo, platformName: openArray[char]): seq[string] =
  var v_ma = fcQLibraryInfo_platformPluginArguments(struct_seaqt_string(data: if len(platformName) > 0: addr platformName[0] else: nil, len: csize_t(len(platformName))))
  var vx_ret = newSeq[string](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    let vx_lv_ms = v_outCast[i]
