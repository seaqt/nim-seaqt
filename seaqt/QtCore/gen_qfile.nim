...
proc readLink*(_: type gen_qfile_types.QFile, fileName: openArray[char]): string =
  let v_ms = fcQFile_readLinkWithFileName(struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc link*(_: type gen_qfile_types.QFile, oldname: openArray[char], newName: openArray[char]): bool =
  fcQFile_link2(struct_miqt_string(data: if len(oldname) > 0: addr oldname[0] else: nil, len: csize_t(len(oldname))), struct_miqt_string(data: if len(newName) > 0: addr newName[0] else: nil, len: csize_t(len(newName))))
