...
proc create*(T: type gen_qdir_types.QDir,
  ...
  let tmp = gen_qdir_types.QDir(h: fcQDir_new3(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))), struct_miqt_string(data: if len(nameFilter) > 0: addr nameFilter[0] else: nil, len: csize_t(len(nameFilter)))), owned: true)
  tmp
proc create*(T: type gen_qdir_types.QDir,
    path: openArray[char]): gen_qdir_types.QDir =
  let tmp = gen_qdir_types.QDir(h: fcQDir_new4(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path)))), owned: true)
  tmp
proc create*(T: type gen_qdir_types.QDir,
    path: openArray[char], nameFilter: openArray[char], sort: cint): gen_qdir_types.QDir =
  let tmp = gen_qdir_types.QDir(h: fcQDir_new5(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))), struct_miqt_string(data: if len(nameFilter) > 0: addr nameFilter[0] else: nil, len: csize_t(len(nameFilter))), cint(sort)), owned: true)
  tmp
proc create*(T: type gen_qdir_types.QDir,
    path: openArray[char], nameFilter: openArray[char], sort: cint, filter: cint): gen_qdir_types.QDir =
  let tmp = gen_qdir_types.QDir(h: fcQDir_new6(struct_miqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))), struct_miqt_string(data: if len(nameFilter) > 0: addr nameFilter[0] else: nil, len: csize_t(len(nameFilter))), cint(sort), cint(filter)), owned: true)
  tmp
