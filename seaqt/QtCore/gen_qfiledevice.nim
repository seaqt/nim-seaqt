...
{.compile("gen_qfiledevice.cpp", QtCoreCFlags).}
...
proc fcQFileDevice_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QFileDevice_trUtf8_s".}
...
proc fcQFileDevice_fileName(self: pointer): struct_seaqt_string {.importc: "QFileDevice_fileName".}
...
proc fcQFileDevice_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QFileDevice_trUtf8_s_c".}
proc fcQFileDevice_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QFileDevice_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qfiledevice_types.QFileDevice, s: cstring): string =
  let v_ms = fcQFileDevice_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setErrorString*(self: gen_qfiledevice_types.QFileDevice, errorString: openArray[char]): void =
  fcQFileDevice_protectedbase_setErrorString(self.h, struct_seaqt_string(data: if len(errorString) > 0: addr errorString[0] else: nil, len: csize_t(len(errorString))))
