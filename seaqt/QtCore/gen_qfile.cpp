...
VirtualQFile* QFile_new(const QFile_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQFile>());
	return _mem_ ? new (_mem_)VirtualQFile(vtbl) : nullptr;
}

VirtualQFile* QFile_new2(const QFile_VTable* vtbl, size_t vdata, struct miqt_string name) {
...
VirtualQFile* QFile_new4(const QFile_VTable* vtbl, size_t vdata, struct miqt_string name, QObject* parent) {
	QString name_QString = QString::fromUtf8(name.data, name.len);
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQFile>());
