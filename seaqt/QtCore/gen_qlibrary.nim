...
proc fcQLibrary_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QLibrary_trUtf8_s".}
...
proc fcQLibrary_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QLibrary_trUtf8_s_c".}
proc fcQLibrary_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QLibrary_trUtf8_s_c_n".}
...
proc isLibrary*(_: type gen_qlibrary_types.QLibrary, fileName: openArray[char]): bool =
  fcQLibrary_isLibrary(struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))

proc setFileName*(self: gen_qlibrary_types.QLibrary, fileName: openArray[char]): void =
  fcQLibrary_setFileName(self.h, struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
...
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName)))), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), verNum), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_seaqt_string(data: if len(version) > 0: addr version[0] else: nil, len: csize_t(len(version)))), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), parent.h), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), verNum, parent.h), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlibrary_types.QLibrary,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLibrary_vtable_callback_disconnectNotify
  let tmp = gen_qlibrary_types.QLibrary(h: fcQLibrary_new8(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_seaqt_string(data: if len(version) > 0: addr version[0] else: nil, len: csize_t(len(version))), parent.h), owned: true)
  fcQLibrary_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new2(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))))
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new3(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), verNum)
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new4(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_seaqt_string(data: if len(version) > 0: addr version[0] else: nil, len: csize_t(len(version))))
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new6(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), parent.h)
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new7(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), verNum, parent.h)
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlibrary_types.QLibrary,
    ...
    inst: VirtualQLibrary) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLibrary_new8(addr(cQLibrary_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), struct_seaqt_string(data: if len(version) > 0: addr version[0] else: nil, len: csize_t(len(version))), parent.h)
  fcQLibrary_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
