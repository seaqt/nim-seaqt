...
proc fcQWebEngineUrlRequestJob_requestBody(self: pointer): pointer {.importc: "QWebEngineUrlRequestJob_requestBody".}
...
proc requestBody*(self: gen_qwebengineurlrequestjob_types.QWebEngineUrlRequestJob): gen_qiodevice_types.QIODevice =
  gen_qiodevice_types.QIODevice(h: fcQWebEngineUrlRequestJob_requestBody(self.h), owned: false)
...
proc reply*(self: gen_qwebengineurlrequestjob_types.QWebEngineUrlRequestJob, contentType: openArray[byte], device: gen_qiodevice_types.QIODevice): void =
  fcQWebEngineUrlRequestJob_reply(self.h, struct_seaqt_string(data: if len(contentType) > 0: addr contentType[0] else: nil, len: csize_t(len(contentType))), device.h)
