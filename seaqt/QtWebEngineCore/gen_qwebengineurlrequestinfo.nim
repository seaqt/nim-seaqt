...
{.compile("gen_qwebengineurlrequestinfo.cpp", QtWebEngineCoreCFlags).}
...
template ResourceTypeJson*(_: type QWebEngineUrlRequestInfoResourceTypeEnum): untyped = 21
template ResourceTypeLast*(_: type QWebEngineUrlRequestInfoResourceTypeEnum): untyped = 21
...
proc fcQWebEngineUrlRequestInfo_requestMethod(self: pointer): struct_seaqt_string {.importc: "QWebEngineUrlRequestInfo_requestMethod".}
proc fcQWebEngineUrlRequestInfo_requestBody(self: pointer): pointer {.importc: "QWebEngineUrlRequestInfo_requestBody".}
proc fcQWebEngineUrlRequestInfo_changed(self: pointer): bool {.importc: "QWebEngineUrlRequestInfo_changed".}
proc fcQWebEngineUrlRequestInfo_isDownload(self: pointer): bool {.importc: "QWebEngineUrlRequestInfo_isDownload".}
...
proc fcQWebEngineUrlRequestInfo_setHttpHeader(self: pointer, name: struct_seaqt_string, value: struct_seaqt_string): void {.importc: "QWebEngineUrlRequestInfo_setHttpHeader".}
...
proc requestBody*(self: gen_qwebengineurlrequestinfo_types.QWebEngineUrlRequestInfo): gen_qiodevice_types.QIODevice =
  gen_qiodevice_types.QIODevice(h: fcQWebEngineUrlRequestInfo_requestBody(self.h), owned: false)
...
proc isDownload*(self: gen_qwebengineurlrequestinfo_types.QWebEngineUrlRequestInfo): bool =
  fcQWebEngineUrlRequestInfo_isDownload(self.h)
...
proc setHttpHeader*(self: gen_qwebengineurlrequestinfo_types.QWebEngineUrlRequestInfo, name: openArray[byte], value: openArray[byte]): void =
  fcQWebEngineUrlRequestInfo_setHttpHeader(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), struct_seaqt_string(data: if len(value) > 0: addr value[0] else: nil, len: csize_t(len(value))))
