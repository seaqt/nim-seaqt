...
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
  ...
  let tmp = gen_qnetworkproxy_types.QNetworkProxyQuery(h: fcQNetworkProxyQuery_new3(struct_miqt_string(data: if len(hostname) > 0: addr hostname[0] else: nil, len: csize_t(len(hostname))), port), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
...
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
  ...
  let tmp = gen_qnetworkproxy_types.QNetworkProxyQuery(h: fcQNetworkProxyQuery_new7(struct_miqt_string(data: if len(hostname) > 0: addr hostname[0] else: nil, len: csize_t(len(hostname))), port, struct_miqt_string(data: if len(protocolTag) > 0: addr protocolTag[0] else: nil, len: csize_t(len(protocolTag)))), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
    hostname: openArray[char], port: cint, protocolTag: openArray[char], queryType: cint): gen_qnetworkproxy_types.QNetworkProxyQuery =
  let tmp = gen_qnetworkproxy_types.QNetworkProxyQuery(h: fcQNetworkProxyQuery_new8(struct_miqt_string(data: if len(hostname) > 0: addr hostname[0] else: nil, len: csize_t(len(hostname))), port, struct_miqt_string(data: if len(protocolTag) > 0: addr protocolTag[0] else: nil, len: csize_t(len(protocolTag))), cint(queryType)), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
    bindPort: cushort, protocolTag: openArray[char]): gen_qnetworkproxy_types.QNetworkProxyQuery =
  let tmp = gen_qnetworkproxy_types.QNetworkProxyQuery(h: fcQNetworkProxyQuery_new9(bindPort, struct_miqt_string(data: if len(protocolTag) > 0: addr protocolTag[0] else: nil, len: csize_t(len(protocolTag)))), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxyQuery,
    bindPort: cushort, protocolTag: openArray[char], queryType: cint): gen_qnetworkproxy_types.QNetworkProxyQuery =
  let tmp = gen_qnetworkproxy_types.QNetworkProxyQuery(h: fcQNetworkProxyQuery_new10(bindPort, struct_miqt_string(data: if len(protocolTag) > 0: addr protocolTag[0] else: nil, len: csize_t(len(protocolTag))), cint(queryType)), owned: true)
  tmp
...
proc hasRawHeader*(self: gen_qnetworkproxy_types.QNetworkProxy, headerName: openArray[byte]): bool =
  fcQNetworkProxy_hasRawHeader(self.h, struct_miqt_string(data: if len(headerName) > 0: addr headerName[0] else: nil, len: csize_t(len(headerName))))
...
proc rawHeader*(self: gen_qnetworkproxy_types.QNetworkProxy, headerName: openArray[byte]): seq[byte] =
  var v_bytearray = fcQNetworkProxy_rawHeader(self.h, struct_miqt_string(data: if len(headerName) > 0: addr headerName[0] else: nil, len: csize_t(len(headerName))))
...
proc setRawHeader*(self: gen_qnetworkproxy_types.QNetworkProxy, headerName: openArray[byte], value: openArray[byte]): void =
  fcQNetworkProxy_setRawHeader(self.h, struct_miqt_string(data: if len(headerName) > 0: addr headerName[0] else: nil, len: csize_t(len(headerName))), struct_miqt_string(data: if len(value) > 0: addr value[0] else: nil, len: csize_t(len(value))))
...
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxy,
  ...
  let tmp = gen_qnetworkproxy_types.QNetworkProxy(h: fcQNetworkProxy_new4(cint(typeVal), struct_miqt_string(data: if len(hostName) > 0: addr hostName[0] else: nil, len: csize_t(len(hostName)))), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxy,
    typeVal: cint, hostName: openArray[char], port: cushort): gen_qnetworkproxy_types.QNetworkProxy =
  let tmp = gen_qnetworkproxy_types.QNetworkProxy(h: fcQNetworkProxy_new5(cint(typeVal), struct_miqt_string(data: if len(hostName) > 0: addr hostName[0] else: nil, len: csize_t(len(hostName))), port), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxy,
    typeVal: cint, hostName: openArray[char], port: cushort, user: openArray[char]): gen_qnetworkproxy_types.QNetworkProxy =
  let tmp = gen_qnetworkproxy_types.QNetworkProxy(h: fcQNetworkProxy_new6(cint(typeVal), struct_miqt_string(data: if len(hostName) > 0: addr hostName[0] else: nil, len: csize_t(len(hostName))), port, struct_miqt_string(data: if len(user) > 0: addr user[0] else: nil, len: csize_t(len(user)))), owned: true)
  tmp
proc create*(T: type gen_qnetworkproxy_types.QNetworkProxy,
