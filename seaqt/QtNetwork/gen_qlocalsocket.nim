...
proc fcQLocalSocket_fullServerName(self: pointer): struct_seaqt_string {.importc: "QLocalSocket_fullServerName".}
...
proc fcQLocalSocket_protectedbase_setErrorString(self: pointer, errorString: struct_seaqt_string): void {.importc: "QLocalSocket_protectedbase_setErrorString".}
...
proc setServerName*(self: gen_qlocalsocket_types.QLocalSocket, name: openArray[char]): void =
  fcQLocalSocket_setServerName(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc setErrorString*(self: gen_qlocalsocket_types.QLocalSocket, errorString: openArray[char]): void =
  fcQLocalSocket_protectedbase_setErrorString(self.h, struct_seaqt_string(data: if len(errorString) > 0: addr errorString[0] else: nil, len: csize_t(len(errorString))))
...
proc create*(T: type gen_qlocalsocket_types.QLocalSocket,
  ...
  if not isNil(vtbl[].skipData):
    vtbl[].vtbl.skipData = fcQLocalSocket_vtable_callback_skipData
  ...
  let tmp = gen_qlocalsocket_types.QLocalSocket(h: fcQLocalSocket_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQLocalSocket_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlocalsocket_types.QLocalSocket,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLocalSocket_vtable_callback_disconnectNotify
  let tmp = gen_qlocalsocket_types.QLocalSocket(h: fcQLocalSocket_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQLocalSocket_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQLocalSocket_mvtbl = cQLocalSocketVTable(
  ...
  skipData: fcQLocalSocket_method_callback_skipData,
...
proc create*(T: type gen_qlocalsocket_types.QLocalSocket,
  ...
  inst[].h = fcQLocalSocket_new(addr(cQLocalSocket_mvtbl), csize_t(sizeof(pointer)))
  fcQLocalSocket_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qlocalsocket_types.QLocalSocket,
    ...
    inst: VirtualQLocalSocket) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQLocalSocket_new2(addr(cQLocalSocket_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQLocalSocket_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
