...
proc fcQNetworkCookie_domain(self: pointer): struct_seaqt_string {.importc: "QNetworkCookie_domain".}
proc fcQNetworkCookie_setDomain(self: pointer, domain: struct_seaqt_string): void {.importc: "QNetworkCookie_setDomain".}
proc fcQNetworkCookie_path(self: pointer): struct_seaqt_string {.importc: "QNetworkCookie_path".}
proc fcQNetworkCookie_setPath(self: pointer, path: struct_seaqt_string): void {.importc: "QNetworkCookie_setPath".}
proc fcQNetworkCookie_name(self: pointer): struct_seaqt_string {.importc: "QNetworkCookie_name".}
proc fcQNetworkCookie_setName(self: pointer, cookieName: struct_seaqt_string): void {.importc: "QNetworkCookie_setName".}
proc fcQNetworkCookie_value(self: pointer): struct_seaqt_string {.importc: "QNetworkCookie_value".}
proc fcQNetworkCookie_setValue(self: pointer, value: struct_seaqt_string): void {.importc: "QNetworkCookie_setValue".}
proc fcQNetworkCookie_toRawForm(self: pointer): struct_seaqt_string {.importc: "QNetworkCookie_toRawForm".}
...
proc setDomain*(self: gen_qnetworkcookie_types.QNetworkCookie, domain: openArray[char]): void =
  fcQNetworkCookie_setDomain(self.h, struct_seaqt_string(data: if len(domain) > 0: addr domain[0] else: nil, len: csize_t(len(domain))))
...
proc setPath*(self: gen_qnetworkcookie_types.QNetworkCookie, path: openArray[char]): void =
  fcQNetworkCookie_setPath(self.h, struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
...
proc setName*(self: gen_qnetworkcookie_types.QNetworkCookie, cookieName: openArray[byte]): void =
  fcQNetworkCookie_setName(self.h, struct_seaqt_string(data: if len(cookieName) > 0: addr cookieName[0] else: nil, len: csize_t(len(cookieName))))
...
proc setValue*(self: gen_qnetworkcookie_types.QNetworkCookie, value: openArray[byte]): void =
  fcQNetworkCookie_setValue(self.h, struct_seaqt_string(data: if len(value) > 0: addr value[0] else: nil, len: csize_t(len(value))))
...
proc parseCookies*(_: type gen_qnetworkcookie_types.QNetworkCookie, cookieString: openArray[byte]): seq[gen_qnetworkcookie_types.QNetworkCookie] =
  var v_ma = fcQNetworkCookie_parseCookies(struct_seaqt_string(data: if len(cookieString) > 0: addr cookieString[0] else: nil, len: csize_t(len(cookieString))))
...
proc create*(T: type gen_qnetworkcookie_types.QNetworkCookie): gen_qnetworkcookie_types.QNetworkCookie =
  let tmp = gen_qnetworkcookie_types.QNetworkCookie(h: fcQNetworkCookie_new())
  tmp
proc create*(T: type gen_qnetworkcookie_types.QNetworkCookie,
  ...
  let tmp = gen_qnetworkcookie_types.QNetworkCookie(h: fcQNetworkCookie_new2(other.h))
  tmp
proc create*(T: type gen_qnetworkcookie_types.QNetworkCookie,
    name: openArray[byte]): gen_qnetworkcookie_types.QNetworkCookie =
  let tmp = gen_qnetworkcookie_types.QNetworkCookie(h: fcQNetworkCookie_new3(struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))))
  tmp
proc create*(T: type gen_qnetworkcookie_types.QNetworkCookie,
    name: openArray[byte], value: openArray[byte]): gen_qnetworkcookie_types.QNetworkCookie =
  let tmp = gen_qnetworkcookie_types.QNetworkCookie(h: fcQNetworkCookie_new4(struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), struct_seaqt_string(data: if len(value) > 0: addr value[0] else: nil, len: csize_t(len(value)))))
  tmp
