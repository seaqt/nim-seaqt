...
proc fcQDtlsClientVerifier_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QDtlsClientVerifier_trUtf8_s".}
...
proc fcQDtlsClientVerifier_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QDtlsClientVerifier_trUtf8_s_c".}
proc fcQDtlsClientVerifier_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QDtlsClientVerifier_trUtf8_s_c_n".}
...
proc fcQDtls_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QDtls_trUtf8_s".}
...
proc fcQDtls_peerVerificationName(self: pointer): struct_seaqt_string {.importc: "QDtls_peerVerificationName".}
...
proc fcQDtls_writeDatagramEncrypted(self: pointer, socket: pointer, dgram: struct_seaqt_string): clonglong {.importc: "QDtls_writeDatagramEncrypted".}
proc fcQDtls_decryptDatagram(self: pointer, socket: pointer, dgram: struct_seaqt_string): struct_seaqt_string {.importc: "QDtls_decryptDatagram".}
proc fcQDtls_dtlsError(self: pointer): cint {.importc: "QDtls_dtlsError".}
proc fcQDtls_dtlsErrorString(self: pointer): struct_seaqt_string {.importc: "QDtls_dtlsErrorString".}
proc fcQDtls_peerVerificationErrors(self: pointer): struct_seaqt_array {.importc: "QDtls_peerVerificationErrors".}
proc fcQDtls_ignoreVerificationErrors(self: pointer, errorsToIgnore: struct_seaqt_array): void {.importc: "QDtls_ignoreVerificationErrors".}
...
proc fcQDtls_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QDtls_trUtf8_s_c".}
proc fcQDtls_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QDtls_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qdtls_types.QDtlsClientVerifier, s: cstring): string =
  let v_ms = fcQDtlsClientVerifier_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc verifyClient*(self: gen_qdtls_types.QDtlsClientVerifier, socket: gen_qudpsocket_types.QUdpSocket, dgram: openArray[byte], address: gen_qhostaddress_types.QHostAddress, port: cushort): bool =
  fcQDtlsClientVerifier_verifyClient(self.h, socket.h, struct_seaqt_string(data: if len(dgram) > 0: addr dgram[0] else: nil, len: csize_t(len(dgram))), address.h, port)
...
proc trUtf8*(_: type gen_qdtls_types.QDtlsClientVerifier, s: cstring, c: cstring): string =
  let v_ms = fcQDtlsClientVerifier_trUtf8SC(s, c)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret

proc trUtf8*(_: type gen_qdtls_types.QDtlsClientVerifier, s: cstring, c: cstring, n: cint): string =
  let v_ms = fcQDtlsClientVerifier_trUtf8SCN(s, c, n)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qdtls_types.QDtlsClientVerifier,
  ...
  inst[].h = fcQDtlsClientVerifier_new(addr(cQDtlsClientVerifier_mvtbl), csize_t(sizeof(pointer)))
  fcQDtlsClientVerifier_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qdtls_types.QDtlsClientVerifier,
    ...
    inst: VirtualQDtlsClientVerifier) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDtlsClientVerifier_new2(addr(cQDtlsClientVerifier_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQDtlsClientVerifier_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc writeDatagramEncrypted*(self: gen_qdtls_types.QDtls, socket: gen_qudpsocket_types.QUdpSocket, dgram: openArray[byte]): clonglong =
  fcQDtls_writeDatagramEncrypted(self.h, socket.h, struct_seaqt_string(data: if len(dgram) > 0: addr dgram[0] else: nil, len: csize_t(len(dgram))))

proc decryptDatagram*(self: gen_qdtls_types.QDtls, socket: gen_qudpsocket_types.QUdpSocket, dgram: openArray[byte]): seq[byte] =
  var v_bytearray = fcQDtls_decryptDatagram(self.h, socket.h, struct_seaqt_string(data: if len(dgram) > 0: addr dgram[0] else: nil, len: csize_t(len(dgram))))
  var vx_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](v_bytearray.data), 0, int(v_bytearray.len)-1))
  c_free(v_bytearray.data)
  vx_ret
...
proc ignoreVerificationErrors*(self: gen_qdtls_types.QDtls, errorsToIgnore: openArray[gen_qsslerror_types.QSslError]): void =
  ...
  fcQDtls_ignoreVerificationErrors(self.h, struct_seaqt_array(len: csize_t(len(errorsToIgnore)), data: if len(errorsToIgnore) == 0: nil else: addr(errorsToIgnore_CArray[0])))
...
proc create*(T: type gen_qdtls_types.QDtls,
    ...
    inst: VirtualQDtls) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDtls_new(addr(cQDtls_mvtbl), csize_t(sizeof(pointer)), cint(mode))
  fcQDtls_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qdtls_types.QDtls,
    ...
    inst: VirtualQDtls) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDtls_new2(addr(cQDtls_mvtbl), csize_t(sizeof(pointer)), cint(mode), parent.h)
  fcQDtls_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc hash*(self: gen_qdtls_types.QDtlsClientVerifierGeneratorParameters): cint =
  cint(fcQDtlsClientVerifierGeneratorParameters_hash(self.h))

proc setHash*(self: gen_qdtls_types.QDtlsClientVerifierGeneratorParameters, hash: cint): void =
  fcQDtlsClientVerifierGeneratorParameters_setHash(self.h, cint(hash))

proc secret*(self: gen_qdtls_types.QDtlsClientVerifierGeneratorParameters): seq[byte] =
  var vsecret_bytearray = fcQDtlsClientVerifierGeneratorParameters_secret(self.h)
  var vsecretx_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](vsecret_bytearray.data), 0, int(vsecret_bytearray.len)-1))
  c_free(vsecret_bytearray.data)
  vsecretx_ret
