...
public:
	...
	friend void QTcpSocket_virtualbase_connectToHost(VirtualQTcpSocket* self, struct miqt_string hostName, unsigned short port, int mode, int protocol);
...
VirtualQTcpSocket* QTcpSocket_new(const QTcpSocket_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQTcpSocket>());
...
void QTcpSocket_virtualbase_connectToHost(VirtualQTcpSocket* self, struct miqt_string hostName, unsigned short port, int mode, int protocol) {
