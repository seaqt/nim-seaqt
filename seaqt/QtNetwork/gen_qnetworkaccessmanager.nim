...
proc fcQNetworkAccessManager_get_QNetworkRequest(self: pointer, request: pointer): pointer {.importc: "QNetworkAccessManager_get_QNetworkRequest".}
proc fcQNetworkAccessManager_get_QNetworkRequest_QIODevice(self: pointer, request: pointer, data: pointer): pointer {.importc: "QNetworkAccessManager_get_QNetworkRequest_QIODevice".}
proc fcQNetworkAccessManager_get_QNetworkRequest_QByteArray(self: pointer, request: pointer, data: struct_seaqt_string): pointer {.importc: "QNetworkAccessManager_get_QNetworkRequest_QByteArray".}
...
type cQNetworkAccessManagerVTable {.pure.} = object
  ...
  supportedSchemes*: proc(self: pointer): struct_seaqt_array {.cdecl, raises: [], gcsafe.}
...
proc fcQNetworkAccessManager_virtualbase_supportedSchemes(self: pointer): struct_seaqt_array {.importc: "QNetworkAccessManager_virtualbase_supportedSchemes".}
...
proc get*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: gen_qnetworkrequest_types.QNetworkRequest): gen_qnetworkreply_types.QNetworkReply =
  gen_qnetworkreply_types.QNetworkReply(h: fcQNetworkAccessManager_get_QNetworkRequest(self.h, request.h), owned: false)

proc get*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: gen_qnetworkrequest_types.QNetworkRequest, data: gen_qiodevice_types.QIODevice): gen_qnetworkreply_types.QNetworkReply =
  gen_qnetworkreply_types.QNetworkReply(h: fcQNetworkAccessManager_get_QNetworkRequest_QIODevice(self.h, request.h, data.h), owned: false)

proc get*(self: gen_qnetworkaccessmanager_types.QNetworkAccessManager, request: gen_qnetworkrequest_types.QNetworkRequest, data: openArray[byte]): gen_qnetworkreply_types.QNetworkReply =
  gen_qnetworkreply_types.QNetworkReply(h: fcQNetworkAccessManager_get_QNetworkRequest_QByteArray(self.h, request.h, struct_seaqt_string(data: if len(data) > 0: addr data[0] else: nil, len: csize_t(len(data)))), owned: false)
...
proc fcQNetworkAccessManager_vtable_callback_supportedSchemes(self: pointer): struct_seaqt_array {.cdecl.} =
  let vtbl = cast[ptr QNetworkAccessManagerVTable](fcQNetworkAccessManager_vdata(self)[])
  let self = QNetworkAccessManager(h: self)
...
proc fcQNetworkAccessManager_method_callback_supportedSchemes(self: pointer): struct_seaqt_array {.cdecl.} =
  let inst = cast[VirtualQNetworkAccessManager](fcQNetworkAccessManager_vdata(self)[])
  var virtualReturn = inst.supportedSchemes()
  var virtualReturn_CArray = cast[ptr UncheckedArray[struct_seaqt_string]](if len(virtualReturn) > 0: c_malloc(c_sizet(sizeof(struct_seaqt_string) * len(virtualReturn))) else: nil)
  for i in 0..<len(virtualReturn):
    var virtualReturn_i_copy = if len(virtualReturn[i]) > 0: c_malloc(csize_t(len(virtualReturn[i]))) else: nil
    if len(virtualReturn[i]) > 0: copyMem(virtualReturn_i_copy, addr virtualReturn[i][0], csize_t(len(virtualReturn[i])))
...
proc create*(T: type gen_qnetworkaccessmanager_types.QNetworkAccessManager,
  ...
  if not isNil(vtbl[].supportedSchemes):
    vtbl[].vtbl.supportedSchemes = fcQNetworkAccessManager_vtable_callback_supportedSchemes
  ...
  let tmp = gen_qnetworkaccessmanager_types.QNetworkAccessManager(h: fcQNetworkAccessManager_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qnetworkaccessmanager_types.QNetworkAccessManager,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQNetworkAccessManager_vtable_callback_disconnectNotify
  let tmp = gen_qnetworkaccessmanager_types.QNetworkAccessManager(h: fcQNetworkAccessManager_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQNetworkAccessManager_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQNetworkAccessManager_mvtbl = cQNetworkAccessManagerVTable(
  ...
  supportedSchemes: fcQNetworkAccessManager_method_callback_supportedSchemes,
