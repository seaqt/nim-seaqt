...
proc fcQNetworkCacheMetaData_operatorAssign(self: pointer, fromVal: pointer): void {.importc: "QNetworkCacheMetaData_operatorAssign".}
...
proc fcQNetworkCacheMetaData_rawHeaders(self: pointer): struct_seaqt_array {.importc: "QNetworkCacheMetaData_rawHeaders".}
proc fcQNetworkCacheMetaData_setRawHeaders(self: pointer, headers: struct_seaqt_array): void {.importc: "QNetworkCacheMetaData_setRawHeaders".}
proc fcQNetworkCacheMetaData_headers(self: pointer): pointer {.importc: "QNetworkCacheMetaData_headers".}
proc fcQNetworkCacheMetaData_setHeaders(self: pointer, headers: pointer): void {.importc: "QNetworkCacheMetaData_setHeaders".}
...
proc fcQNetworkCacheMetaData_attributes(self: pointer): struct_seaqt_map {.importc: "QNetworkCacheMetaData_attributes".}
proc fcQNetworkCacheMetaData_setAttributes(self: pointer, attributes: struct_seaqt_map): void {.importc: "QNetworkCacheMetaData_setAttributes".}
proc fcQNetworkCacheMetaData_new(): ptr cQNetworkCacheMetaData {.importc: "QNetworkCacheMetaData_new".}
proc fcQNetworkCacheMetaData_new2(fromVal: pointer): ptr cQNetworkCacheMetaData {.importc: "QNetworkCacheMetaData_new_from".}
...
proc fcQAbstractNetworkCache_trS(s: cstring): struct_seaqt_string {.importc: "QAbstractNetworkCache_tr_s".}
...
proc fcQAbstractNetworkCache_trSC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QAbstractNetworkCache_tr_s_c".}
proc fcQAbstractNetworkCache_trSCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QAbstractNetworkCache_tr_s_c_n".}
...
proc operatorAssign*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData, fromVal: gen_qabstractnetworkcache_types.QNetworkCacheMetaData): void =
  fcQNetworkCacheMetaData_operatorAssign(self.h, fromVal.h)
...
proc setRawHeaders*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData, headers: openArray[tuple[first: seq[byte], second: seq[byte]]]): void =
  var headers_CArray = newSeq[struct_seaqt_map](len(headers))
  for i in 0..<len(headers):
    var headers_i_CArray_First: struct_seaqt_string
    var headers_i_CArray_Second: struct_seaqt_string
    headers_i_CArray_First = struct_seaqt_string(data: if len(headers[i].first) > 0: addr headers[i].first[0] else: nil, len: csize_t(len(headers[i].first)))
    headers_i_CArray_Second = struct_seaqt_string(data: if len(headers[i].second) > 0: addr headers[i].second[0] else: nil, len: csize_t(len(headers[i].second)))
    headers_CArray[i] = struct_seaqt_map(len: 1,keys: addr(headers_i_CArray_First),values: addr(headers_i_CArray_Second),)

  fcQNetworkCacheMetaData_setRawHeaders(self.h, struct_seaqt_array(len: csize_t(len(headers)), data: if len(headers) == 0: nil else: addr(headers_CArray[0])))

proc headers*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData): gen_qhttpheaders_types.QHttpHeaders =
  gen_qhttpheaders_types.QHttpHeaders(h: fcQNetworkCacheMetaData_headers(self.h), owned: true)

proc setHeaders*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData, headers: gen_qhttpheaders_types.QHttpHeaders): void =
  fcQNetworkCacheMetaData_setHeaders(self.h, headers.h)
...
proc setAttributes*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData, attributes: Table[cint,gen_qvariant_types.QVariant]): void =
  ...
  fcQNetworkCacheMetaData_setAttributes(self.h, struct_seaqt_map(len: csize_t(len(attributes)),keys: if len(attributes) == 0: nil else: addr(attributes_Keys_CArray[0]), values: if len(attributes) == 0: nil else: addr(attributes_Values_CArray[0]),))
...
proc create*(T: type gen_qabstractnetworkcache_types.QNetworkCacheMetaData): gen_qabstractnetworkcache_types.QNetworkCacheMetaData =
  let tmp = gen_qabstractnetworkcache_types.QNetworkCacheMetaData(h: fcQNetworkCacheMetaData_new(), owned: true)
  tmp
proc create*(T: type gen_qabstractnetworkcache_types.QNetworkCacheMetaData,
    fromVal: gen_qabstractnetworkcache_types.QNetworkCacheMetaData): gen_qabstractnetworkcache_types.QNetworkCacheMetaData =
  let tmp = gen_qabstractnetworkcache_types.QNetworkCacheMetaData(h: fcQNetworkCacheMetaData_new2(fromVal.h), owned: true)
  tmp
...
proc tr*(_: type gen_qabstractnetworkcache_types.QAbstractNetworkCache, s: cstring): string =
  let v_ms = fcQAbstractNetworkCache_trS(s)
