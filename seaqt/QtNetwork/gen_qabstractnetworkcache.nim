...
proc setRawHeaders*(self: gen_qabstractnetworkcache_types.QNetworkCacheMetaData, headers: openArray[tuple[first: seq[byte], second: seq[byte]]]): void =
  ...
  for i in 0..<len(headers):
    ...
    headers_i_CArray_First = struct_miqt_string(data: if len(headers[i].first) > 0: addr headers[i].first[0] else: nil, len: csize_t(len(headers[i].first)))
    headers_i_CArray_Second = struct_miqt_string(data: if len(headers[i].second) > 0: addr headers[i].second[0] else: nil, len: csize_t(len(headers[i].second)))
