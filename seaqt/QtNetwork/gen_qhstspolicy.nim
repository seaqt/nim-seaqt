...
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
  ...
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new2(expiry.h, cint(flags), struct_miqt_string(data: if len(host) > 0: addr host[0] else: nil, len: csize_t(len(host)))))
  tmp
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
...
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
  ...
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new4(expiry.h, cint(flags), struct_miqt_string(data: if len(host) > 0: addr host[0] else: nil, len: csize_t(len(host))), cint(mode)))
  tmp
