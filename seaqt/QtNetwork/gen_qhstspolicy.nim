...
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy): gen_qhstspolicy_types.QHstsPolicy =
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new())
  tmp
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
  ...
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new2(expiry.h, cint(flags), struct_seaqt_string(data: if len(host) > 0: addr host[0] else: nil, len: csize_t(len(host)))))
  tmp
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
    rhs: gen_qhstspolicy_types.QHstsPolicy): gen_qhstspolicy_types.QHstsPolicy =
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new3(rhs.h))
  tmp
proc create*(T: type gen_qhstspolicy_types.QHstsPolicy,
    expiry: gen_qdatetime_types.QDateTime, flags: cint, host: openArray[char], mode: cint): gen_qhstspolicy_types.QHstsPolicy =
  let tmp = gen_qhstspolicy_types.QHstsPolicy(h: fcQHstsPolicy_new4(expiry.h, cint(flags), struct_seaqt_string(data: if len(host) > 0: addr host[0] else: nil, len: csize_t(len(host))), cint(mode)))
  tmp
