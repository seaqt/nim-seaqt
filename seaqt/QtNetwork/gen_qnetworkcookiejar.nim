...
type cQNetworkCookieJarVTable {.pure.} = object
  ...
  cookiesForUrl*: proc(self: pointer, url: pointer): struct_seaqt_array {.cdecl, raises: [], gcsafe.}
  setCookiesFromUrl*: proc(self: pointer, cookieList: struct_seaqt_array, url: pointer): bool {.cdecl, raises: [], gcsafe.}
...
proc fcQNetworkCookieJar_virtualbase_cookiesForUrl(self: pointer, url: pointer): struct_seaqt_array {.importc: "QNetworkCookieJar_virtualbase_cookiesForUrl".}
proc fcQNetworkCookieJar_virtualbase_setCookiesFromUrl(self: pointer, cookieList: struct_seaqt_array, url: pointer): bool {.importc: "QNetworkCookieJar_virtualbase_setCookiesFromUrl".}
...
proc fcQNetworkCookieJar_protectedbase_allCookies(self: pointer): struct_seaqt_array {.importc: "QNetworkCookieJar_protectedbase_allCookies".}
proc fcQNetworkCookieJar_protectedbase_setAllCookies(self: pointer, cookieList: struct_seaqt_array): void {.importc: "QNetworkCookieJar_protectedbase_setAllCookies".}
...
proc setCookiesFromUrl*(self: gen_qnetworkcookiejar_types.QNetworkCookieJar, cookieList: openArray[gen_qnetworkcookie_types.QNetworkCookie], url: gen_qurl_types.QUrl): bool =
  ...
  fcQNetworkCookieJar_setCookiesFromUrl(self.h, struct_seaqt_array(len: csize_t(len(cookieList)), data: if len(cookieList) == 0: nil else: addr(cookieList_CArray[0])), url.h)
...
proc QNetworkCookieJarsetCookiesFromUrl*(self: gen_qnetworkcookiejar_types.QNetworkCookieJar, cookieList: openArray[gen_qnetworkcookie_types.QNetworkCookie], url: gen_qurl_types.QUrl): bool =
  ...
  fcQNetworkCookieJar_virtualbase_setCookiesFromUrl(self.h, struct_seaqt_array(len: csize_t(len(cookieList)), data: if len(cookieList) == 0: nil else: addr(cookieList_CArray[0])), url.h)
...
proc fcQNetworkCookieJar_vtable_callback_cookiesForUrl(self: pointer, url: pointer): struct_seaqt_array {.cdecl.} =
  let vtbl = cast[ptr QNetworkCookieJarVTable](fcQNetworkCookieJar_vdata(self)[])
  let self = QNetworkCookieJar(h: self)
...
proc fcQNetworkCookieJar_vtable_callback_setCookiesFromUrl(self: pointer, cookieList: struct_seaqt_array, url: pointer): bool {.cdecl.} =
  let vtbl = cast[ptr QNetworkCookieJarVTable](fcQNetworkCookieJar_vdata(self)[])
  let self = QNetworkCookieJar(h: self)
...
proc fcQNetworkCookieJar_method_callback_cookiesForUrl(self: pointer, url: pointer): struct_seaqt_array {.cdecl.} =
  let inst = cast[VirtualQNetworkCookieJar](fcQNetworkCookieJar_vdata(self)[])
  let slotval1 = gen_qurl_types.QUrl(h: url, owned: false)
...
proc fcQNetworkCookieJar_method_callback_setCookiesFromUrl(self: pointer, cookieList: struct_seaqt_array, url: pointer): bool {.cdecl.} =
  let inst = cast[VirtualQNetworkCookieJar](fcQNetworkCookieJar_vdata(self)[])
  var vcookieList_ma = cookieList
  var vcookieListx_ret = newSeq[gen_qnetworkcookie_types.QNetworkCookie](int(vcookieList_ma.len))
  let vcookieList_outCast = cast[ptr UncheckedArray[pointer]](vcookieList_ma.data)
...
proc setAllCookies*(self: gen_qnetworkcookiejar_types.QNetworkCookieJar, cookieList: openArray[gen_qnetworkcookie_types.QNetworkCookie]): void =
  ...
  fcQNetworkCookieJar_protectedbase_setAllCookies(self.h, struct_seaqt_array(len: csize_t(len(cookieList)), data: if len(cookieList) == 0: nil else: addr(cookieList_CArray[0])))
...
proc create*(T: type gen_qnetworkcookiejar_types.QNetworkCookieJar,
  ...
  let tmp = gen_qnetworkcookiejar_types.QNetworkCookieJar(h: fcQNetworkCookieJar_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQNetworkCookieJar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qnetworkcookiejar_types.QNetworkCookieJar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQNetworkCookieJar_vtable_callback_disconnectNotify
  let tmp = gen_qnetworkcookiejar_types.QNetworkCookieJar(h: fcQNetworkCookieJar_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQNetworkCookieJar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
