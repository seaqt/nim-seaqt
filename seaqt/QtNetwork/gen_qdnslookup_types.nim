...
type QDnsTlsAssociationRecord* {.inheritable.} = object
  h*: pointer
  owned*: bool

proc fcQDnsTlsAssociationRecord_delete(self: pointer) {.importc: "QDnsTlsAssociationRecord_delete".}
proc `=destroy`(self: var QDnsTlsAssociationRecord) =
  if self.owned: fcQDnsTlsAssociationRecord_delete(self.h)

proc `=sink`(dest: var QDnsTlsAssociationRecord, source: QDnsTlsAssociationRecord) =
  `=destroy`(dest)
  wasMoved(dest)
  dest.h = source.h
  dest.owned = source.owned

proc `=copy`(dest: var QDnsTlsAssociationRecord, source: QDnsTlsAssociationRecord) {.error.}
proc delete*(self: sink QDnsTlsAssociationRecord) =
  let h = self.h
  wasMoved(self)
  fcQDnsTlsAssociationRecord_delete(h)
