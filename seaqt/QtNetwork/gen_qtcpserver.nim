...
proc fcQTcpServer_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QTcpServer_trUtf8_s".}
...
proc fcQTcpServer_errorString(self: pointer): struct_seaqt_string {.importc: "QTcpServer_errorString".}
...
proc fcQTcpServer_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QTcpServer_trUtf8_s_c".}
proc fcQTcpServer_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QTcpServer_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qtcpserver_types.QTcpServer, s: cstring): string =
  let v_ms = fcQTcpServer_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qtcpserver_types.QTcpServer,
  ...
  let tmp = gen_qtcpserver_types.QTcpServer(h: fcQTcpServer_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQTcpServer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtcpserver_types.QTcpServer,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTcpServer_vtable_callback_disconnectNotify
  let tmp = gen_qtcpserver_types.QTcpServer(h: fcQTcpServer_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQTcpServer_vdata(tmp.h)[] = addr(vtbl[])
  tmp
