...
proc fcQDnsLookup_new2(vtbl: pointer, vdata: csize_t, typeVal: cint, name: struct_miqt_string): ptr cQDnsLookup {.importc: "QDnsLookup_new2".}
proc fcQDnsLookup_new3(vtbl: pointer, vdata: csize_t, typeVal: cint, name: struct_miqt_string, nameserver: pointer): ptr cQDnsLookup {.importc: "QDnsLookup_new3".}
proc fcQDnsLookup_new4(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQDnsLookup {.importc: "QDnsLookup_new4".}
proc fcQDnsLookup_new5(vtbl: pointer, vdata: csize_t, typeVal: cint, name: struct_miqt_string, parent: pointer): ptr cQDnsLookup {.importc: "QDnsLookup_new5".}
proc fcQDnsLookup_new6(vtbl: pointer, vdata: csize_t, typeVal: cint, name: struct_miqt_string, nameserver: pointer, parent: pointer): ptr cQDnsLookup {.importc: "QDnsLookup_new6".}
...
proc setName*(self: gen_qdnslookup_types.QDnsLookup, name: string): void =
  fcQDnsLookup_setName(self.h, struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc nameChanged*(self: gen_qdnslookup_types.QDnsLookup, name: string): void =
  fcQDnsLookup_nameChanged(self.h, struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))

type QDnsLookupnameChangedSlot* = proc(name: string)
proc fcQDnsLookup_slot_callback_nameChanged(slot: int, name: struct_miqt_string) {.cdecl.} =
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQDnsLookup_vtable_callback_disconnectNotify
  let tmp = gen_qdnslookup_types.QDnsLookup(h: fcQDnsLookup_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))))
  fcQDnsLookup_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQDnsLookup_vtable_callback_disconnectNotify
  let tmp = gen_qdnslookup_types.QDnsLookup(h: fcQDnsLookup_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), nameserver.h))
  fcQDnsLookup_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQDnsLookup_vtable_callback_disconnectNotify
  let tmp = gen_qdnslookup_types.QDnsLookup(h: fcQDnsLookup_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), parent.h))
  fcQDnsLookup_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQDnsLookup_vtable_callback_disconnectNotify
  let tmp = gen_qdnslookup_types.QDnsLookup(h: fcQDnsLookup_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), nameserver.h, parent.h))
  fcQDnsLookup_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
    ...
    inst: VirtualQDnsLookup) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDnsLookup_new2(addr(cQDnsLookup_mvtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
  fcQDnsLookup_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
    ...
    inst: VirtualQDnsLookup) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDnsLookup_new3(addr(cQDnsLookup_mvtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), nameserver.h)
  fcQDnsLookup_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
    ...
    inst: VirtualQDnsLookup) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDnsLookup_new5(addr(cQDnsLookup_mvtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), parent.h)
  fcQDnsLookup_vdata(inst[].h)[] = addr inst[]
...
proc create*(T: type gen_qdnslookup_types.QDnsLookup,
    ...
    inst: VirtualQDnsLookup) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQDnsLookup_new6(addr(cQDnsLookup_mvtbl), csize_t(sizeof(pointer)), cint(typeVal), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))), nameserver.h, parent.h)
  fcQDnsLookup_vdata(inst[].h)[] = addr inst[]
