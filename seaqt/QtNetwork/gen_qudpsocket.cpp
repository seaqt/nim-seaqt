...
public:
	...
	friend void QUdpSocket_virtualbase_connectToHost(VirtualQUdpSocket* self, struct miqt_string hostName, unsigned short port, int mode, int protocol);
...
VirtualQUdpSocket* QUdpSocket_new(const QUdpSocket_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQUdpSocket>());
...
long long QUdpSocket_readDatagram4(QUdpSocket* self, char* data, long long maxlen, QHostAddress* host, unsigned short* port) {
...
void QUdpSocket_virtualbase_connectToHost(VirtualQUdpSocket* self, struct miqt_string hostName, unsigned short port, int mode, int protocol) {
