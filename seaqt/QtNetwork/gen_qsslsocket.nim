...
proc fcQSslSocket_slot_callback_sslErrors(slot: int, errors: struct_miqt_array) {.cdecl.} =
...
proc fcQSslSocket_slot_callback_alertSent(slot: int, level: cint, typeVal: cint, description: struct_miqt_string) {.cdecl.} =
...
proc fcQSslSocket_slot_callback_alertReceived(slot: int, level: cint, typeVal: cint, description: struct_miqt_string) {.cdecl.} =
...
proc setPrivateKey*(self: gen_qsslsocket_types.QSslSocket, fileName: openArray[char], algorithm: cint, format: cint, passPhrase: openArray[byte]): void =
  fcQSslSocket_setPrivateKey4(self.h, struct_miqt_string(data: if len(fileName) > 0: addr fileName[0] else: nil, len: csize_t(len(fileName))), cint(algorithm), cint(format), struct_miqt_string(data: if len(passPhrase) > 0: addr passPhrase[0] else: nil, len: csize_t(len(passPhrase))))
...
proc fcQSslSocket_vtable_callback_connectToHost(self: pointer, hostName: struct_miqt_string, port: cushort, openMode: cint, protocol: cint): void {.cdecl.} =
  let vtbl = cast[ptr QSslSocketVTable](fcQSslSocket_vdata(self)[])
  let self = QSslSocket(h: self)
...
proc fcQSslSocket_method_callback_connectToHost(self: pointer, hostName: struct_miqt_string, port: cushort, openMode: cint, protocol: cint): void {.cdecl.} =
  let inst = cast[VirtualQSslSocket](fcQSslSocket_vdata(self)[])
  let vhostName_ms = hostName
  let vhostNamex_ret = string.fromBytes(vhostName_ms)
  c_free(vhostName_ms.data)
