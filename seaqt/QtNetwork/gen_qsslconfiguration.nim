...
proc fcQSslConfiguration_setCiphers_QListOfQSslCipher(self: pointer, ciphers: struct_seaqt_array): void {.importc: "QSslConfiguration_setCiphers_QListOfQSslCipher".}
proc fcQSslConfiguration_setCiphers_QString(self: pointer, ciphers: struct_seaqt_string): void {.importc: "QSslConfiguration_setCiphers_QString".}
...
proc setCiphers*(self: gen_qsslconfiguration_types.QSslConfiguration, ciphers: openArray[gen_qsslcipher_types.QSslCipher]): void =
  ...
  fcQSslConfiguration_setCiphers_QListOfQSslCipher(self.h, struct_seaqt_array(len: csize_t(len(ciphers)), data: if len(ciphers) == 0: nil else: addr(ciphers_CArray[0])))

proc setCiphers*(self: gen_qsslconfiguration_types.QSslConfiguration, ciphers: openArray[char]): void =
  fcQSslConfiguration_setCiphers_QString(self.h, struct_seaqt_string(data: if len(ciphers) > 0: addr ciphers[0] else: nil, len: csize_t(len(ciphers))))
