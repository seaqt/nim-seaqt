...
proc create*(T: type gen_qhostaddress_types.QHostAddress,
  ...
  let tmp = gen_qhostaddress_types.QHostAddress(h: fcQHostAddress_new5(struct_miqt_string(data: if len(address) > 0: addr address[0] else: nil, len: csize_t(len(address)))))
  tmp
proc create*(T: type gen_qhostaddress_types.QHostAddress,
