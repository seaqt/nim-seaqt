...
proc create*(T: type gen_qnetworkdatagram_types.QNetworkDatagram,
  ...
  let tmp = gen_qnetworkdatagram_types.QNetworkDatagram(h: fcQNetworkDatagram_new4(struct_seaqt_string(data: if len(data) > 0: addr data[0] else: nil, len: csize_t(len(data))), destinationAddress.h), owned: true)
  tmp
proc create*(T: type gen_qnetworkdatagram_types.QNetworkDatagram,
  ...
  let tmp = gen_qnetworkdatagram_types.QNetworkDatagram(h: fcQNetworkDatagram_new5(struct_seaqt_string(data: if len(data) > 0: addr data[0] else: nil, len: csize_t(len(data))), destinationAddress.h, port), owned: true)
  tmp
