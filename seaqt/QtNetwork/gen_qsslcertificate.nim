...
proc fcQSslCertificate_fromPath(path: struct_seaqt_string): struct_seaqt_array {.importc: "QSslCertificate_fromPath".}
...
proc fromPath*(_: type gen_qsslcertificate_types.QSslCertificate, path: openArray[char]): seq[gen_qsslcertificate_types.QSslCertificate] =
  var v_ma = fcQSslCertificate_fromPath(struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
