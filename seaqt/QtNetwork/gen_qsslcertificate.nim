...
proc create*(T: type gen_qsslcertificate_types.QSslCertificate,
  ...
  let tmp = gen_qsslcertificate_types.QSslCertificate(h: fcQSslCertificate_new5(struct_miqt_string(data: if len(data) > 0: addr data[0] else: nil, len: csize_t(len(data)))))
  tmp
proc create*(T: type gen_qsslcertificate_types.QSslCertificate,
    data: seq[byte], format: cint): gen_qsslcertificate_types.QSslCertificate =
  let tmp = gen_qsslcertificate_types.QSslCertificate(h: fcQSslCertificate_new6(struct_miqt_string(data: if len(data) > 0: addr data[0] else: nil, len: csize_t(len(data))), cint(format)))
  tmp
