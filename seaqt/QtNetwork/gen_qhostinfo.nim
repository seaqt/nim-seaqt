...
proc fcQHostInfo_addresses(self: pointer): struct_seaqt_array {.importc: "QHostInfo_addresses".}
proc fcQHostInfo_setAddresses(self: pointer, addresses: struct_seaqt_array): void {.importc: "QHostInfo_setAddresses".}
...
proc fcQHostInfo_errorString(self: pointer): struct_seaqt_string {.importc: "QHostInfo_errorString".}
proc fcQHostInfo_setErrorString(self: pointer, errorString: struct_seaqt_string): void {.importc: "QHostInfo_setErrorString".}
...
proc fcQHostInfo_fromName(name: struct_seaqt_string): pointer {.importc: "QHostInfo_fromName".}
...
proc fcQHostInfo_staticMetaObject(): pointer {.importc: "QHostInfo_staticMetaObject".}
...
proc setHostName*(self: gen_qhostinfo_types.QHostInfo, name: openArray[char]): void =
  fcQHostInfo_setHostName(self.h, struct_seaqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name))))
...
proc setAddresses*(self: gen_qhostinfo_types.QHostInfo, addresses: openArray[gen_qhostaddress_types.QHostAddress]): void =
  ...
  fcQHostInfo_setAddresses(self.h, struct_seaqt_array(len: csize_t(len(addresses)), data: if len(addresses) == 0: nil else: addr(addresses_CArray[0])))
...
proc setErrorString*(self: gen_qhostinfo_types.QHostInfo, errorString: openArray[char]): void =
  fcQHostInfo_setErrorString(self.h, struct_seaqt_string(data: if len(errorString) > 0: addr errorString[0] else: nil, len: csize_t(len(errorString))))
...
proc create*(T: type gen_qhostinfo_types.QHostInfo,
  ...
  tmp
proc staticMetaObject*(_: type gen_qhostinfo_types.QHostInfo): gen_qobjectdefs_types.QMetaObject =
  gen_qobjectdefs_types.QMetaObject(h: fcQHostInfo_staticMetaObject())
