...
proc fcQComboBox_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QComboBox_virtualbase_nativeEvent".}
...
proc fcQComboBox_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQComboBox {.importc: "QComboBox_new2".}
...
proc currentData*(self: gen_qcombobox_types.QComboBox, role: cint): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQComboBox_currentDataWithRole(self.h, role))
...
proc fcQComboBox_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QComboBoxVTable](fcQComboBox_vdata(self)[])
  let self = QComboBox(h: self)
...
proc fcQComboBox_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQComboBox](fcQComboBox_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qcombobox_types.QComboBox,
  ...
  let tmp = gen_qcombobox_types.QComboBox(h: fcQComboBox_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qcombobox_types.QComboBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQComboBox_vtable_callback_disconnectNotify
  let tmp = gen_qcombobox_types.QComboBox(h: fcQComboBox_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQComboBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
