...
type QComboBoxLabelDrawingModeEnum* = distinct cint
template UseStyle*(_: type QComboBoxLabelDrawingModeEnum): untyped = 0
template UseDelegate*(_: type QComboBoxLabelDrawingModeEnum): untyped = 1
...
proc fcQComboBox_setPlaceholderText(self: pointer, placeholderText: struct_seaqt_string): void {.importc: "QComboBox_setPlaceholderText".}
proc fcQComboBox_placeholderText(self: pointer): struct_seaqt_string {.importc: "QComboBox_placeholderText".}
...
proc fcQComboBox_labelDrawingMode(self: pointer): cint {.importc: "QComboBox_labelDrawingMode".}
proc fcQComboBox_setLabelDrawingMode(self: pointer, labelDrawing: cint): void {.importc: "QComboBox_setLabelDrawingMode".}
proc fcQComboBox_currentIndex(self: pointer): cint {.importc: "QComboBox_currentIndex".}
proc fcQComboBox_currentText(self: pointer): struct_seaqt_string {.importc: "QComboBox_currentText".}
...
proc fcQComboBox_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QComboBox_virtualbase_nativeEvent".}
...
proc fcQComboBox_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QComboBox_protectedbase_getDecodedMetricF".}
proc fcQComboBox_new(vtbl: pointer, vdata: csize_t): ptr cQComboBox {.importc: "QComboBox_new".}
proc fcQComboBox_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQComboBox {.importc: "QComboBox_new_parent".}
...
proc setPlaceholderText*(self: gen_qcombobox_types.QComboBox, placeholderText: openArray[char]): void =
  fcQComboBox_setPlaceholderText(self.h, struct_seaqt_string(data: if len(placeholderText) > 0: addr placeholderText[0] else: nil, len: csize_t(len(placeholderText))))
...
proc labelDrawingMode*(self: gen_qcombobox_types.QComboBox): cint =
  cint(fcQComboBox_labelDrawingMode(self.h))

proc setLabelDrawingMode*(self: gen_qcombobox_types.QComboBox, labelDrawing: cint): void =
  fcQComboBox_setLabelDrawingMode(self.h, cint(labelDrawing))
...
proc addItems*(self: gen_qcombobox_types.QComboBox, texts: openArray[string]): void =
  ...
  for i in 0..<len(texts):
    texts_CArray[i] = struct_seaqt_string(data: if len(texts[i]) > 0: addr texts[i][0] else: nil, len: csize_t(len(texts[i])))
...
proc insertItems*(self: gen_qcombobox_types.QComboBox, index: cint, texts: openArray[string]): void =
  ...
  fcQComboBox_insertItems(self.h, index, struct_seaqt_array(len: csize_t(len(texts)), data: if len(texts) == 0: nil else: addr(texts_CArray[0])))
...
proc setItemText*(self: gen_qcombobox_types.QComboBox, index: cint, text: openArray[char]): void =
  fcQComboBox_setItemText(self.h, index, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc setEditText*(self: gen_qcombobox_types.QComboBox, text: openArray[char]): void =
  fcQComboBox_setEditText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc setCurrentText*(self: gen_qcombobox_types.QComboBox, text: openArray[char]): void =
  fcQComboBox_setCurrentText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))

proc editTextChanged*(self: gen_qcombobox_types.QComboBox, param1: openArray[char]): void =
  fcQComboBox_editTextChanged(self.h, struct_seaqt_string(data: if len(param1) > 0: addr param1[0] else: nil, len: csize_t(len(param1))))

type QComboBoxeditTextChangedSlot* = proc(param1: openArray[char])
proc fcQComboBox_slot_callback_editTextChanged(slot: int, param1: struct_seaqt_string) {.cdecl.} =
...
proc fcQComboBox_slot_callback_textActivated(slot: int, param1: struct_seaqt_string) {.cdecl.} =
...
proc fcQComboBox_slot_callback_textHighlighted(slot: int, param1: struct_seaqt_string) {.cdecl.} =
...
proc fcQComboBox_slot_callback_currentTextChanged(slot: int, param1: struct_seaqt_string) {.cdecl.} =
...
proc QComboBoxnativeEvent*(self: gen_qcombobox_types.QComboBox, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQComboBox_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQComboBox_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QComboBoxVTable](fcQComboBox_vdata(self)[])
  let self = QComboBox(h: self)
...
proc fcQComboBox_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQComboBox](fcQComboBox_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qcombobox_types.QComboBox, metricA: cint, metricB: cint): float64 =
  fcQComboBox_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qcombobox_types.QComboBox,
  ...
  let tmp = gen_qcombobox_types.QComboBox(h: fcQComboBox_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQComboBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcombobox_types.QComboBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQComboBox_vtable_callback_disconnectNotify
  let tmp = gen_qcombobox_types.QComboBox(h: fcQComboBox_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQComboBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qcombobox_types.QComboBox,
  ...
  inst[].h = fcQComboBox_new(addr(cQComboBox_mvtbl), csize_t(sizeof(pointer)))
  fcQComboBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcombobox_types.QComboBox,
    ...
    inst: VirtualQComboBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQComboBox_new2(addr(cQComboBox_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQComboBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
