...
method nativeEvent*(self: VirtualQComboBox, eventType: seq[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
  QComboBoxnativeEvent(self[], eventType, message, resultVal)
proc miqt_exec_method_cQComboBox_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[VirtualQComboBox](cast[uint](vtbl) - uint(offsetOf(VirtualQComboBox, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
