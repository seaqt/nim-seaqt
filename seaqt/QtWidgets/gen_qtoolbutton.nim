...
proc fcQToolButton_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QToolButton_virtualbase_nativeEvent".}
...
proc fcQToolButton_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QToolButton_protectedbase_getDecodedMetricF".}
...
proc QToolButtonnativeEvent*(self: gen_qtoolbutton_types.QToolButton, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQToolButton_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQToolButton_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QToolButtonVTable](fcQToolButton_vdata(self)[])
  let self = QToolButton(h: self)
...
proc fcQToolButton_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QToolButtonVTable](fcQToolButton_vdata(self)[])
  let self = QToolButton(h: self)
...
proc fcQToolButton_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQToolButton](fcQToolButton_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionToolButton(h: option, owned: false)
...
proc fcQToolButton_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQToolButton](fcQToolButton_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qtoolbutton_types.QToolButton, metricA: cint, metricB: cint): float64 =
  fcQToolButton_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qtoolbutton_types.QToolButton,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQToolButton_vtable_callback_initStyleOption
  ...
  let tmp = gen_qtoolbutton_types.QToolButton(h: fcQToolButton_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQToolButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbutton_types.QToolButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolButton_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbutton_types.QToolButton(h: fcQToolButton_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQToolButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQToolButton_mvtbl = cQToolButtonVTable(
  ...
  initStyleOption: fcQToolButton_method_callback_initStyleOption,
...
proc create*(T: type gen_qtoolbutton_types.QToolButton,
  ...
  inst[].h = fcQToolButton_new(addr(cQToolButton_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qtoolbutton_types.QToolButton,
    ...
    inst: VirtualQToolButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolButton_new2(addr(cQToolButton_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQToolButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
