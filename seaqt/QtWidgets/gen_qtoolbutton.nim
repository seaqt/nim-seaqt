...
proc create*(T: type gen_qtoolbutton_types.QToolButton,
  ...
  let tmp = gen_qtoolbutton_types.QToolButton(h: fcQToolButton_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qtoolbutton_types.QToolButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolButton_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbutton_types.QToolButton(h: fcQToolButton_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQToolButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
