...
proc miqt_exec_method_cQItemEditorFactory_createEditor(vtbl: pointer, inst: pointer, userType: cint, parent: pointer): pointer {.cdecl.} =
  let vtbl = cast[VirtualQItemEditorFactory](cast[uint](vtbl) - uint(offsetOf(VirtualQItemEditorFactory, vtbl)))
  let slotval1 = userType
  let slotval2 = gen_qwidget_types.QWidget(h: parent, owned: false)
  var virtualReturn = vtbl.createEditor(slotval1, slotval2)
  virtualReturn.owned = false # TODO move?
  let virtualReturn_h = virtualReturn.h
  virtualReturn.h = nil
  virtualReturn_h
...
proc miqt_exec_method_cQItemEditorFactory_valuePropertyName(vtbl: pointer, inst: pointer, userType: cint): struct_miqt_string {.cdecl.} =
  let vtbl = cast[VirtualQItemEditorFactory](cast[uint](vtbl) - uint(offsetOf(VirtualQItemEditorFactory, vtbl)))
  let slotval1 = userType
  var virtualReturn = vtbl.valuePropertyName(slotval1)
  var virtualReturn_copy = cast[cstring](if len(virtualReturn) > 0: c_malloc(csize_t(len(virtualReturn))) else: nil)
  if len(virtualReturn) > 0: copyMem(cast[pointer](virtualReturn_copy), addr virtualReturn[0], csize_t(len(virtualReturn)))
  struct_miqt_string(data: virtualReturn_copy, len: csize_t(len(virtualReturn)))
...
proc create*(T: type gen_qitemeditorfactory_types.QItemEditorFactory,
  ...
  gen_qitemeditorfactory_types.QItemEditorFactory(h: fcQItemEditorFactory_new(addr(vtbl[].vtbl), ), owned: true)
...
proc create*(T: type gen_qitemeditorfactory_types.QItemEditorFactory,
  ...
  if not isNil(vtbl[].valuePropertyName):
    vtbl[].vtbl.valuePropertyName = miqt_exec_callback_cQItemEditorFactory_valuePropertyName
  gen_qitemeditorfactory_types.QItemEditorFactory(h: fcQItemEditorFactory_new2(addr(vtbl[].vtbl), param1.h), owned: true)

proc create*(T: type gen_qitemeditorfactory_types.QItemEditorFactory,
    vtbl: VirtualQItemEditorFactory) =

  vtbl[].vtbl.destructor = proc(vtbl: ptr cQItemEditorFactoryVTable, _: ptr cQItemEditorFactory) {.cdecl.} =
    let vtbl = cast[ptr typeof(VirtualQItemEditorFactory()[])](cast[uint](vtbl) - uint(offsetOf(VirtualQItemEditorFactory, vtbl)))
    vtbl[].h = nil
    vtbl[].owned = false
  vtbl[].vtbl.createEditor = miqt_exec_method_cQItemEditorFactory_createEditor
  vtbl[].vtbl.valuePropertyName = miqt_exec_method_cQItemEditorFactory_valuePropertyName
  if vtbl[].h != nil: delete(move(vtbl[]))
  vtbl[].h = fcQItemEditorFactory_new(addr(vtbl[].vtbl), )
  vtbl[].owned = true
...
proc create*(T: type gen_qitemeditorfactory_types.QItemEditorFactory,
  ...
  vtbl[].vtbl.valuePropertyName = miqt_exec_method_cQItemEditorFactory_valuePropertyName
  if vtbl[].h != nil: delete(move(vtbl[]))
  vtbl[].h = fcQItemEditorFactory_new2(addr(vtbl[].vtbl), param1.h)
  vtbl[].owned = true
