...
proc fcQStackedLayout_widgetAdded(self: pointer, index: cint): void {.importc: "QStackedLayout_widgetAdded".}
proc fcQStackedLayout_connect_widgetAdded(self: pointer, slot: int, callback: proc (slot: int, index: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QStackedLayout_connect_widgetAdded".}
...
proc widgetAdded*(self: gen_qstackedlayout_types.QStackedLayout, index: cint): void =
  fcQStackedLayout_widgetAdded(self.h, index)

type QStackedLayoutwidgetAddedSlot* = proc(index: cint)
proc fcQStackedLayout_slot_callback_widgetAdded(slot: int, index: cint) {.cdecl.} =
  let nimfunc = cast[ptr QStackedLayoutwidgetAddedSlot](cast[pointer](slot))
  let slotval1 = index

  nimfunc[](slotval1)

proc fcQStackedLayout_slot_callback_widgetAdded_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QStackedLayoutwidgetAddedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onWidgetAdded*(self: gen_qstackedlayout_types.QStackedLayout, slot: QStackedLayoutwidgetAddedSlot) =
  var tmp = new QStackedLayoutwidgetAddedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQStackedLayout_connect_widgetAdded(self.h, cast[int](addr tmp[]), fcQStackedLayout_slot_callback_widgetAdded, fcQStackedLayout_slot_callback_widgetAdded_release)
...
proc fcQStackedLayout_vtable_callback_spacing(self: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QStackedLayoutVTable](fcQStackedLayout_vdata(self)[])
  let self = QStackedLayout(h: self)
  var virtualReturn = vtbl[].spacing(self)
  virtualReturn

proc fcQStackedLayout_vtable_callback_setSpacing(self: pointer, spacing: cint): void {.cdecl.} =
  let vtbl = cast[ptr QStackedLayoutVTable](fcQStackedLayout_vdata(self)[])
  let self = QStackedLayout(h: self)
...
proc fcQStackedLayout_vtable_callback_replaceWidget(self: pointer, fromVal: pointer, to: pointer, options: cint): pointer {.cdecl.} =
  let vtbl = cast[ptr QStackedLayoutVTable](fcQStackedLayout_vdata(self)[])
  let self = QStackedLayout(h: self)
...
proc fcQStackedLayout_method_callback_spacing(self: pointer): cint {.cdecl.} =
  let inst = cast[VirtualQStackedLayout](fcQStackedLayout_vdata(self)[])
  var virtualReturn = inst.spacing()
  virtualReturn

proc fcQStackedLayout_method_callback_setSpacing(self: pointer, spacing: cint): void {.cdecl.} =
  let inst = cast[VirtualQStackedLayout](fcQStackedLayout_vdata(self)[])
  let slotval1 = spacing
...
proc fcQStackedLayout_method_callback_replaceWidget(self: pointer, fromVal: pointer, to: pointer, options: cint): pointer {.cdecl.} =
  let inst = cast[VirtualQStackedLayout](fcQStackedLayout_vdata(self)[])
  let slotval1 = gen_qwidget_types.QWidget(h: fromVal, owned: false)
  let slotval2 = gen_qwidget_types.QWidget(h: to, owned: false)
  let slotval3 = cint(options)
...
proc create*(T: type gen_qstackedlayout_types.QStackedLayout,
  ...
  if not isNil(vtbl[].spacing):
    vtbl[].vtbl.spacing = fcQStackedLayout_vtable_callback_spacing
  if not isNil(vtbl[].setSpacing):
    vtbl[].vtbl.setSpacing = fcQStackedLayout_vtable_callback_setSpacing
  ...
  if not isNil(vtbl[].replaceWidget):
    vtbl[].vtbl.replaceWidget = fcQStackedLayout_vtable_callback_replaceWidget
  ...
  let tmp = gen_qstackedlayout_types.QStackedLayout(h: fcQStackedLayout_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQStackedLayout_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstackedlayout_types.QStackedLayout,
  ...
  if not isNil(vtbl[].spacerItem):
    vtbl[].vtbl.spacerItem = fcQStackedLayout_vtable_callback_spacerItem
  let tmp = gen_qstackedlayout_types.QStackedLayout(h: fcQStackedLayout_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQStackedLayout_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstackedlayout_types.QStackedLayout,
  ...
  if not isNil(vtbl[].spacerItem):
    vtbl[].vtbl.spacerItem = fcQStackedLayout_vtable_callback_spacerItem
  let tmp = gen_qstackedlayout_types.QStackedLayout(h: fcQStackedLayout_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parentLayout.h), owned: true)
  fcQStackedLayout_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQStackedLayout_mvtbl = cQStackedLayoutVTable(
  ...
  spacing: fcQStackedLayout_method_callback_spacing,
  setSpacing: fcQStackedLayout_method_callback_setSpacing,
  ...
  replaceWidget: fcQStackedLayout_method_callback_replaceWidget,
...
proc create*(T: type gen_qstackedlayout_types.QStackedLayout,
  ...
  inst[].h = fcQStackedLayout_new(addr(cQStackedLayout_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qstackedlayout_types.QStackedLayout,
    ...
    inst: VirtualQStackedLayout) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQStackedLayout_new2(addr(cQStackedLayout_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQStackedLayout_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
