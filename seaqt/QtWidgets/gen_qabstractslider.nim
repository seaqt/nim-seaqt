...
proc fcQAbstractSlider_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QAbstractSlider_virtualbase_nativeEvent".}
...
proc fcQAbstractSlider_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QAbstractSlider_protectedbase_getDecodedMetricF".}
proc fcQAbstractSlider_new(vtbl: pointer, vdata: csize_t): ptr cQAbstractSlider {.importc: "QAbstractSlider_new".}
proc fcQAbstractSlider_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQAbstractSlider {.importc: "QAbstractSlider_new_parent".}
...
proc QAbstractSlidernativeEvent*(self: gen_qabstractslider_types.QAbstractSlider, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQAbstractSlider_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQAbstractSlider_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QAbstractSliderVTable](fcQAbstractSlider_vdata(self)[])
  let self = QAbstractSlider(h: self)
...
proc fcQAbstractSlider_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQAbstractSlider](fcQAbstractSlider_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qabstractslider_types.QAbstractSlider, metricA: cint, metricB: cint): float64 =
  fcQAbstractSlider_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qabstractslider_types.QAbstractSlider,
  ...
  let tmp = gen_qabstractslider_types.QAbstractSlider(h: fcQAbstractSlider_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qabstractslider_types.QAbstractSlider,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQAbstractSlider_vtable_callback_disconnectNotify
  let tmp = gen_qabstractslider_types.QAbstractSlider(h: fcQAbstractSlider_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQAbstractSlider_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qabstractslider_types.QAbstractSlider,
  ...
  inst[].h = fcQAbstractSlider_new(addr(cQAbstractSlider_mvtbl), csize_t(sizeof(pointer)))
  fcQAbstractSlider_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qabstractslider_types.QAbstractSlider,
    ...
    inst: VirtualQAbstractSlider) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQAbstractSlider_new2(addr(cQAbstractSlider_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQAbstractSlider_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
