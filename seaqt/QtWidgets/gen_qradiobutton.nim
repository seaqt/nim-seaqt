...
proc create*(T: type gen_qradiobutton_types.QRadioButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQRadioButton_vtable_callback_disconnectNotify
  let tmp = gen_qradiobutton_types.QRadioButton(h: fcQRadioButton_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))))
  fcQRadioButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qradiobutton_types.QRadioButton,
