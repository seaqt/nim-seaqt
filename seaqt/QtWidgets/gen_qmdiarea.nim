...
proc fcQMdiArea_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QMdiArea_virtualbase_nativeEvent".}
...
proc fcQMdiArea_new(vtbl: pointer, vdata: csize_t): ptr cQMdiArea {.importc: "QMdiArea_new".}
proc fcQMdiArea_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQMdiArea {.importc: "QMdiArea_new_parent".}
...
proc QMdiAreanativeEvent*(self: gen_qmdiarea_types.QMdiArea, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQMdiArea_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQMdiArea_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QMdiAreaVTable](fcQMdiArea_vdata(self)[])
  let self = QMdiArea(h: self)
...
proc fcQMdiArea_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQMdiArea](fcQMdiArea_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qmdiarea_types.QMdiArea,
  ...
  let tmp = gen_qmdiarea_types.QMdiArea(h: fcQMdiArea_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qmdiarea_types.QMdiArea,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMdiArea_vtable_callback_disconnectNotify
  let tmp = gen_qmdiarea_types.QMdiArea(h: fcQMdiArea_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQMdiArea_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qmdiarea_types.QMdiArea,
  ...
  inst[].h = fcQMdiArea_new(addr(cQMdiArea_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qmdiarea_types.QMdiArea,
    ...
    inst: VirtualQMdiArea) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQMdiArea_new2(addr(cQMdiArea_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQMdiArea_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
