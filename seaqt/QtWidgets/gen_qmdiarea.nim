...
proc fcQMdiArea_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QMdiArea_virtualbase_nativeEvent".}
...
proc fcQMdiArea_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QMdiArea_protectedbase_getDecodedMetricF".}
...
proc fcQMdiArea_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QMdiAreaVTable](fcQMdiArea_vdata(self)[])
  let self = QMdiArea(h: self)
...
proc fcQMdiArea_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQMdiArea](fcQMdiArea_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qmdiarea_types.QMdiArea, metricA: cint, metricB: cint): float64 =
  fcQMdiArea_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
