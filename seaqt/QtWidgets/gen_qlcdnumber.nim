...
proc cQLCDNumber_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QLCDNumberVTable](fcQLCDNumber_vdata(self))
  let self = QLCDNumber(h: self)
...
proc cQLCDNumber_vtable_callback_nativeEvent(self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QLCDNumberVTable](fcQLCDNumber_vdata(self))
  let self = QLCDNumber(h: self)
...
proc cQLCDNumber_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQLCDNumber](fcQLCDNumber_vdata(self))
  let slotval1 = gen_qstyleoption_types.QStyleOptionFrame(h: option, owned: false)
...
proc cQLCDNumber_method_callback_nativeEvent(self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQLCDNumber](fcQLCDNumber_vdata(self))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qlcdnumber_types.QLCDNumber,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = cQLCDNumber_vtable_callback_initStyleOption
...
const cQLCDNumber_mvtbl = cQLCDNumberVTable(
  ...
  initStyleOption: cQLCDNumber_method_callback_initStyleOption,
