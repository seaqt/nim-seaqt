...
proc fcQLCDNumber_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QLCDNumber_virtualbase_nativeEvent".}
...
proc fcQLCDNumber_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQLCDNumber {.importc: "QLCDNumber_new_parent".}
proc fcQLCDNumber_new2(vtbl: pointer, vdata: csize_t): ptr cQLCDNumber {.importc: "QLCDNumber_new".}
proc fcQLCDNumber_new3(vtbl: pointer, vdata: csize_t, numDigits: cuint): ptr cQLCDNumber {.importc: "QLCDNumber_new_numDigits".}
...
proc QLCDNumbernativeEvent*(self: gen_qlcdnumber_types.QLCDNumber, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQLCDNumber_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc create*(T: type gen_qlcdnumber_types.QLCDNumber,
  ...
  let tmp = gen_qlcdnumber_types.QLCDNumber(h: fcQLCDNumber_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qlcdnumber_types.QLCDNumber,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLCDNumber_vtable_callback_disconnectNotify
  let tmp = gen_qlcdnumber_types.QLCDNumber(h: fcQLCDNumber_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQLCDNumber_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlcdnumber_types.QLCDNumber,
    numDigits: cuint,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQLCDNumber_vtable_callback_disconnectNotify
  let tmp = gen_qlcdnumber_types.QLCDNumber(h: fcQLCDNumber_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), numDigits), owned: true)
  fcQLCDNumber_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qlcdnumber_types.QLCDNumber,
