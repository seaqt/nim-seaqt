...
proc fcQProgressDialog_labelText(self: pointer): struct_seaqt_string {.importc: "QProgressDialog_labelText".}
...
proc fcQProgressDialog_setLabelText(self: pointer, text: struct_seaqt_string): void {.importc: "QProgressDialog_setLabelText".}
proc fcQProgressDialog_setCancelButtonText(self: pointer, text: struct_seaqt_string): void {.importc: "QProgressDialog_setCancelButtonText".}
...
proc fcQProgressDialog_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QProgressDialog_virtualbase_nativeEvent".}
...
proc fcQProgressDialog_new(vtbl: pointer, vdata: csize_t): ptr cQProgressDialog {.importc: "QProgressDialog_new".}
proc fcQProgressDialog_new2(vtbl: pointer, vdata: csize_t, labelText: struct_seaqt_string, cancelButtonText: struct_seaqt_string, minimum: cint, maximum: cint): ptr cQProgressDialog {.importc: "QProgressDialog_new_labelText_cancelButtonText_minimum_maximum".}
proc fcQProgressDialog_new3(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQProgressDialog {.importc: "QProgressDialog_new_parent".}
...
proc setLabelText*(self: gen_qprogressdialog_types.QProgressDialog, text: openArray[char]): void =
  fcQProgressDialog_setLabelText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))

proc setCancelButtonText*(self: gen_qprogressdialog_types.QProgressDialog, text: openArray[char]): void =
  fcQProgressDialog_setCancelButtonText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
type QProgressDialognativeEventProc* = proc(self: QProgressDialog, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc QProgressDialognativeEvent*(self: gen_qprogressdialog_types.QProgressDialog, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQProgressDialog_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQProgressDialog_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QProgressDialogVTable](fcQProgressDialog_vdata(self)[])
  let self = QProgressDialog(h: self)
...
method enterEvent*(self: VirtualQProgressDialog, event: gen_qcoreevent_types.QEvent): void {.base.} =
...
method nativeEvent*(self: VirtualQProgressDialog, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
...
proc fcQProgressDialog_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQProgressDialog](fcQProgressDialog_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  let tmp = gen_qprogressdialog_types.QProgressDialog(h: fcQProgressDialog_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQProgressDialog_vtable_callback_disconnectNotify
  let tmp = gen_qprogressdialog_types.QProgressDialog(h: fcQProgressDialog_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(labelText) > 0: addr labelText[0] else: nil, len: csize_t(len(labelText))), struct_seaqt_string(data: if len(cancelButtonText) > 0: addr cancelButtonText[0] else: nil, len: csize_t(len(cancelButtonText))), minimum, maximum), owned: true)
  fcQProgressDialog_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQProgressDialog_vtable_callback_disconnectNotify
  let tmp = gen_qprogressdialog_types.QProgressDialog(h: fcQProgressDialog_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQProgressDialog_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQProgressDialog_vtable_callback_disconnectNotify
  let tmp = gen_qprogressdialog_types.QProgressDialog(h: fcQProgressDialog_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h, cint(flags)), owned: true)
  fcQProgressDialog_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQProgressDialog_vtable_callback_disconnectNotify
  let tmp = gen_qprogressdialog_types.QProgressDialog(h: fcQProgressDialog_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(labelText) > 0: addr labelText[0] else: nil, len: csize_t(len(labelText))), struct_seaqt_string(data: if len(cancelButtonText) > 0: addr cancelButtonText[0] else: nil, len: csize_t(len(cancelButtonText))), minimum, maximum, parent.h), owned: true)
  fcQProgressDialog_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
  ...
  inst[].h = fcQProgressDialog_new(addr(cQProgressDialog_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
    ...
    inst: VirtualQProgressDialog) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQProgressDialog_new2(addr(cQProgressDialog_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(labelText) > 0: addr labelText[0] else: nil, len: csize_t(len(labelText))), struct_seaqt_string(data: if len(cancelButtonText) > 0: addr cancelButtonText[0] else: nil, len: csize_t(len(cancelButtonText))), minimum, maximum)
  fcQProgressDialog_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
    ...
    inst: VirtualQProgressDialog) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQProgressDialog_new3(addr(cQProgressDialog_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQProgressDialog_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qprogressdialog_types.QProgressDialog,
    ...
    inst: VirtualQProgressDialog) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQProgressDialog_new5(addr(cQProgressDialog_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(labelText) > 0: addr labelText[0] else: nil, len: csize_t(len(labelText))), struct_seaqt_string(data: if len(cancelButtonText) > 0: addr cancelButtonText[0] else: nil, len: csize_t(len(cancelButtonText))), minimum, maximum, parent.h)
  fcQProgressDialog_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
