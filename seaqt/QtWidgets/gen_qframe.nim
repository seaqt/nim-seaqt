...
proc fcQFrame_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QFrame_virtualbase_nativeEvent".}
...
proc fcQFrame_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QFrame_protectedbase_getDecodedMetricF".}
...
proc fcQFrame_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QFrameVTable](fcQFrame_vdata(self)[])
  let self = QFrame(h: self)
...
proc fcQFrame_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQFrame](fcQFrame_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qframe_types.QFrame, metricA: cint, metricB: cint): float64 =
  fcQFrame_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qframe_types.QFrame,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQFrame_vtable_callback_initStyleOption
...
const cQFrame_mvtbl = cQFrameVTable(
  ...
  initStyleOption: fcQFrame_method_callback_initStyleOption,
