...
proc fcQTabWidget_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QTabWidget_virtualbase_nativeEvent".}
...
proc fcQTabWidget_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QTabWidget_protectedbase_getDecodedMetricF".}
...
proc fcQTabWidget_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QTabWidgetVTable](fcQTabWidget_vdata(self)[])
  let self = QTabWidget(h: self)
...
proc fcQTabWidget_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QTabWidgetVTable](fcQTabWidget_vdata(self)[])
  let self = QTabWidget(h: self)
...
proc fcQTabWidget_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQTabWidget](fcQTabWidget_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionTabWidgetFrame(h: option, owned: false)
...
proc fcQTabWidget_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQTabWidget](fcQTabWidget_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qtabwidget_types.QTabWidget, metricA: cint, metricB: cint): float64 =
  fcQTabWidget_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qtabwidget_types.QTabWidget,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQTabWidget_vtable_callback_initStyleOption
  ...
  let tmp = gen_qtabwidget_types.QTabWidget(h: fcQTabWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qtabwidget_types.QTabWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTabWidget_vtable_callback_disconnectNotify
  let tmp = gen_qtabwidget_types.QTabWidget(h: fcQTabWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQTabWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQTabWidget_mvtbl = cQTabWidgetVTable(
  ...
  initStyleOption: fcQTabWidget_method_callback_initStyleOption,
...
proc create*(T: type gen_qtabwidget_types.QTabWidget,
  ...
  inst[].h = fcQTabWidget_new(addr(cQTabWidget_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qtabwidget_types.QTabWidget,
    ...
    inst: VirtualQTabWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQTabWidget_new2(addr(cQTabWidget_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQTabWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
