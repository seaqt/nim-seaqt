...
proc fcQPushButton_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QPushButton_virtualbase_nativeEvent".}
...
proc fcQPushButton_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QPushButton_protectedbase_getDecodedMetricF".}
proc fcQPushButton_new(vtbl: pointer, vdata: csize_t): ptr cQPushButton {.importc: "QPushButton_new".}
proc fcQPushButton_new2(vtbl: pointer, vdata: csize_t, text: struct_seaqt_string): ptr cQPushButton {.importc: "QPushButton_new_text".}
proc fcQPushButton_new3(vtbl: pointer, vdata: csize_t, icon: pointer, text: struct_seaqt_string): ptr cQPushButton {.importc: "QPushButton_new_icon_text".}
proc fcQPushButton_new4(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQPushButton {.importc: "QPushButton_new_parent".}
...
proc QPushButtonnativeEvent*(self: gen_qpushbutton_types.QPushButton, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQPushButton_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQPushButton_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QPushButtonVTable](fcQPushButton_vdata(self)[])
  let self = QPushButton(h: self)
...
proc fcQPushButton_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQPushButton](fcQPushButton_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qpushbutton_types.QPushButton, metricA: cint, metricB: cint): float64 =
  fcQPushButton_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].metaObject):
    vtbl[].vtbl.metaObject = fcQPushButton_vtable_callback_metaObject
  if not isNil(vtbl[].metacast):
    vtbl[].vtbl.metacast = fcQPushButton_vtable_callback_metacast
  if not isNil(vtbl[].metacall):
    vtbl[].vtbl.metacall = fcQPushButton_vtable_callback_metacall
  if not isNil(vtbl[].sizeHint):
    vtbl[].vtbl.sizeHint = fcQPushButton_vtable_callback_sizeHint
  if not isNil(vtbl[].minimumSizeHint):
    vtbl[].vtbl.minimumSizeHint = fcQPushButton_vtable_callback_minimumSizeHint
  if not isNil(vtbl[].event):
    vtbl[].vtbl.event = fcQPushButton_vtable_callback_event
  if not isNil(vtbl[].paintEvent):
    vtbl[].vtbl.paintEvent = fcQPushButton_vtable_callback_paintEvent
  if not isNil(vtbl[].keyPressEvent):
    vtbl[].vtbl.keyPressEvent = fcQPushButton_vtable_callback_keyPressEvent
  if not isNil(vtbl[].focusInEvent):
    vtbl[].vtbl.focusInEvent = fcQPushButton_vtable_callback_focusInEvent
  if not isNil(vtbl[].focusOutEvent):
    vtbl[].vtbl.focusOutEvent = fcQPushButton_vtable_callback_focusOutEvent
  if not isNil(vtbl[].mouseMoveEvent):
    vtbl[].vtbl.mouseMoveEvent = fcQPushButton_vtable_callback_mouseMoveEvent
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQPushButton_vtable_callback_initStyleOption
  if not isNil(vtbl[].hitButton):
    vtbl[].vtbl.hitButton = fcQPushButton_vtable_callback_hitButton
  if not isNil(vtbl[].checkStateSet):
    vtbl[].vtbl.checkStateSet = fcQPushButton_vtable_callback_checkStateSet
  if not isNil(vtbl[].nextCheckState):
    vtbl[].vtbl.nextCheckState = fcQPushButton_vtable_callback_nextCheckState
  if not isNil(vtbl[].keyReleaseEvent):
    vtbl[].vtbl.keyReleaseEvent = fcQPushButton_vtable_callback_keyReleaseEvent
  if not isNil(vtbl[].mousePressEvent):
    vtbl[].vtbl.mousePressEvent = fcQPushButton_vtable_callback_mousePressEvent
  if not isNil(vtbl[].mouseReleaseEvent):
    vtbl[].vtbl.mouseReleaseEvent = fcQPushButton_vtable_callback_mouseReleaseEvent
  if not isNil(vtbl[].changeEvent):
    vtbl[].vtbl.changeEvent = fcQPushButton_vtable_callback_changeEvent
  if not isNil(vtbl[].timerEvent):
    vtbl[].vtbl.timerEvent = fcQPushButton_vtable_callback_timerEvent
  if not isNil(vtbl[].devType):
    vtbl[].vtbl.devType = fcQPushButton_vtable_callback_devType
  if not isNil(vtbl[].setVisible):
    vtbl[].vtbl.setVisible = fcQPushButton_vtable_callback_setVisible
  if not isNil(vtbl[].heightForWidth):
    vtbl[].vtbl.heightForWidth = fcQPushButton_vtable_callback_heightForWidth
  if not isNil(vtbl[].hasHeightForWidth):
    vtbl[].vtbl.hasHeightForWidth = fcQPushButton_vtable_callback_hasHeightForWidth
  if not isNil(vtbl[].paintEngine):
    vtbl[].vtbl.paintEngine = fcQPushButton_vtable_callback_paintEngine
  if not isNil(vtbl[].mouseDoubleClickEvent):
    vtbl[].vtbl.mouseDoubleClickEvent = fcQPushButton_vtable_callback_mouseDoubleClickEvent
  if not isNil(vtbl[].wheelEvent):
    vtbl[].vtbl.wheelEvent = fcQPushButton_vtable_callback_wheelEvent
  if not isNil(vtbl[].enterEvent):
    vtbl[].vtbl.enterEvent = fcQPushButton_vtable_callback_enterEvent
  if not isNil(vtbl[].leaveEvent):
    vtbl[].vtbl.leaveEvent = fcQPushButton_vtable_callback_leaveEvent
  if not isNil(vtbl[].moveEvent):
    vtbl[].vtbl.moveEvent = fcQPushButton_vtable_callback_moveEvent
  if not isNil(vtbl[].resizeEvent):
    vtbl[].vtbl.resizeEvent = fcQPushButton_vtable_callback_resizeEvent
  if not isNil(vtbl[].closeEvent):
    vtbl[].vtbl.closeEvent = fcQPushButton_vtable_callback_closeEvent
  if not isNil(vtbl[].contextMenuEvent):
    vtbl[].vtbl.contextMenuEvent = fcQPushButton_vtable_callback_contextMenuEvent
  if not isNil(vtbl[].tabletEvent):
    vtbl[].vtbl.tabletEvent = fcQPushButton_vtable_callback_tabletEvent
  if not isNil(vtbl[].actionEvent):
    vtbl[].vtbl.actionEvent = fcQPushButton_vtable_callback_actionEvent
  if not isNil(vtbl[].dragEnterEvent):
    vtbl[].vtbl.dragEnterEvent = fcQPushButton_vtable_callback_dragEnterEvent
  if not isNil(vtbl[].dragMoveEvent):
    vtbl[].vtbl.dragMoveEvent = fcQPushButton_vtable_callback_dragMoveEvent
  if not isNil(vtbl[].dragLeaveEvent):
    vtbl[].vtbl.dragLeaveEvent = fcQPushButton_vtable_callback_dragLeaveEvent
  if not isNil(vtbl[].dropEvent):
    vtbl[].vtbl.dropEvent = fcQPushButton_vtable_callback_dropEvent
  if not isNil(vtbl[].showEvent):
    vtbl[].vtbl.showEvent = fcQPushButton_vtable_callback_showEvent
  if not isNil(vtbl[].hideEvent):
    vtbl[].vtbl.hideEvent = fcQPushButton_vtable_callback_hideEvent
  if not isNil(vtbl[].nativeEvent):
    vtbl[].vtbl.nativeEvent = fcQPushButton_vtable_callback_nativeEvent
  if not isNil(vtbl[].metric):
    vtbl[].vtbl.metric = fcQPushButton_vtable_callback_metric
  if not isNil(vtbl[].initPainter):
    vtbl[].vtbl.initPainter = fcQPushButton_vtable_callback_initPainter
  if not isNil(vtbl[].redirected):
    vtbl[].vtbl.redirected = fcQPushButton_vtable_callback_redirected
  if not isNil(vtbl[].sharedPainter):
    vtbl[].vtbl.sharedPainter = fcQPushButton_vtable_callback_sharedPainter
  if not isNil(vtbl[].inputMethodEvent):
    vtbl[].vtbl.inputMethodEvent = fcQPushButton_vtable_callback_inputMethodEvent
  if not isNil(vtbl[].inputMethodQuery):
    vtbl[].vtbl.inputMethodQuery = fcQPushButton_vtable_callback_inputMethodQuery
  if not isNil(vtbl[].focusNextPrevChild):
    vtbl[].vtbl.focusNextPrevChild = fcQPushButton_vtable_callback_focusNextPrevChild
  if not isNil(vtbl[].eventFilter):
    vtbl[].vtbl.eventFilter = fcQPushButton_vtable_callback_eventFilter
  if not isNil(vtbl[].childEvent):
    vtbl[].vtbl.childEvent = fcQPushButton_vtable_callback_childEvent
  if not isNil(vtbl[].customEvent):
    vtbl[].vtbl.customEvent = fcQPushButton_vtable_callback_customEvent
  if not isNil(vtbl[].connectNotify):
    vtbl[].vtbl.connectNotify = fcQPushButton_vtable_callback_connectNotify
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQPushButton_mvtbl = cQPushButtonVTable(
  ...
  metaObject: fcQPushButton_method_callback_metaObject,
  metacast: fcQPushButton_method_callback_metacast,
  metacall: fcQPushButton_method_callback_metacall,
  sizeHint: fcQPushButton_method_callback_sizeHint,
  minimumSizeHint: fcQPushButton_method_callback_minimumSizeHint,
  event: fcQPushButton_method_callback_event,
  paintEvent: fcQPushButton_method_callback_paintEvent,
  keyPressEvent: fcQPushButton_method_callback_keyPressEvent,
  focusInEvent: fcQPushButton_method_callback_focusInEvent,
  focusOutEvent: fcQPushButton_method_callback_focusOutEvent,
  mouseMoveEvent: fcQPushButton_method_callback_mouseMoveEvent,
  initStyleOption: fcQPushButton_method_callback_initStyleOption,
  hitButton: fcQPushButton_method_callback_hitButton,
  checkStateSet: fcQPushButton_method_callback_checkStateSet,
  nextCheckState: fcQPushButton_method_callback_nextCheckState,
  keyReleaseEvent: fcQPushButton_method_callback_keyReleaseEvent,
  mousePressEvent: fcQPushButton_method_callback_mousePressEvent,
  mouseReleaseEvent: fcQPushButton_method_callback_mouseReleaseEvent,
  changeEvent: fcQPushButton_method_callback_changeEvent,
  timerEvent: fcQPushButton_method_callback_timerEvent,
  devType: fcQPushButton_method_callback_devType,
  setVisible: fcQPushButton_method_callback_setVisible,
  heightForWidth: fcQPushButton_method_callback_heightForWidth,
  hasHeightForWidth: fcQPushButton_method_callback_hasHeightForWidth,
  paintEngine: fcQPushButton_method_callback_paintEngine,
  mouseDoubleClickEvent: fcQPushButton_method_callback_mouseDoubleClickEvent,
  wheelEvent: fcQPushButton_method_callback_wheelEvent,
  enterEvent: fcQPushButton_method_callback_enterEvent,
  leaveEvent: fcQPushButton_method_callback_leaveEvent,
  moveEvent: fcQPushButton_method_callback_moveEvent,
  resizeEvent: fcQPushButton_method_callback_resizeEvent,
  closeEvent: fcQPushButton_method_callback_closeEvent,
  contextMenuEvent: fcQPushButton_method_callback_contextMenuEvent,
  tabletEvent: fcQPushButton_method_callback_tabletEvent,
  actionEvent: fcQPushButton_method_callback_actionEvent,
  dragEnterEvent: fcQPushButton_method_callback_dragEnterEvent,
  dragMoveEvent: fcQPushButton_method_callback_dragMoveEvent,
  dragLeaveEvent: fcQPushButton_method_callback_dragLeaveEvent,
  dropEvent: fcQPushButton_method_callback_dropEvent,
  showEvent: fcQPushButton_method_callback_showEvent,
  hideEvent: fcQPushButton_method_callback_hideEvent,
  nativeEvent: fcQPushButton_method_callback_nativeEvent,
  metric: fcQPushButton_method_callback_metric,
  initPainter: fcQPushButton_method_callback_initPainter,
  redirected: fcQPushButton_method_callback_redirected,
  sharedPainter: fcQPushButton_method_callback_sharedPainter,
  inputMethodEvent: fcQPushButton_method_callback_inputMethodEvent,
  inputMethodQuery: fcQPushButton_method_callback_inputMethodQuery,
  focusNextPrevChild: fcQPushButton_method_callback_focusNextPrevChild,
  eventFilter: fcQPushButton_method_callback_eventFilter,
  childEvent: fcQPushButton_method_callback_childEvent,
  customEvent: fcQPushButton_method_callback_customEvent,
  connectNotify: fcQPushButton_method_callback_connectNotify,
  disconnectNotify: fcQPushButton_method_callback_disconnectNotify,
)
proc create*(T: type gen_qpushbutton_types.QPushButton,
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new2(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new3(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new4(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new5(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h)
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new6(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h)
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
