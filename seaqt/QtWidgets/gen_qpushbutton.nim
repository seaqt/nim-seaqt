...
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
    text: openArray[char],
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQPushButton_vtable_callback_disconnectNotify
  let tmp = gen_qpushbutton_types.QPushButton(h: fcQPushButton_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQPushButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qpushbutton_types.QPushButton,
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new3(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qpushbutton_types.QPushButton,
    ...
    inst: VirtualQPushButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQPushButton_new4(addr(cQPushButton_mvtbl), csize_t(sizeof(pointer)), icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQPushButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
