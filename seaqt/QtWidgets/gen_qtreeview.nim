...
proc fcQTreeView_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QTreeView_trUtf8_s".}
...
proc fcQTreeView_sortByColumnColumn(self: pointer, column: cint): void {.importc: "QTreeView_sortByColumn_column".}
proc fcQTreeView_sortByColumnColumnOrder(self: pointer, column: cint, order: cint): void {.importc: "QTreeView_sortByColumn_column_order".}
...
proc fcQTreeView_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QTreeView_trUtf8_s_c".}
proc fcQTreeView_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QTreeView_trUtf8_s_c_n".}
...
proc fcQTreeView_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QTreeView_virtualbase_nativeEvent".}
...
proc fcQTreeView_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQTreeView {.importc: "QTreeView_new_parent".}
proc fcQTreeView_new2(vtbl: pointer, vdata: csize_t): ptr cQTreeView {.importc: "QTreeView_new".}
...
proc trUtf8*(_: type gen_qtreeview_types.QTreeView, s: cstring): string =
  let v_ms = fcQTreeView_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc sortByColumn*(self: gen_qtreeview_types.QTreeView, column: cint): void =
  fcQTreeView_sortByColumnColumn(self.h, column)

proc sortByColumn*(self: gen_qtreeview_types.QTreeView, column: cint, order: cint): void =
  fcQTreeView_sortByColumnColumnOrder(self.h, column, cint(order))
...
type QTreeViewnativeEventProc* = proc(self: QTreeView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc QTreeViewnativeEvent*(self: gen_qtreeview_types.QTreeView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
...
proc fcQTreeView_vtable_callback_viewOptions(self: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](fcQTreeView_vdata(self)[])
  let self = QTreeView(h: self)
...
proc fcQTreeView_vtable_callback_changeEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](fcQTreeView_vdata(self)[])
  let self = QTreeView(h: self)
...
proc fcQTreeView_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QTreeViewVTable](fcQTreeView_vdata(self)[])
  let self = QTreeView(h: self)
...
method nativeEvent*(self: VirtualQTreeView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
...
proc fcQTreeView_method_callback_viewOptions(self: pointer): pointer {.cdecl.} =
  let inst = cast[VirtualQTreeView](fcQTreeView_vdata(self)[])
...
proc fcQTreeView_method_callback_changeEvent(self: pointer, param1: pointer): void {.cdecl.} =
  let inst = cast[VirtualQTreeView](fcQTreeView_vdata(self)[])
  let slotval1 = gen_qcoreevent_types.QEvent(h: param1, owned: false)
...
proc fcQTreeView_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQTreeView](fcQTreeView_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  if not isNil(vtbl[].viewOptions):
    vtbl[].vtbl.viewOptions = fcQTreeView_vtable_callback_viewOptions
  ...
  let tmp = gen_qtreeview_types.QTreeView(h: fcQTreeView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTreeView_vtable_callback_disconnectNotify
  let tmp = gen_qtreeview_types.QTreeView(h: fcQTreeView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQTreeView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQTreeView_mvtbl = cQTreeViewVTable(
  ...
  viewOptions: fcQTreeView_method_callback_viewOptions,
...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  inst[].h = fcQTreeView_new(addr(cQTreeView_mvtbl), csize_t(sizeof(pointer)), parent.h)
...
proc create*(T: type gen_qtreeview_types.QTreeView,
    inst: VirtualQTreeView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQTreeView_new2(addr(cQTreeView_mvtbl), csize_t(sizeof(pointer)))
  fcQTreeView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
