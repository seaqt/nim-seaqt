...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  let tmp = gen_qtreeview_types.QTreeView(h: fcQTreeView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qtreeview_types.QTreeView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTreeView_vtable_callback_disconnectNotify
  let tmp = gen_qtreeview_types.QTreeView(h: fcQTreeView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQTreeView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
