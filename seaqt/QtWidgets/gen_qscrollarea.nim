...
proc create*(T: type gen_qscrollarea_types.QScrollArea,
  ...
  let tmp = gen_qscrollarea_types.QScrollArea(h: fcQScrollArea_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qscrollarea_types.QScrollArea,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQScrollArea_vtable_callback_disconnectNotify
  let tmp = gen_qscrollarea_types.QScrollArea(h: fcQScrollArea_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQScrollArea_vdata(tmp.h)[] = addr(vtbl[])
  tmp
