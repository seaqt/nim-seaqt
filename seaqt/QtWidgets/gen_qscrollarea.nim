...
proc fcQScrollArea_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QScrollArea_virtualbase_nativeEvent".}
...
proc fcQScrollArea_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QScrollArea_protectedbase_getDecodedMetricF".}
...
proc fcQScrollArea_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QScrollAreaVTable](fcQScrollArea_vdata(self)[])
  let self = QScrollArea(h: self)
...
proc fcQScrollArea_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QScrollAreaVTable](fcQScrollArea_vdata(self)[])
  let self = QScrollArea(h: self)
...
proc fcQScrollArea_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQScrollArea](fcQScrollArea_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionFrame(h: option, owned: false)
...
proc fcQScrollArea_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQScrollArea](fcQScrollArea_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qscrollarea_types.QScrollArea, metricA: cint, metricB: cint): float64 =
  fcQScrollArea_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qscrollarea_types.QScrollArea,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQScrollArea_vtable_callback_initStyleOption
...
const cQScrollArea_mvtbl = cQScrollAreaVTable(
  ...
  initStyleOption: fcQScrollArea_method_callback_initStyleOption,
