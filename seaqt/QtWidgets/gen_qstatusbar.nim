...
proc create*(T: type gen_qstatusbar_types.QStatusBar,
  ...
  let tmp = gen_qstatusbar_types.QStatusBar(h: fcQStatusBar_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qstatusbar_types.QStatusBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQStatusBar_vtable_callback_disconnectNotify
  let tmp = gen_qstatusbar_types.QStatusBar(h: fcQStatusBar_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQStatusBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
