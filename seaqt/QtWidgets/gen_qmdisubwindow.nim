...
proc fcQMdiSubWindow_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QMdiSubWindow_virtualbase_nativeEvent".}
...
proc fcQMdiSubWindow_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QMdiSubWindow_protectedbase_getDecodedMetricF".}
...
proc fcQMdiSubWindow_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QMdiSubWindowVTable](fcQMdiSubWindow_vdata(self)[])
  let self = QMdiSubWindow(h: self)
...
proc fcQMdiSubWindow_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQMdiSubWindow](fcQMdiSubWindow_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qmdisubwindow_types.QMdiSubWindow, metricA: cint, metricB: cint): float64 =
  fcQMdiSubWindow_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
