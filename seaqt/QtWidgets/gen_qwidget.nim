...
proc create*(T: type gen_qwidget_types.QWidget,
  ...
  let tmp = gen_qwidget_types.QWidget(h: fcQWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qwidget_types.QWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQWidget_vtable_callback_disconnectNotify
  let tmp = gen_qwidget_types.QWidget(h: fcQWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qwidget_types.QWidget,
