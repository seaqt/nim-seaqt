...
proc create*(T: type gen_qgroupbox_types.QGroupBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQGroupBox_vtable_callback_disconnectNotify
  let tmp = gen_qgroupbox_types.QGroupBox(h: fcQGroupBox_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title)))))
  fcQGroupBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qgroupbox_types.QGroupBox,
