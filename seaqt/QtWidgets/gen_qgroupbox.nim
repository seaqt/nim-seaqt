...
proc fcQGroupBox_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQGroupBox {.importc: "QGroupBox_new_parent".}
proc fcQGroupBox_new2(vtbl: pointer, vdata: csize_t): ptr cQGroupBox {.importc: "QGroupBox_new".}
proc fcQGroupBox_new3(vtbl: pointer, vdata: csize_t, title: struct_seaqt_string): ptr cQGroupBox {.importc: "QGroupBox_new_title".}
...
proc create*(T: type gen_qgroupbox_types.QGroupBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQGroupBox_vtable_callback_disconnectNotify
  let tmp = gen_qgroupbox_types.QGroupBox(h: fcQGroupBox_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title)))), owned: true)
  fcQGroupBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qgroupbox_types.QGroupBox,
...
proc create*(T: type gen_qgroupbox_types.QGroupBox,
    ...
    inst: VirtualQGroupBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQGroupBox_new3(addr(cQGroupBox_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))))
  fcQGroupBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
