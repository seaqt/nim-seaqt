...
proc fcQCalendarWidget_clearMinimumDate(self: pointer): void {.importc: "QCalendarWidget_clearMinimumDate".}
...
proc fcQCalendarWidget_clearMaximumDate(self: pointer): void {.importc: "QCalendarWidget_clearMaximumDate".}
...
proc fcQCalendarWidget_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QCalendarWidget_virtualbase_nativeEvent".}
...
proc fcQCalendarWidget_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QCalendarWidget_protectedbase_getDecodedMetricF".}
proc fcQCalendarWidget_new(vtbl: pointer, vdata: csize_t): ptr cQCalendarWidget {.importc: "QCalendarWidget_new".}
proc fcQCalendarWidget_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQCalendarWidget {.importc: "QCalendarWidget_new_parent".}
...
proc clearMinimumDate*(self: gen_qcalendarwidget_types.QCalendarWidget): void =
  fcQCalendarWidget_clearMinimumDate(self.h)
...
proc clearMaximumDate*(self: gen_qcalendarwidget_types.QCalendarWidget): void =
  fcQCalendarWidget_clearMaximumDate(self.h)
...
proc QCalendarWidgetnativeEvent*(self: gen_qcalendarwidget_types.QCalendarWidget, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQCalendarWidget_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQCalendarWidget_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QCalendarWidgetVTable](fcQCalendarWidget_vdata(self)[])
  let self = QCalendarWidget(h: self)
...
proc fcQCalendarWidget_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQCalendarWidget](fcQCalendarWidget_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qcalendarwidget_types.QCalendarWidget, metricA: cint, metricB: cint): float64 =
  fcQCalendarWidget_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qcalendarwidget_types.QCalendarWidget,
  ...
  let tmp = gen_qcalendarwidget_types.QCalendarWidget(h: fcQCalendarWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQCalendarWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcalendarwidget_types.QCalendarWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCalendarWidget_vtable_callback_disconnectNotify
  let tmp = gen_qcalendarwidget_types.QCalendarWidget(h: fcQCalendarWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQCalendarWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qcalendarwidget_types.QCalendarWidget,
  ...
  inst[].h = fcQCalendarWidget_new(addr(cQCalendarWidget_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qcalendarwidget_types.QCalendarWidget,
    ...
    inst: VirtualQCalendarWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCalendarWidget_new2(addr(cQCalendarWidget_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQCalendarWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
