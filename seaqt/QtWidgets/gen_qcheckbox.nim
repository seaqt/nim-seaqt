...
proc fcQCheckBox_checkStateChanged(self: pointer, param1: cint): void {.importc: "QCheckBox_checkStateChanged".}
proc fcQCheckBox_connect_checkStateChanged(self: pointer, slot: int, callback: proc (slot: int, param1: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QCheckBox_connect_checkStateChanged".}
...
proc fcQCheckBox_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QCheckBox_virtualbase_nativeEvent".}
...
proc fcQCheckBox_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QCheckBox_protectedbase_getDecodedMetricF".}
proc fcQCheckBox_new(vtbl: pointer, vdata: csize_t): ptr cQCheckBox {.importc: "QCheckBox_new".}
proc fcQCheckBox_new2(vtbl: pointer, vdata: csize_t, text: struct_seaqt_string): ptr cQCheckBox {.importc: "QCheckBox_new_text".}
proc fcQCheckBox_new3(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQCheckBox {.importc: "QCheckBox_new_parent".}
...
proc checkStateChanged*(self: gen_qcheckbox_types.QCheckBox, param1: cint): void =
  fcQCheckBox_checkStateChanged(self.h, cint(param1))

type QCheckBoxcheckStateChangedSlot* = proc(param1: cint)
proc fcQCheckBox_slot_callback_checkStateChanged(slot: int, param1: cint) {.cdecl.} =
  let nimfunc = cast[ptr QCheckBoxcheckStateChangedSlot](cast[pointer](slot))
  let slotval1 = cint(param1)

  nimfunc[](slotval1)

proc fcQCheckBox_slot_callback_checkStateChanged_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QCheckBoxcheckStateChangedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onCheckStateChanged*(self: gen_qcheckbox_types.QCheckBox, slot: QCheckBoxcheckStateChangedSlot) =
  var tmp = new QCheckBoxcheckStateChangedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQCheckBox_connect_checkStateChanged(self.h, cast[int](addr tmp[]), fcQCheckBox_slot_callback_checkStateChanged, fcQCheckBox_slot_callback_checkStateChanged_release)
...
proc QCheckBoxnativeEvent*(self: gen_qcheckbox_types.QCheckBox, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQCheckBox_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc getDecodedMetricF*(self: gen_qcheckbox_types.QCheckBox, metricA: cint, metricB: cint): float64 =
  fcQCheckBox_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
  ...
  let tmp = gen_qcheckbox_types.QCheckBox(h: fcQCheckBox_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCheckBox_vtable_callback_disconnectNotify
  let tmp = gen_qcheckbox_types.QCheckBox(h: fcQCheckBox_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQCheckBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcheckbox_types.QCheckBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCheckBox_vtable_callback_disconnectNotify
  let tmp = gen_qcheckbox_types.QCheckBox(h: fcQCheckBox_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQCheckBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcheckbox_types.QCheckBox,
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
  ...
  inst[].h = fcQCheckBox_new(addr(cQCheckBox_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
    ...
    inst: VirtualQCheckBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCheckBox_new2(addr(cQCheckBox_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQCheckBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
    ...
    inst: VirtualQCheckBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCheckBox_new3(addr(cQCheckBox_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQCheckBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcheckbox_types.QCheckBox,
    ...
    inst: VirtualQCheckBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCheckBox_new4(addr(cQCheckBox_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h)
  fcQCheckBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
