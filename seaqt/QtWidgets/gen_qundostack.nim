...
proc fcQUndoStack_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QUndoStack_trUtf8_s".}
...
proc fcQUndoStack_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QUndoStack_trUtf8_s_c".}
proc fcQUndoStack_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QUndoStack_trUtf8_s_c_n".}
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h), owned: true)
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc trUtf8*(_: type gen_qundostack_types.QUndoStack, s: cstring): string =
  let v_ms = fcQUndoStack_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qundostack_types.QUndoStack,
  ...
  let tmp = gen_qundostack_types.QUndoStack(h: fcQUndoStack_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qundostack_types.QUndoStack,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQUndoStack_vtable_callback_disconnectNotify
  let tmp = gen_qundostack_types.QUndoStack(h: fcQUndoStack_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQUndoStack_vdata(tmp.h)[] = addr(vtbl[])
  tmp
