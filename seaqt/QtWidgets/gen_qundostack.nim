...
proc fcQUndoCommand_text(self: pointer): struct_seaqt_string {.importc: "QUndoCommand_text".}
proc fcQUndoCommand_actionText(self: pointer): struct_seaqt_string {.importc: "QUndoCommand_actionText".}
proc fcQUndoCommand_setText(self: pointer, text: struct_seaqt_string): void {.importc: "QUndoCommand_setText".}
...
proc fcQUndoStack_trUtf8(s: cstring): struct_seaqt_string {.importc: "QUndoStack_trUtf8".}
...
proc fcQUndoStack_undoText(self: pointer): struct_seaqt_string {.importc: "QUndoStack_undoText".}
proc fcQUndoStack_redoText(self: pointer): struct_seaqt_string {.importc: "QUndoStack_redoText".}
...
proc fcQUndoStack_beginMacro(self: pointer, text: struct_seaqt_string): void {.importc: "QUndoStack_beginMacro".}
...
proc fcQUndoStack_undoTextChanged(self: pointer, undoText: struct_seaqt_string): void {.importc: "QUndoStack_undoTextChanged".}
...
proc fcQUndoStack_trUtf82(s: cstring, c: cstring): struct_seaqt_string {.importc: "QUndoStack_trUtf82".}
proc fcQUndoStack_trUtf83(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QUndoStack_trUtf83".}
...
proc setText*(self: gen_qundostack_types.QUndoCommand, text: openArray[char]): void =
  fcQUndoCommand_setText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoCommand,
    text: openArray[char],
    vtbl: ref QUndoCommandVTable = nil): gen_qundostack_types.QUndoCommand =
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))))
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  if not isNil(vtbl[].mergeWith):
    vtbl[].vtbl.mergeWith = fcQUndoCommand_vtable_callback_mergeWith
  let tmp = gen_qundostack_types.QUndoCommand(h: fcQUndoCommand_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h))
  fcQUndoCommand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
  ...
  inst[].h = fcQUndoCommand_new(addr(cQUndoCommand_mvtbl), csize_t(sizeof(pointer)))
  fcQUndoCommand_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
    ...
    inst: VirtualQUndoCommand) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQUndoCommand_new2(addr(cQUndoCommand_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQUndoCommand_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
    ...
    inst: VirtualQUndoCommand) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQUndoCommand_new3(addr(cQUndoCommand_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQUndoCommand_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qundostack_types.QUndoCommand,
    ...
    inst: VirtualQUndoCommand) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQUndoCommand_new4(addr(cQUndoCommand_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), parent.h)
  fcQUndoCommand_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc beginMacro*(self: gen_qundostack_types.QUndoStack, text: openArray[char]): void =
  fcQUndoStack_beginMacro(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc undoTextChanged*(self: gen_qundostack_types.QUndoStack, undoText: openArray[char]): void =
  fcQUndoStack_undoTextChanged(self.h, struct_seaqt_string(data: if len(undoText) > 0: addr undoText[0] else: nil, len: csize_t(len(undoText))))

type QUndoStackundoTextChangedSlot* = proc(undoText: openArray[char])
proc fcQUndoStack_slot_callback_undoTextChanged(slot: int, undoText: struct_seaqt_string) {.cdecl.} =
...
proc redoTextChanged*(self: gen_qundostack_types.QUndoStack, redoText: openArray[char]): void =
  fcQUndoStack_redoTextChanged(self.h, struct_seaqt_string(data: if len(redoText) > 0: addr redoText[0] else: nil, len: csize_t(len(redoText))))

type QUndoStackredoTextChangedSlot* = proc(redoText: openArray[char])
proc fcQUndoStack_slot_callback_redoTextChanged(slot: int, redoText: struct_seaqt_string) {.cdecl.} =
...
proc createUndoAction*(self: gen_qundostack_types.QUndoStack, parent: gen_qobject_types.QObject, prefix: openArray[char]): gen_qaction_types.QAction =
  gen_qaction_types.QAction(h: fcQUndoStack_createUndoAction2(self.h, parent.h, struct_seaqt_string(data: if len(prefix) > 0: addr prefix[0] else: nil, len: csize_t(len(prefix)))))
...
proc createRedoAction*(self: gen_qundostack_types.QUndoStack, parent: gen_qobject_types.QObject, prefix: openArray[char]): gen_qaction_types.QAction =
  gen_qaction_types.QAction(h: fcQUndoStack_createRedoAction2(self.h, parent.h, struct_seaqt_string(data: if len(prefix) > 0: addr prefix[0] else: nil, len: csize_t(len(prefix)))))
...
proc create*(T: type gen_qundostack_types.QUndoStack,
  ...
  let tmp = gen_qundostack_types.QUndoStack(h: fcQUndoStack_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
  fcQUndoStack_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qundostack_types.QUndoStack,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQUndoStack_vtable_callback_disconnectNotify
  let tmp = gen_qundostack_types.QUndoStack(h: fcQUndoStack_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQUndoStack_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qundostack_types.QUndoStack,
  ...
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQUndoStack_new(addr(cQUndoStack_mvtbl), csize_t(sizeof(pointer)))
  fcQUndoStack_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qundostack_types.QUndoStack,
    ...
    inst: VirtualQUndoStack) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQUndoStack_new2(addr(cQUndoStack_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQUndoStack_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
