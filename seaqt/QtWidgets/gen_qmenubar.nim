...
proc fcQMenuBar_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QMenuBar_virtualbase_nativeEvent".}
...
proc addAction*(self: gen_qmenubar_types.QMenuBar, text: openArray[char]): gen_qaction_types.QAction =
  gen_qaction_types.QAction(h: fcQMenuBar_addAction(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))))
...
proc fcQMenuBar_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QMenuBarVTable](fcQMenuBar_vdata(self)[])
  let self = QMenuBar(h: self)
...
proc fcQMenuBar_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQMenuBar](fcQMenuBar_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
