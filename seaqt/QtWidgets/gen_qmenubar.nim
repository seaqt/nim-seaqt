...
proc cQMenuBar_vtable_callback_initStyleOption(self: pointer, option: pointer, action: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QMenuBarVTable](fcQMenuBar_vdata(self))
  let self = QMenuBar(h: self)
...
proc cQMenuBar_vtable_callback_nativeEvent(self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QMenuBarVTable](fcQMenuBar_vdata(self))
  let self = QMenuBar(h: self)
...
proc cQMenuBar_method_callback_initStyleOption(self: pointer, option: pointer, action: pointer): void {.cdecl.} =
  let inst = cast[VirtualQMenuBar](fcQMenuBar_vdata(self))
  let slotval1 = gen_qstyleoption_types.QStyleOptionMenuItem(h: option, owned: false)
  let slotval2 = gen_qaction_types.QAction(h: action, owned: false)
...
proc cQMenuBar_method_callback_nativeEvent(self: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQMenuBar](fcQMenuBar_vdata(self))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qmenubar_types.QMenuBar,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = cQMenuBar_vtable_callback_initStyleOption
...
const cQMenuBar_mvtbl = cQMenuBarVTable(
  ...
  initStyleOption: cQMenuBar_method_callback_initStyleOption,
