...
proc miqt_exec_method_cQMenuBar_initStyleOption(vtbl: pointer, inst: pointer, option: pointer, action: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQMenuBar](cast[uint](vtbl) - uint(offsetOf(VirtualQMenuBar, vtbl)))
  let slotval1 = gen_qstyleoption_types.QStyleOptionMenuItem(h: option, owned: false)
  let slotval2 = gen_qaction_types.QAction(h: action, owned: false)
...
proc create*(T: type gen_qmenubar_types.QMenuBar,
  ...
  vtbl[].vtbl.initStyleOption = miqt_exec_method_cQMenuBar_initStyleOption
