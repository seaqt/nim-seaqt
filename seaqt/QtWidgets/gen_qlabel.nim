...
proc miqt_exec_method_cQLabel_initStyleOption(vtbl: pointer, inst: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQLabel](cast[uint](vtbl) - uint(offsetOf(VirtualQLabel, vtbl)))
  let slotval1 = gen_qstyleoption_types.QStyleOptionFrame(h: option, owned: false)
...
proc miqt_exec_method_cQLabel_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[VirtualQLabel](cast[uint](vtbl) - uint(offsetOf(VirtualQLabel, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qlabel_types.QLabel,
  ...
  vtbl[].vtbl.initStyleOption = miqt_exec_method_cQLabel_initStyleOption
