...
proc fcQCompleter_currentCompletion(self: pointer): struct_seaqt_string {.importc: "QCompleter_currentCompletion".}
proc fcQCompleter_completionModel(self: pointer): pointer {.importc: "QCompleter_completionModel".}
proc fcQCompleter_completionPrefix(self: pointer): struct_seaqt_string {.importc: "QCompleter_completionPrefix".}
proc fcQCompleter_setCompletionPrefix(self: pointer, prefix: struct_seaqt_string): void {.importc: "QCompleter_setCompletionPrefix".}
...
type cQCompleterVTable {.pure.} = object
  ...
  pathFromIndex*: proc(self: pointer, index: pointer): struct_seaqt_string {.cdecl, raises: [], gcsafe.}
  splitPath*: proc(self: pointer, path: struct_seaqt_string): struct_seaqt_array {.cdecl, raises: [], gcsafe.}
...
proc fcQCompleter_virtualbase_pathFromIndex(self: pointer, index: pointer): struct_seaqt_string {.importc: "QCompleter_virtualbase_pathFromIndex".}
proc fcQCompleter_virtualbase_splitPath(self: pointer, path: struct_seaqt_string): struct_seaqt_array {.importc: "QCompleter_virtualbase_splitPath".}
...
proc setCompletionPrefix*(self: gen_qcompleter_types.QCompleter, prefix: openArray[char]): void =
  fcQCompleter_setCompletionPrefix(self.h, struct_seaqt_string(data: if len(prefix) > 0: addr prefix[0] else: nil, len: csize_t(len(prefix))))
...
proc splitPath*(self: gen_qcompleter_types.QCompleter, path: openArray[char]): seq[string] =
  var v_ma = fcQCompleter_splitPath(self.h, struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
...
proc QCompletersplitPath*(self: gen_qcompleter_types.QCompleter, path: openArray[char]): seq[string] =
  var v_ma = fcQCompleter_virtualbase_splitPath(self.h, struct_seaqt_string(data: if len(path) > 0: addr path[0] else: nil, len: csize_t(len(path))))
  var vx_ret = newSeq[string](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[struct_seaqt_string]](v_ma.data)
  for i in 0 ..< v_ma.len:
    let vx_lv_ms = v_outCast[i]
...
proc fcQCompleter_vtable_callback_pathFromIndex(self: pointer, index: pointer): struct_seaqt_string {.cdecl.} =
  let vtbl = cast[ptr QCompleterVTable](fcQCompleter_vdata(self)[])
  let self = QCompleter(h: self)
  let slotval1 = gen_qabstractitemmodel_types.QModelIndex(h: index, owned: false)
...
proc fcQCompleter_vtable_callback_splitPath(self: pointer, path: struct_seaqt_string): struct_seaqt_array {.cdecl.} =
  let vtbl = cast[ptr QCompleterVTable](fcQCompleter_vdata(self)[])
  let self = QCompleter(h: self)
  let vpath_ms = path
...
proc fcQCompleter_method_callback_pathFromIndex(self: pointer, index: pointer): struct_seaqt_string {.cdecl.} =
  let inst = cast[VirtualQCompleter](fcQCompleter_vdata(self)[])
  let slotval1 = gen_qabstractitemmodel_types.QModelIndex(h: index, owned: false)
...
proc fcQCompleter_method_callback_splitPath(self: pointer, path: struct_seaqt_string): struct_seaqt_array {.cdecl.} =
  let inst = cast[VirtualQCompleter](fcQCompleter_vdata(self)[])
  let vpath_ms = path
  let vpathx_ret = string.fromBytes(vpath_ms)
  c_free(vpath_ms.data)
...
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCompleter_vtable_callback_disconnectNotify
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), model.h), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcompleter_types.QCompleter,
    completions: openArray[string],
    vtbl: ref QCompleterVTable = nil): gen_qcompleter_types.QCompleter =
  var completions_CArray = newSeq[struct_seaqt_string](len(completions))
  for i in 0..<len(completions):
    completions_CArray[i] = struct_seaqt_string(data: if len(completions[i]) > 0: addr completions[i][0] else: nil, len: csize_t(len(completions[i])))
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCompleter_vtable_callback_disconnectNotify
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(completions)), data: if len(completions) == 0: nil else: addr(completions_CArray[0]))), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCompleter_vtable_callback_disconnectNotify
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCompleter_vtable_callback_disconnectNotify
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), model.h, parent.h), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCompleter_vtable_callback_disconnectNotify
  let tmp = gen_qcompleter_types.QCompleter(h: fcQCompleter_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(completions)), data: if len(completions) == 0: nil else: addr(completions_CArray[0])), parent.h), owned: true)
  fcQCompleter_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  inst[].h = fcQCompleter_new(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)))
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcompleter_types.QCompleter,
    ...
    inst: VirtualQCompleter) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCompleter_new2(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)), model.h)
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCompleter_new3(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(completions)), data: if len(completions) == 0: nil else: addr(completions_CArray[0])))
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcompleter_types.QCompleter,
    ...
    inst: VirtualQCompleter) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCompleter_new4(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcompleter_types.QCompleter,
    ...
    inst: VirtualQCompleter) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCompleter_new5(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)), model.h, parent.h)
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcompleter_types.QCompleter,
  ...
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCompleter_new6(addr(cQCompleter_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_array(len: csize_t(len(completions)), data: if len(completions) == 0: nil else: addr(completions_CArray[0])), parent.h)
  fcQCompleter_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
