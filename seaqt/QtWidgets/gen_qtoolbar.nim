...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBar_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title)))))
  fcQToolBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbar_types.QToolBar,
    vtbl: ref QToolBarVTable = nil): gen_qtoolbar_types.QToolBar =
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBar_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))), parent.h))
  fcQToolBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
