...
proc create*(T: type gen_qtoolbar_types.QToolBar,
    ...
    inst: VirtualQToolBar) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBar_new2(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))))
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
    ...
    inst: VirtualQToolBar) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBar_new4(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))), parent.h)
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
