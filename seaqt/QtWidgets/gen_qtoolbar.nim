...
proc fcQToolBar_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QToolBar_trUtf8_s".}
...
proc fcQToolBar_addActionText(self: pointer, text: struct_seaqt_string): pointer {.importc: "QToolBar_addAction_text".}
proc fcQToolBar_addActionIconText(self: pointer, icon: pointer, text: struct_seaqt_string): pointer {.importc: "QToolBar_addAction_icon_text".}
...
proc fcQToolBar_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QToolBar_trUtf8_s_c".}
proc fcQToolBar_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QToolBar_trUtf8_s_c_n".}
...
proc fcQToolBar_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QToolBar_virtualbase_nativeEvent".}
...
proc fcQToolBar_new(vtbl: pointer, vdata: csize_t, title: struct_seaqt_string): ptr cQToolBar {.importc: "QToolBar_new_title".}
proc fcQToolBar_new2(vtbl: pointer, vdata: csize_t): ptr cQToolBar {.importc: "QToolBar_new".}
proc fcQToolBar_new3(vtbl: pointer, vdata: csize_t, title: struct_seaqt_string, parent: pointer): ptr cQToolBar {.importc: "QToolBar_new_title_parent".}
proc fcQToolBar_new4(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQToolBar {.importc: "QToolBar_new_parent".}
...
proc addAction*(self: gen_qtoolbar_types.QToolBar, text: openArray[char]): gen_qaction_types.QAction =
  gen_qaction_types.QAction(h: fcQToolBar_addActionText(self.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: false)

proc addAction*(self: gen_qtoolbar_types.QToolBar, icon: gen_qicon_types.QIcon, text: openArray[char]): gen_qaction_types.QAction =
  gen_qaction_types.QAction(h: fcQToolBar_addActionIconText(self.h, icon.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: false)
...
proc QToolBarnativeEvent*(self: gen_qtoolbar_types.QToolBar, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQToolBar_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title)))), owned: true)
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBar_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQToolBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBar_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))), parent.h), owned: true)
  fcQToolBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBar_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbar_types.QToolBar(h: fcQToolBar_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQToolBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  inst[].h = fcQToolBar_new(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))))
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true

proc create*(T: type gen_qtoolbar_types.QToolBar,
    inst: VirtualQToolBar) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBar_new2(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)))
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
    ...
    inst: VirtualQToolBar) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBar_new3(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title))), parent.h)
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
    ...
    inst: VirtualQToolBar) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBar_new4(addr(cQToolBar_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQToolBar_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
