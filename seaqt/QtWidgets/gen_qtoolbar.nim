...
proc fcQToolBar_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QToolBar_virtualbase_nativeEvent".}
...
proc fcQToolBar_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QToolBar_protectedbase_getDecodedMetricF".}
...
proc fcQToolBar_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QToolBarVTable](fcQToolBar_vdata(self)[])
  let self = QToolBar(h: self)
...
proc fcQToolBar_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QToolBarVTable](fcQToolBar_vdata(self)[])
  let self = QToolBar(h: self)
...
proc fcQToolBar_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQToolBar](fcQToolBar_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionToolBar(h: option, owned: false)
...
proc fcQToolBar_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQToolBar](fcQToolBar_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qtoolbar_types.QToolBar, metricA: cint, metricB: cint): float64 =
  fcQToolBar_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qtoolbar_types.QToolBar,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQToolBar_vtable_callback_initStyleOption
...
const cQToolBar_mvtbl = cQToolBarVTable(
  ...
  initStyleOption: fcQToolBar_method_callback_initStyleOption,
