...
proc find*(self: gen_qplaintextedit_types.QPlainTextEdit, exp: openArray[char], options: cint): bool =
  fcQPlainTextEdit_find22(self.h, struct_miqt_string(data: if len(exp) > 0: addr exp[0] else: nil, len: csize_t(len(exp))), cint(options))
...
type QPlainTextEditnativeEventProc* = proc(self: QPlainTextEdit, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc QPlainTextEditnativeEvent*(self: gen_qplaintextedit_types.QPlainTextEdit, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
...
method nativeEvent*(self: VirtualQPlainTextEdit, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
