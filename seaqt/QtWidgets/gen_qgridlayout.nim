...
proc fcQGridLayout_new(vtbl: pointer, vdata: csize_t): ptr cQGridLayout {.importc: "QGridLayout_new".}
proc fcQGridLayout_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQGridLayout {.importc: "QGridLayout_new_parent".}
...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
  ...
  let tmp = gen_qgridlayout_types.QGridLayout(h: fcQGridLayout_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
  ...
  if not isNil(vtbl[].spacerItem):
    vtbl[].vtbl.spacerItem = fcQGridLayout_vtable_callback_spacerItem
  let tmp = gen_qgridlayout_types.QGridLayout(h: fcQGridLayout_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQGridLayout_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
  ...
  inst[].h = fcQGridLayout_new(addr(cQGridLayout_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
    ...
    inst: VirtualQGridLayout) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQGridLayout_new2(addr(cQGridLayout_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQGridLayout_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
