...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
  ...
  let tmp = gen_qgridlayout_types.QGridLayout(h: fcQGridLayout_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qgridlayout_types.QGridLayout,
  ...
  if not isNil(vtbl[].spacerItem):
    vtbl[].vtbl.spacerItem = fcQGridLayout_vtable_callback_spacerItem
  let tmp = gen_qgridlayout_types.QGridLayout(h: fcQGridLayout_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQGridLayout_vdata(tmp.h)[] = addr(vtbl[])
  tmp
