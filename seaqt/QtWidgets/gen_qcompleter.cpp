...
VirtualQCompleter* QCompleter_new(const QCompleter_VTable* vtbl, size_t vdata) {
	void* _mem_ = malloc(seaqt_aligned_sizeof<VirtualQCompleter>());
...
VirtualQCompleter* QCompleter_new3(const QCompleter_VTable* vtbl, size_t vdata, struct miqt_array /* of struct miqt_string */  completions) {
...
VirtualQCompleter* QCompleter_new6(const QCompleter_VTable* vtbl, size_t vdata, struct miqt_array /* of struct miqt_string */  completions, QObject* parent) {
	QStringList completions_QList;
	completions_QList.reserve(completions.len);
	struct miqt_string* completions_arr = static_cast<struct miqt_string*>(completions.data);
	for(size_t i = 0; i < completions.len; ++i) {
