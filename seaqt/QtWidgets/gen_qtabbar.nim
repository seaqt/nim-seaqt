...
proc create*(T: type gen_qtabbar_types.QTabBar,
  ...
  let tmp = gen_qtabbar_types.QTabBar(h: fcQTabBar_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qtabbar_types.QTabBar,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTabBar_vtable_callback_disconnectNotify
  let tmp = gen_qtabbar_types.QTabBar(h: fcQTabBar_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQTabBar_vdata(tmp.h)[] = addr(vtbl[])
  tmp
