...
proc create*(T: type gen_qsplashscreen_types.QSplashScreen,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQSplashScreen_vtable_callback_disconnectNotify
  let tmp = gen_qsplashscreen_types.QSplashScreen(h: fcQSplashScreen_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQSplashScreen_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qsplashscreen_types.QSplashScreen,
...
proc create*(T: type gen_qsplashscreen_types.QSplashScreen,
    ...
    inst: VirtualQSplashScreen) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSplashScreen_new3(addr(cQSplashScreen_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQSplashScreen_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
