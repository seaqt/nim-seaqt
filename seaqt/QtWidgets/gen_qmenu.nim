...
proc fcQMenu_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QMenu_virtualbase_nativeEvent".}
...
proc fcQMenu_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QMenuVTable](fcQMenu_vdata(self)[])
  let self = QMenu(h: self)
...
proc fcQMenu_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQMenu](fcQMenu_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qmenu_types.QMenu,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQMenu_vtable_callback_initStyleOption
...
const cQMenu_mvtbl = cQMenuVTable(
  ...
  initStyleOption: fcQMenu_method_callback_initStyleOption,
