...
proc fcQMenu_new(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQMenu {.importc: "QMenu_new_parent".}
proc fcQMenu_new2(vtbl: pointer, vdata: csize_t): ptr cQMenu {.importc: "QMenu_new".}
proc fcQMenu_new3(vtbl: pointer, vdata: csize_t, title: struct_seaqt_string): ptr cQMenu {.importc: "QMenu_new_title".}
...
proc create*(T: type gen_qmenu_types.QMenu,
  ...
  let tmp = gen_qmenu_types.QMenu(h: fcQMenu_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
...
proc create*(T: type gen_qmenu_types.QMenu,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMenu_vtable_callback_disconnectNotify
  let tmp = gen_qmenu_types.QMenu(h: fcQMenu_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQMenu_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qmenu_types.QMenu,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQMenu_vtable_callback_disconnectNotify
  let tmp = gen_qmenu_types.QMenu(h: fcQMenu_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(title) > 0: addr title[0] else: nil, len: csize_t(len(title)))), owned: true)
  fcQMenu_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qmenu_types.QMenu,
