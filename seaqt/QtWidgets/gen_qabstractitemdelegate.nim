...
proc fcQAbstractItemDelegate_elidedText(fontMetrics: pointer, width: cint, mode: cint, text: struct_seaqt_string): struct_seaqt_string {.importc: "QAbstractItemDelegate_elidedText".}
...
proc elidedText*(_: type gen_qabstractitemdelegate_types.QAbstractItemDelegate, fontMetrics: gen_qfontmetrics_types.QFontMetrics, width: cint, mode: cint, text: openArray[char]): string =
  let v_ms = fcQAbstractItemDelegate_elidedText(fontMetrics.h, width, cint(mode), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
