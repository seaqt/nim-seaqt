...
proc fcQStackedWidget_widgetAdded(self: pointer, index: cint): void {.importc: "QStackedWidget_widgetAdded".}
proc fcQStackedWidget_connect_widgetAdded(self: pointer, slot: int, callback: proc (slot: int, index: cint) {.cdecl.}, release: proc(slot: int) {.cdecl.}) {.importc: "QStackedWidget_connect_widgetAdded".}
...
proc fcQStackedWidget_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QStackedWidget_virtualbase_nativeEvent".}
...
proc fcQStackedWidget_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QStackedWidget_protectedbase_getDecodedMetricF".}
proc fcQStackedWidget_new(vtbl: pointer, vdata: csize_t): ptr cQStackedWidget {.importc: "QStackedWidget_new".}
proc fcQStackedWidget_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQStackedWidget {.importc: "QStackedWidget_new_parent".}
...
proc widgetAdded*(self: gen_qstackedwidget_types.QStackedWidget, index: cint): void =
  fcQStackedWidget_widgetAdded(self.h, index)

type QStackedWidgetwidgetAddedSlot* = proc(index: cint)
proc fcQStackedWidget_slot_callback_widgetAdded(slot: int, index: cint) {.cdecl.} =
  let nimfunc = cast[ptr QStackedWidgetwidgetAddedSlot](cast[pointer](slot))
  let slotval1 = index

  nimfunc[](slotval1)

proc fcQStackedWidget_slot_callback_widgetAdded_release(slot: int) {.cdecl.} =
  let nimfunc = cast[ref QStackedWidgetwidgetAddedSlot](cast[pointer](slot))
  GC_unref(nimfunc)

proc onWidgetAdded*(self: gen_qstackedwidget_types.QStackedWidget, slot: QStackedWidgetwidgetAddedSlot) =
  var tmp = new QStackedWidgetwidgetAddedSlot
  tmp[] = slot
  GC_ref(tmp)
  fcQStackedWidget_connect_widgetAdded(self.h, cast[int](addr tmp[]), fcQStackedWidget_slot_callback_widgetAdded, fcQStackedWidget_slot_callback_widgetAdded_release)
...
proc QStackedWidgetnativeEvent*(self: gen_qstackedwidget_types.QStackedWidget, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQStackedWidget_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQStackedWidget_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QStackedWidgetVTable](fcQStackedWidget_vdata(self)[])
  let self = QStackedWidget(h: self)
...
proc fcQStackedWidget_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQStackedWidget](fcQStackedWidget_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qstackedwidget_types.QStackedWidget, metricA: cint, metricB: cint): float64 =
  fcQStackedWidget_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qstackedwidget_types.QStackedWidget,
  ...
  let tmp = gen_qstackedwidget_types.QStackedWidget(h: fcQStackedWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQStackedWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qstackedwidget_types.QStackedWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQStackedWidget_vtable_callback_disconnectNotify
  let tmp = gen_qstackedwidget_types.QStackedWidget(h: fcQStackedWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQStackedWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qstackedwidget_types.QStackedWidget,
  ...
  inst[].h = fcQStackedWidget_new(addr(cQStackedWidget_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qstackedwidget_types.QStackedWidget,
    ...
    inst: VirtualQStackedWidget) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQStackedWidget_new2(addr(cQStackedWidget_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQStackedWidget_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
