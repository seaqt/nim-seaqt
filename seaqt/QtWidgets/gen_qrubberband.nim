...
proc fcQRubberBand_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QRubberBand_virtualbase_nativeEvent".}
...
proc fcQRubberBand_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QRubberBand_protectedbase_getDecodedMetricF".}
...
proc QRubberBandnativeEvent*(self: gen_qrubberband_types.QRubberBand, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQRubberBand_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQRubberBand_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QRubberBandVTable](fcQRubberBand_vdata(self)[])
  let self = QRubberBand(h: self)
...
proc fcQRubberBand_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QRubberBandVTable](fcQRubberBand_vdata(self)[])
  let self = QRubberBand(h: self)
...
proc fcQRubberBand_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQRubberBand](fcQRubberBand_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionRubberBand(h: option, owned: false)
...
proc fcQRubberBand_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQRubberBand](fcQRubberBand_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qrubberband_types.QRubberBand, metricA: cint, metricB: cint): float64 =
  fcQRubberBand_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qrubberband_types.QRubberBand,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQRubberBand_vtable_callback_initStyleOption
  ...
  let tmp = gen_qrubberband_types.QRubberBand(h: fcQRubberBand_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(param1)), owned: true)
  fcQRubberBand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qrubberband_types.QRubberBand,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQRubberBand_vtable_callback_disconnectNotify
  let tmp = gen_qrubberband_types.QRubberBand(h: fcQRubberBand_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), cint(param1), param2.h), owned: true)
  fcQRubberBand_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQRubberBand_mvtbl = cQRubberBandVTable(
  ...
  initStyleOption: fcQRubberBand_method_callback_initStyleOption,
