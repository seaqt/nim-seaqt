...
proc fcQRubberBand_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QRubberBand_trUtf8_s".}
...
proc fcQRubberBand_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QRubberBand_trUtf8_s_c".}
proc fcQRubberBand_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QRubberBand_trUtf8_s_c_n".}
...
proc fcQRubberBand_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QRubberBand_virtualbase_nativeEvent".}
...
proc trUtf8*(_: type gen_qrubberband_types.QRubberBand, s: cstring): string =
  let v_ms = fcQRubberBand_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc fcQRubberBand_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QRubberBandVTable](fcQRubberBand_vdata(self)[])
  let self = QRubberBand(h: self)
...
proc fcQRubberBand_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQRubberBand](fcQRubberBand_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
