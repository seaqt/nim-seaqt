...
proc miqt_exec_method_cQRubberBand_initStyleOption(vtbl: pointer, inst: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQRubberBand](cast[uint](vtbl) - uint(offsetOf(VirtualQRubberBand, vtbl)))
  let slotval1 = gen_qstyleoption_types.QStyleOptionRubberBand(h: option, owned: false)
...
proc miqt_exec_method_cQRubberBand_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[VirtualQRubberBand](cast[uint](vtbl) - uint(offsetOf(VirtualQRubberBand, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qrubberband_types.QRubberBand,
  ...
  vtbl[].vtbl.initStyleOption = miqt_exec_method_cQRubberBand_initStyleOption
