...
proc fcQColumnView_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QColumnView_trUtf8_s".}
...
proc fcQColumnView_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QColumnView_trUtf8_s_c".}
proc fcQColumnView_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QColumnView_trUtf8_s_c_n".}
...
proc fcQColumnView_virtualbase_keyboardSearch(self: pointer, search: struct_seaqt_string): void {.importc: "QColumnView_virtualbase_keyboardSearch".}
...
proc fcQColumnView_virtualbase_dataChanged(self: pointer, topLeft: pointer, bottomRight: pointer, roles: struct_seaqt_array): void {.importc: "QColumnView_virtualbase_dataChanged".}
...
proc fcQColumnView_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.importc: "QColumnView_virtualbase_nativeEvent".}
...
proc fcQColumnView_new(vtbl: pointer, vdata: csize_t): ptr cQColumnView {.importc: "QColumnView_new".}
proc fcQColumnView_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQColumnView {.importc: "QColumnView_new_parent".}
...
proc trUtf8*(_: type gen_qcolumnview_types.QColumnView, s: cstring): string =
  let v_ms = fcQColumnView_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc setColumnWidths*(self: gen_qcolumnview_types.QColumnView, list: openArray[cint]): void =
  ...
  fcQColumnView_setColumnWidths(self.h, struct_seaqt_array(len: csize_t(len(list)), data: if len(list) == 0: nil else: addr(list_CArray[0])))
...
type QColumnViewnativeEventProc* = proc(self: QColumnView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.raises: [], gcsafe.}
...
proc QColumnViewkeyboardSearch*(self: gen_qcolumnview_types.QColumnView, search: openArray[char]): void =
  fcQColumnView_virtualbase_keyboardSearch(self.h, struct_seaqt_string(data: if len(search) > 0: addr search[0] else: nil, len: csize_t(len(search))))
...
proc QColumnViewdataChanged*(self: gen_qcolumnview_types.QColumnView, topLeft: gen_qabstractitemmodel_types.QModelIndex, bottomRight: gen_qabstractitemmodel_types.QModelIndex, roles: openArray[cint]): void =
  ...
  fcQColumnView_virtualbase_dataChanged(self.h, topLeft.h, bottomRight.h, struct_seaqt_array(len: csize_t(len(roles)), data: if len(roles) == 0: nil else: addr(roles_CArray[0])))
...
proc QColumnViewnativeEvent*(self: gen_qcolumnview_types.QColumnView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool =
  fcQColumnView_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQColumnView_vtable_callback_keyboardSearch(self: pointer, search: struct_seaqt_string): void {.cdecl.} =
  let vtbl = cast[ptr QColumnViewVTable](fcQColumnView_vdata(self)[])
  let self = QColumnView(h: self)
...
proc fcQColumnView_vtable_callback_dataChanged(self: pointer, topLeft: pointer, bottomRight: pointer, roles: struct_seaqt_array): void {.cdecl.} =
  let vtbl = cast[ptr QColumnViewVTable](fcQColumnView_vdata(self)[])
  let self = QColumnView(h: self)
...
proc fcQColumnView_vtable_callback_selectedIndexes(self: pointer): struct_seaqt_array {.cdecl.} =
  let vtbl = cast[ptr QColumnViewVTable](fcQColumnView_vdata(self)[])
  let self = QColumnView(h: self)
...
proc fcQColumnView_vtable_callback_viewOptions(self: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QColumnViewVTable](fcQColumnView_vdata(self)[])
  let self = QColumnView(h: self)
...
proc fcQColumnView_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[ptr QColumnViewVTable](fcQColumnView_vdata(self)[])
  let self = QColumnView(h: self)
...
method nativeEvent*(self: VirtualQColumnView, eventType: openArray[byte], message: pointer, resultVal: ptr clong): bool {.base.} =
...
proc fcQColumnView_method_callback_keyboardSearch(self: pointer, search: struct_seaqt_string): void {.cdecl.} =
  let inst = cast[VirtualQColumnView](fcQColumnView_vdata(self)[])
  let vsearch_ms = search
  let vsearchx_ret = string.fromBytes(vsearch_ms)
  c_free(vsearch_ms.data)
...
proc fcQColumnView_method_callback_dataChanged(self: pointer, topLeft: pointer, bottomRight: pointer, roles: struct_seaqt_array): void {.cdecl.} =
  let inst = cast[VirtualQColumnView](fcQColumnView_vdata(self)[])
  let slotval1 = gen_qabstractitemmodel_types.QModelIndex(h: topLeft, owned: false)
  let slotval2 = gen_qabstractitemmodel_types.QModelIndex(h: bottomRight, owned: false)
  var vroles_ma = roles
...
proc fcQColumnView_method_callback_selectedIndexes(self: pointer): struct_seaqt_array {.cdecl.} =
  let inst = cast[VirtualQColumnView](fcQColumnView_vdata(self)[])
...
proc fcQColumnView_method_callback_viewOptions(self: pointer): pointer {.cdecl.} =
  let inst = cast[VirtualQColumnView](fcQColumnView_vdata(self)[])
...
proc fcQColumnView_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let inst = cast[VirtualQColumnView](fcQColumnView_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qcolumnview_types.QColumnView,
  ...
  if not isNil(vtbl[].viewOptions):
    vtbl[].vtbl.viewOptions = fcQColumnView_vtable_callback_viewOptions
  ...
  let tmp = gen_qcolumnview_types.QColumnView(h: fcQColumnView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQColumnView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcolumnview_types.QColumnView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQColumnView_vtable_callback_disconnectNotify
  let tmp = gen_qcolumnview_types.QColumnView(h: fcQColumnView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQColumnView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQColumnView_mvtbl = cQColumnViewVTable(
  ...
  viewOptions: fcQColumnView_method_callback_viewOptions,
...
proc create*(T: type gen_qcolumnview_types.QColumnView,
  ...
  inst[].h = fcQColumnView_new(addr(cQColumnView_mvtbl), csize_t(sizeof(pointer)))
  fcQColumnView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcolumnview_types.QColumnView,
    ...
    inst: VirtualQColumnView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQColumnView_new2(addr(cQColumnView_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQColumnView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
