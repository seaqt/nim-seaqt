...
proc create*(T: type gen_qcolumnview_types.QColumnView,
  ...
  let tmp = gen_qcolumnview_types.QColumnView(h: fcQColumnView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qcolumnview_types.QColumnView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQColumnView_vtable_callback_disconnectNotify
  let tmp = gen_qcolumnview_types.QColumnView(h: fcQColumnView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQColumnView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
