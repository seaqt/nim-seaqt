...
proc create*(T: type gen_qaccessiblewidget_types.QAccessibleWidget,
  ...
  let tmp = gen_qaccessiblewidget_types.QAccessibleWidget(h: fcQAccessibleWidget_new3(o.h, cint(r), struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))))
  tmp
