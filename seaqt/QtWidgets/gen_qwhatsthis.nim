...
proc fcQWhatsThis_showText(pos: pointer, text: struct_seaqt_string): void {.importc: "QWhatsThis_showText".}
...
proc showText*(_: type gen_qwhatsthis_types.QWhatsThis, pos: gen_qpoint_types.QPoint, text: openArray[char]): void =
  fcQWhatsThis_showText(pos.h, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
