...
proc fcQErrorMessage_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QErrorMessage_virtualbase_nativeEvent".}
...
proc fcQErrorMessage_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QErrorMessage_protectedbase_getDecodedMetricF".}
proc fcQErrorMessage_new(vtbl: pointer, vdata: csize_t): ptr cQErrorMessage {.importc: "QErrorMessage_new".}
proc fcQErrorMessage_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQErrorMessage {.importc: "QErrorMessage_new_parent".}
...
proc QErrorMessagenativeEvent*(self: gen_qerrormessage_types.QErrorMessage, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQErrorMessage_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQErrorMessage_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QErrorMessageVTable](fcQErrorMessage_vdata(self)[])
  let self = QErrorMessage(h: self)
...
proc fcQErrorMessage_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQErrorMessage](fcQErrorMessage_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qerrormessage_types.QErrorMessage, metricA: cint, metricB: cint): float64 =
  fcQErrorMessage_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qerrormessage_types.QErrorMessage,
  ...
  let tmp = gen_qerrormessage_types.QErrorMessage(h: fcQErrorMessage_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQErrorMessage_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qerrormessage_types.QErrorMessage,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQErrorMessage_vtable_callback_disconnectNotify
  let tmp = gen_qerrormessage_types.QErrorMessage(h: fcQErrorMessage_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQErrorMessage_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qerrormessage_types.QErrorMessage,
  ...
  inst[].h = fcQErrorMessage_new(addr(cQErrorMessage_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qerrormessage_types.QErrorMessage,
    ...
    inst: VirtualQErrorMessage) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQErrorMessage_new2(addr(cQErrorMessage_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQErrorMessage_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
