...
proc create*(T: type gen_qtreewidget_types.QTreeWidget,
  ...
  let tmp = gen_qtreewidget_types.QTreeWidget(h: fcQTreeWidget_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))))
...
proc create*(T: type gen_qtreewidget_types.QTreeWidget,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQTreeWidget_vtable_callback_disconnectNotify
  let tmp = gen_qtreewidget_types.QTreeWidget(h: fcQTreeWidget_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h))
  fcQTreeWidget_vdata(tmp.h)[] = addr(vtbl[])
  tmp
