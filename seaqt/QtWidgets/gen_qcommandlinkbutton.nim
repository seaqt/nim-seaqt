...
proc create*(T: type gen_qcommandlinkbutton_types.QCommandLinkButton,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQCommandLinkButton_vtable_callback_disconnectNotify
  let tmp = gen_qcommandlinkbutton_types.QCommandLinkButton(h: fcQCommandLinkButton_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))), owned: true)
  fcQCommandLinkButton_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qcommandlinkbutton_types.QCommandLinkButton,
...
proc create*(T: type gen_qcommandlinkbutton_types.QCommandLinkButton,
    ...
    inst: VirtualQCommandLinkButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCommandLinkButton_new3(addr(cQCommandLinkButton_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
  fcQCommandLinkButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qcommandlinkbutton_types.QCommandLinkButton,
    ...
    inst: VirtualQCommandLinkButton) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQCommandLinkButton_new4(addr(cQCommandLinkButton_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))), struct_seaqt_string(data: if len(description) > 0: addr description[0] else: nil, len: csize_t(len(description))))
  fcQCommandLinkButton_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
