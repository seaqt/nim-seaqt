...
proc miqt_exec_method_cQCommandLinkButton_mouseMoveEvent(vtbl: pointer, inst: pointer, e: pointer): void {.cdecl.} =
  let vtbl = cast[VirtualQCommandLinkButton](cast[uint](vtbl) - uint(offsetOf(VirtualQCommandLinkButton, vtbl)))
  let slotval1 = gen_qevent_types.QMouseEvent(h: e, owned: false)
...
proc miqt_exec_method_cQCommandLinkButton_nativeEvent(vtbl: pointer, inst: pointer, eventType: struct_miqt_string, message: pointer, resultVal: ptr clong): bool {.cdecl.} =
  let vtbl = cast[VirtualQCommandLinkButton](cast[uint](vtbl) - uint(offsetOf(VirtualQCommandLinkButton, vtbl)))
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArrayByte(veventType_bytearray.data, 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
