...
proc fcQToolBox_setItemText(self: pointer, index: cint, text: struct_seaqt_string): void {.importc: "QToolBox_setItemText".}
proc fcQToolBox_itemText(self: pointer, index: cint): struct_seaqt_string {.importc: "QToolBox_itemText".}
...
proc fcQToolBox_setItemToolTip(self: pointer, index: cint, toolTip: struct_seaqt_string): void {.importc: "QToolBox_setItemToolTip".}
proc fcQToolBox_itemToolTip(self: pointer, index: cint): struct_seaqt_string {.importc: "QToolBox_itemToolTip".}
...
proc fcQToolBox_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QToolBox_virtualbase_nativeEvent".}
...
proc fcQToolBox_protectedbase_getDecodedMetricF(self: pointer, metricA: cint, metricB: cint): float64 {.importc: "QToolBox_protectedbase_getDecodedMetricF".}
proc fcQToolBox_new(vtbl: pointer, vdata: csize_t): ptr cQToolBox {.importc: "QToolBox_new".}
proc fcQToolBox_new2(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQToolBox {.importc: "QToolBox_new_parent".}
...
proc setItemText*(self: gen_qtoolbox_types.QToolBox, index: cint, text: openArray[char]): void =
  fcQToolBox_setItemText(self.h, index, struct_seaqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text))))
...
proc setItemToolTip*(self: gen_qtoolbox_types.QToolBox, index: cint, toolTip: openArray[char]): void =
  fcQToolBox_setItemToolTip(self.h, index, struct_seaqt_string(data: if len(toolTip) > 0: addr toolTip[0] else: nil, len: csize_t(len(toolTip))))
...
proc QToolBoxnativeEvent*(self: gen_qtoolbox_types.QToolBox, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQToolBox_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQToolBox_vtable_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let vtbl = cast[ptr QToolBoxVTable](fcQToolBox_vdata(self)[])
  let self = QToolBox(h: self)
...
proc fcQToolBox_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QToolBoxVTable](fcQToolBox_vdata(self)[])
  let self = QToolBox(h: self)
...
proc fcQToolBox_method_callback_initStyleOption(self: pointer, option: pointer): void {.cdecl.} =
  let inst = cast[VirtualQToolBox](fcQToolBox_vdata(self)[])
  let slotval1 = gen_qstyleoption_types.QStyleOptionFrame(h: option, owned: false)
...
proc fcQToolBox_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQToolBox](fcQToolBox_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc getDecodedMetricF*(self: gen_qtoolbox_types.QToolBox, metricA: cint, metricB: cint): float64 =
  fcQToolBox_protectedbase_getDecodedMetricF(self.h, cint(metricA), cint(metricB))
...
proc create*(T: type gen_qtoolbox_types.QToolBox,
  ...
  if not isNil(vtbl[].initStyleOption):
    vtbl[].vtbl.initStyleOption = fcQToolBox_vtable_callback_initStyleOption
  ...
  let tmp = gen_qtoolbox_types.QToolBox(h: fcQToolBox_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQToolBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbox_types.QToolBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBox_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbox_types.QToolBox(h: fcQToolBox_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQToolBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qtoolbox_types.QToolBox,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQToolBox_vtable_callback_disconnectNotify
  let tmp = gen_qtoolbox_types.QToolBox(h: fcQToolBox_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h, cint(f)), owned: true)
  fcQToolBox_vdata(tmp.h)[] = addr(vtbl[])
  tmp
const cQToolBox_mvtbl = cQToolBoxVTable(
  ...
  initStyleOption: fcQToolBox_method_callback_initStyleOption,
...
proc create*(T: type gen_qtoolbox_types.QToolBox,
  ...
  inst[].h = fcQToolBox_new(addr(cQToolBox_mvtbl), csize_t(sizeof(pointer)))
...
proc create*(T: type gen_qtoolbox_types.QToolBox,
    ...
    inst: VirtualQToolBox) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQToolBox_new2(addr(cQToolBox_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQToolBox_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
