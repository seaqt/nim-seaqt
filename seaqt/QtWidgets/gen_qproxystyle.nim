...
proc fcQProxyStyle_new2(vtbl: pointer, vdata: csize_t, key: struct_miqt_string): ptr cQProxyStyle {.importc: "QProxyStyle_new2".}
...
proc itemTextRect*(self: gen_qproxystyle_types.QProxyStyle, fm: gen_qfontmetrics_types.QFontMetrics, r: gen_qrect_types.QRect, flags: cint, enabled: bool, text: string): gen_qrect_types.QRect =
  gen_qrect_types.QRect(h: fcQProxyStyle_itemTextRect(self.h, fm.h, r.h, flags, enabled, struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))))
...
proc QProxyStyleitemTextRect*(self: gen_qproxystyle_types.QProxyStyle, fm: gen_qfontmetrics_types.QFontMetrics, r: gen_qrect_types.QRect, flags: cint, enabled: bool, text: string): gen_qrect_types.QRect =
  gen_qrect_types.QRect(h: fcQProxyStyle_virtualbase_itemTextRect(self.h, fm.h, r.h, flags, enabled, struct_miqt_string(data: if len(text) > 0: addr text[0] else: nil, len: csize_t(len(text)))))
...
proc create*(T: type gen_qproxystyle_types.QProxyStyle,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQProxyStyle_vtable_callback_disconnectNotify
  let tmp = gen_qproxystyle_types.QProxyStyle(h: fcQProxyStyle_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))))
  fcQProxyStyle_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qproxystyle_types.QProxyStyle,
...
proc create*(T: type gen_qproxystyle_types.QProxyStyle,
    ...
    inst: VirtualQProxyStyle) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQProxyStyle_new2(addr(cQProxyStyle_mvtbl), csize_t(sizeof(pointer)), struct_miqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
  fcQProxyStyle_vdata(inst[].h)[] = addr inst[]
