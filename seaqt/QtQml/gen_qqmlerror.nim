...
proc fcQQmlError_description(self: pointer): struct_seaqt_string {.importc: "QQmlError_description".}
proc fcQQmlError_setDescription(self: pointer, description: struct_seaqt_string): void {.importc: "QQmlError_setDescription".}
...
proc setDescription*(self: gen_qqmlerror_types.QQmlError, description: openArray[char]): void =
  fcQQmlError_setDescription(self.h, struct_seaqt_string(data: if len(description) > 0: addr description[0] else: nil, len: csize_t(len(description))))
