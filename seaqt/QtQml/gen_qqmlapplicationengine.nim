...
proc fcQQmlApplicationEngine_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QQmlApplicationEngine_trUtf8_s".}
proc fcQQmlApplicationEngine_rootObjects(self: pointer): struct_seaqt_array {.importc: "QQmlApplicationEngine_rootObjects".}
proc fcQQmlApplicationEngine_rootObjectsConst(self: pointer): struct_seaqt_array {.importc: "QQmlApplicationEngine_rootObjects_const".}
...
proc fcQQmlApplicationEngine_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QQmlApplicationEngine_trUtf8_s_c".}
proc fcQQmlApplicationEngine_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QQmlApplicationEngine_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qqmlapplicationengine_types.QQmlApplicationEngine, s: cstring): string =
  let v_ms = fcQQmlApplicationEngine_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc rootObjects2*(self: gen_qqmlapplicationengine_types.QQmlApplicationEngine): seq[gen_qobject_types.QObject] =
  var v_ma = fcQQmlApplicationEngine_rootObjectsConst(self.h)
  var vx_ret = newSeq[gen_qobject_types.QObject](int(v_ma.len))
  let v_outCast = cast[ptr UncheckedArray[pointer]](v_ma.data)
  for i in 0 ..< v_ma.len:
    vx_ret[i] = gen_qobject_types.QObject(h: v_outCast[i], owned: false)
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath)))), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), url.h, parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))), parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new3(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))))
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new4(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new5(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), url.h, parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new6(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))), parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
