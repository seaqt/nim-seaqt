...
{.compile("gen_qqmlapplicationengine.cpp", QtQmlCFlags).}
...
proc fcQQmlApplicationEngine_loadFromModule(self: pointer, uri: struct_seaqt_string, typeName: struct_seaqt_string): void {.importc: "QQmlApplicationEngine_loadFromModule".}
...
proc fcQQmlApplicationEngine_new3(vtbl: pointer, vdata: csize_t, uri: struct_seaqt_string, typeName: struct_seaqt_string): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_uri_typeName".}
proc fcQQmlApplicationEngine_new4(vtbl: pointer, vdata: csize_t, filePath: struct_seaqt_string): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_filePath".}
proc fcQQmlApplicationEngine_new5(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_parent".}
proc fcQQmlApplicationEngine_new6(vtbl: pointer, vdata: csize_t, url: pointer, parent: pointer): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_url_parent".}
proc fcQQmlApplicationEngine_new7(vtbl: pointer, vdata: csize_t, uri: struct_seaqt_string, typeName: struct_seaqt_string, parent: pointer): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_uri_typeName_parent".}
proc fcQQmlApplicationEngine_new8(vtbl: pointer, vdata: csize_t, filePath: struct_seaqt_string, parent: pointer): ptr cQQmlApplicationEngine {.importc: "QQmlApplicationEngine_new_filePath_parent".}
...
proc loadFromModule*(self: gen_qqmlapplicationengine_types.QQmlApplicationEngine, uri: openArray[char], typeName: openArray[char]): void =
  fcQQmlApplicationEngine_loadFromModule(self.h, struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))))
...
proc setInitialProperties*(self: gen_qqmlapplicationengine_types.QQmlApplicationEngine, initialProperties: Table[string,gen_qvariant_types.QVariant]): void =
  ...
  fcQQmlApplicationEngine_setInitialProperties(self.h, struct_seaqt_map(len: csize_t(len(initialProperties)),keys: if len(initialProperties) == 0: nil else: addr(initialProperties_Keys_CArray[0]), values: if len(initialProperties) == 0: nil else: addr(initialProperties_Values_CArray[0]),))

proc setExtraFileSelectors*(self: gen_qqmlapplicationengine_types.QQmlApplicationEngine, extraFileSelectors: openArray[string]): void =
  var extraFileSelectors_CArray = newSeq[struct_seaqt_string](len(extraFileSelectors))
  for i in 0..<len(extraFileSelectors):
    extraFileSelectors_CArray[i] = struct_seaqt_string(data: if len(extraFileSelectors[i]) > 0: addr extraFileSelectors[i][0] else: nil, len: csize_t(len(extraFileSelectors[i])))
...
proc fcQQmlApplicationEngine_slot_callback_objectCreationFailed(slot: int, url: pointer) {.cdecl.} =
...
proc fcQQmlApplicationEngine_slot_callback_objectCreationFailed_release(slot: int) {.cdecl.} =
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), url.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    uri: openArray[char], typeName: openArray[char],
    vtbl: ref QQmlApplicationEngineVTable = nil): gen_qqmlapplicationengine_types.QQmlApplicationEngine =
  let vtbl = if vtbl == nil: new QQmlApplicationEngineVTable else: vtbl
  GC_ref(vtbl)
  vtbl[].vtbl.destructor = proc(self: pointer) {.cdecl.} =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName)))), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath)))), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), url.h, parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    uri: openArray[char], typeName: openArray[char], parent: gen_qobject_types.QObject,
    vtbl: ref QQmlApplicationEngineVTable = nil): gen_qqmlapplicationengine_types.QQmlApplicationEngine =
  let vtbl = if vtbl == nil: new QQmlApplicationEngineVTable else: vtbl
  GC_ref(vtbl)
  vtbl[].vtbl.destructor = proc(self: pointer) {.cdecl.} =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))), parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlApplicationEngine_vtable_callback_disconnectNotify
  let tmp = gen_qqmlapplicationengine_types.QQmlApplicationEngine(h: fcQQmlApplicationEngine_new8(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))), parent.h), owned: true)
  fcQQmlApplicationEngine_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
  ...
  inst[].h = fcQQmlApplicationEngine_new(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)))
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new2(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), url.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new3(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))))
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new4(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))))
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new5(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new6(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), url.h, parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new7(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))), parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qqmlapplicationengine_types.QQmlApplicationEngine,
    ...
    inst: VirtualQQmlApplicationEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQmlApplicationEngine_new8(addr(cQQmlApplicationEngine_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(filePath) > 0: addr filePath[0] else: nil, len: csize_t(len(filePath))), parent.h)
  fcQQmlApplicationEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
