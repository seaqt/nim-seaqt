...
proc fcQQmlDebuggingEnabler_debuggerServices(): struct_seaqt_array {.importc: "QQmlDebuggingEnabler_debuggerServices".}
proc fcQQmlDebuggingEnabler_inspectorServices(): struct_seaqt_array {.importc: "QQmlDebuggingEnabler_inspectorServices".}
...
proc setServices*(_: type gen_qqmldebug_types.QQmlDebuggingEnabler, services: openArray[string]): void =
  var services_CArray = newSeq[struct_seaqt_string](len(services))
  for i in 0..<len(services):
    services_CArray[i] = struct_seaqt_string(data: if len(services[i]) > 0: addr services[i][0] else: nil, len: csize_t(len(services[i])))
...
proc startDebugConnector*(_: type gen_qqmldebug_types.QQmlDebuggingEnabler, pluginName: openArray[char], configuration: Table[string,gen_qvariant_types.QVariant]): bool =
  ...
  for configuration_k in configuration.keys():
    configuration_Keys_CArray[configuration_ctr] = struct_seaqt_string(data: if len(configuration_k) > 0: addr configuration_k[0] else: nil, len: csize_t(len(configuration_k)))
    configuration_ctr += 1
  configuration_ctr = 0
