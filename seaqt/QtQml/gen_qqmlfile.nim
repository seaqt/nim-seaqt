...
proc fcQQmlFile_error(self: pointer): struct_seaqt_string {.importc: "QQmlFile_error".}
...
proc fcQQmlFile_clearObject(self: pointer, objectVal: pointer): void {.importc: "QQmlFile_clear_object".}
...
proc fcQQmlFile_new2(engine: pointer, url: pointer): ptr cQQmlFile {.importc: "QQmlFile_new_QQmlEngine_QUrl".}
proc fcQQmlFile_new3(engine: pointer, url: struct_seaqt_string): ptr cQQmlFile {.importc: "QQmlFile_new_QQmlEngine_QString".}
...
proc clear*(self: gen_qqmlfile_types.QQmlFile, objectVal: gen_qobject_types.QObject): void =
  fcQQmlFile_clearObject(self.h, objectVal.h)
...
proc create*(T: type gen_qqmlfile_types.QQmlFile,
    engine: gen_qqmlengine_types.QQmlEngine, url: gen_qurl_types.QUrl): gen_qqmlfile_types.QQmlFile =
  let tmp = gen_qqmlfile_types.QQmlFile(h: fcQQmlFile_new2(engine.h, url.h), owned: true)
  tmp
proc create*(T: type gen_qqmlfile_types.QQmlFile,
    engine: gen_qqmlengine_types.QQmlEngine, url: openArray[char]): gen_qqmlfile_types.QQmlFile =
  let tmp = gen_qqmlfile_types.QQmlFile(h: fcQQmlFile_new3(engine.h, struct_seaqt_string(data: if len(url) > 0: addr url[0] else: nil, len: csize_t(len(url)))), owned: true)
  tmp
