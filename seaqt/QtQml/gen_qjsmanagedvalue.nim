...
proc property*(self: gen_qjsmanagedvalue_types.QJSManagedValue, name: string): gen_qjsvalue_types.QJSValue =
  gen_qjsvalue_types.QJSValue(h: fcQJSManagedValue_property(self.h, struct_miqt_string(data: if len(name) > 0: addr name[0] else: nil, len: csize_t(len(name)))))
...
proc create*(T: type gen_qjsmanagedvalue_types.QJSManagedValue,
  ...
  let tmp = gen_qjsmanagedvalue_types.QJSManagedValue(h: fcQJSManagedValue_new5(struct_miqt_string(data: if len(string) > 0: addr string[0] else: nil, len: csize_t(len(string))), engine.h))
  tmp
