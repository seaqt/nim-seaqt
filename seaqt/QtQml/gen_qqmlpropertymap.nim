...
proc fcQQmlPropertyMap_createX(): pointer {.importc: "QQmlPropertyMap_create".}
proc fcQQmlPropertyMap_value(self: pointer, key: struct_seaqt_string): pointer {.importc: "QQmlPropertyMap_value".}
proc fcQQmlPropertyMap_insertKeyValue(self: pointer, key: struct_seaqt_string, value: pointer): void {.importc: "QQmlPropertyMap_insert_key_value".}
proc fcQQmlPropertyMap_insertValues(self: pointer, values: struct_seaqt_map): void {.importc: "QQmlPropertyMap_insert_values".}
...
proc fcQQmlPropertyMap_keys(self: pointer): struct_seaqt_array {.importc: "QQmlPropertyMap_keys".}
...
proc fcQQmlPropertyMap_createParent(parent: pointer): pointer {.importc: "QQmlPropertyMap_create_parent".}
...
type cQQmlPropertyMapVTable {.pure.} = object
  ...
  updateValue*: proc(self: pointer, key: struct_seaqt_string, input: pointer): pointer {.cdecl, raises: [], gcsafe.}
...
proc fcQQmlPropertyMap_virtualbase_updateValue(self: pointer, key: struct_seaqt_string, input: pointer): pointer {.importc: "QQmlPropertyMap_virtualbase_updateValue".}
...
proc createX*(_: type gen_qqmlpropertymap_types.QQmlPropertyMap): gen_qqmlpropertymap_types.QQmlPropertyMap =
  gen_qqmlpropertymap_types.QQmlPropertyMap(h: fcQQmlPropertyMap_createX(), owned: false)

proc value*(self: gen_qqmlpropertymap_types.QQmlPropertyMap, key: openArray[char]): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQQmlPropertyMap_value(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key)))), owned: true)

proc insert*(self: gen_qqmlpropertymap_types.QQmlPropertyMap, key: openArray[char], value: gen_qvariant_types.QVariant): void =
  fcQQmlPropertyMap_insertKeyValue(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), value.h)

proc insert*(self: gen_qqmlpropertymap_types.QQmlPropertyMap, values: Table[string,gen_qvariant_types.QVariant]): void =
  var values_Keys_CArray = newSeq[struct_seaqt_string](len(values))
  var values_Values_CArray = newSeq[pointer](len(values))
  var values_ctr = 0
  for values_k in values.keys():
    values_Keys_CArray[values_ctr] = struct_seaqt_string(data: if len(values_k) > 0: addr values_k[0] else: nil, len: csize_t(len(values_k)))
    values_ctr += 1
  values_ctr = 0
  ...
  fcQQmlPropertyMap_insertValues(self.h, struct_seaqt_map(len: csize_t(len(values)),keys: if len(values) == 0: nil else: addr(values_Keys_CArray[0]), values: if len(values) == 0: nil else: addr(values_Values_CArray[0]),))

proc clear*(self: gen_qqmlpropertymap_types.QQmlPropertyMap, key: openArray[char]): void =
  fcQQmlPropertyMap_clear(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))))
...
proc fcQQmlPropertyMap_slot_callback_valueChanged(slot: int, key: struct_seaqt_string, value: pointer) {.cdecl.} =
...
proc createX*(_: type gen_qqmlpropertymap_types.QQmlPropertyMap, parent: gen_qobject_types.QObject): gen_qqmlpropertymap_types.QQmlPropertyMap =
  gen_qqmlpropertymap_types.QQmlPropertyMap(h: fcQQmlPropertyMap_createParent(parent.h), owned: false)
...
proc QQmlPropertyMapupdateValue*(self: gen_qqmlpropertymap_types.QQmlPropertyMap, key: openArray[char], input: gen_qvariant_types.QVariant): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQQmlPropertyMap_virtualbase_updateValue(self.h, struct_seaqt_string(data: if len(key) > 0: addr key[0] else: nil, len: csize_t(len(key))), input.h), owned: true)
...
proc fcQQmlPropertyMap_vtable_callback_updateValue(self: pointer, key: struct_seaqt_string, input: pointer): pointer {.cdecl.} =
  let vtbl = cast[ptr QQmlPropertyMapVTable](fcQQmlPropertyMap_vdata(self)[])
  let self = QQmlPropertyMap(h: self)
  let vkey_ms = key
  let vkeyx_ret = string.fromBytes(vkey_ms)
...
proc fcQQmlPropertyMap_method_callback_updateValue(self: pointer, key: struct_seaqt_string, input: pointer): pointer {.cdecl.} =
  let inst = cast[VirtualQQmlPropertyMap](fcQQmlPropertyMap_vdata(self)[])
  let vkey_ms = key
  let vkeyx_ret = string.fromBytes(vkey_ms)
  c_free(vkey_ms.data)
...
proc create*(T: type gen_qqmlpropertymap_types.QQmlPropertyMap,
  ...
  let tmp = gen_qqmlpropertymap_types.QQmlPropertyMap(h: fcQQmlPropertyMap_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQQmlPropertyMap_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qqmlpropertymap_types.QQmlPropertyMap,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQmlPropertyMap_vtable_callback_disconnectNotify
  let tmp = gen_qqmlpropertymap_types.QQmlPropertyMap(h: fcQQmlPropertyMap_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQQmlPropertyMap_vdata(tmp.h)[] = addr(vtbl[])
  tmp
