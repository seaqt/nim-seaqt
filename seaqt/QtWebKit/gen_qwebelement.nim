...
proc findAll*(self: gen_qwebelement_types.QWebElement, selectorQuery: openArray[char]): gen_qwebelement_types.QWebElementCollection =
  gen_qwebelement_types.QWebElementCollection(h: fcQWebElement_findAll(self.h, struct_seaqt_string(data: if len(selectorQuery) > 0: addr selectorQuery[0] else: nil, len: csize_t(len(selectorQuery)))), owned: true)

proc findFirst*(self: gen_qwebelement_types.QWebElement, selectorQuery: openArray[char]): gen_qwebelement_types.QWebElement =
  gen_qwebelement_types.QWebElement(h: fcQWebElement_findFirst(self.h, struct_seaqt_string(data: if len(selectorQuery) > 0: addr selectorQuery[0] else: nil, len: csize_t(len(selectorQuery)))), owned: true)
...
proc evaluateJavaScript*(self: gen_qwebelement_types.QWebElement, scriptSource: openArray[char]): gen_qvariant_types.QVariant =
  gen_qvariant_types.QVariant(h: fcQWebElement_evaluateJavaScript(self.h, struct_seaqt_string(data: if len(scriptSource) > 0: addr scriptSource[0] else: nil, len: csize_t(len(scriptSource)))), owned: true)
