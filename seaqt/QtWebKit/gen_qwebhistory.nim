...
proc fcQWebHistory_forwardItems(self: pointer, maxItems: cint): struct_seaqt_array {.importc: "QWebHistory_forwardItems".}
...
proc create*(T: type gen_qwebhistory_types.QWebHistoryItem,
  ...
  let tmp = gen_qwebhistory_types.QWebHistoryItem(h: fcQWebHistoryItem_new(other.h))
  tmp
...
proc loadFromMap*(self: gen_qwebhistory_types.QWebHistory, map: Table[string,gen_qvariant_types.QVariant]): void =
  ...
  fcQWebHistory_loadFromMap(self.h, struct_seaqt_map(len: csize_t(len(map)),keys: if len(map) == 0: nil else: addr(map_Keys_CArray[0]), values: if len(map) == 0: nil else: addr(map_Values_CArray[0]),))
