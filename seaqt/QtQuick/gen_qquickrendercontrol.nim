...
proc fcQQuickRenderControl_trUtf8S(s: cstring): struct_seaqt_string {.importc: "QQuickRenderControl_trUtf8_s".}
...
proc fcQQuickRenderControl_trUtf8SC(s: cstring, c: cstring): struct_seaqt_string {.importc: "QQuickRenderControl_trUtf8_s_c".}
proc fcQQuickRenderControl_trUtf8SCN(s: cstring, c: cstring, n: cint): struct_seaqt_string {.importc: "QQuickRenderControl_trUtf8_s_c_n".}
...
proc trUtf8*(_: type gen_qquickrendercontrol_types.QQuickRenderControl, s: cstring): string =
  let v_ms = fcQQuickRenderControl_trUtf8S(s)
  let vx_ret = string.fromBytes(v_ms)
  c_free(v_ms.data)
  vx_ret
...
proc create*(T: type gen_qquickrendercontrol_types.QQuickRenderControl,
  ...
  let tmp = gen_qquickrendercontrol_types.QQuickRenderControl(h: fcQQuickRenderControl_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQQuickRenderControl_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickrendercontrol_types.QQuickRenderControl,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickRenderControl_vtable_callback_disconnectNotify
  let tmp = gen_qquickrendercontrol_types.QQuickRenderControl(h: fcQQuickRenderControl_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQQuickRenderControl_vdata(tmp.h)[] = addr(vtbl[])
  tmp
