...
proc fcQQuickView_errors(self: pointer): struct_seaqt_array {.importc: "QQuickView_errors".}
...
proc fcQQuickView_loadFromModule(self: pointer, uri: struct_seaqt_string, typeName: struct_seaqt_string): void {.importc: "QQuickView_loadFromModule".}
proc fcQQuickView_setInitialProperties(self: pointer, initialProperties: struct_seaqt_map): void {.importc: "QQuickView_setInitialProperties".}
...
proc fcQQuickView_virtualbase_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.importc: "QQuickView_virtualbase_nativeEvent".}
...
proc fcQQuickView_new4(vtbl: pointer, vdata: csize_t, uri: struct_seaqt_string, typeName: struct_seaqt_string): ptr cQQuickView {.importc: "QQuickView_new_uri_typeName".}
proc fcQQuickView_new5(vtbl: pointer, vdata: csize_t, source: pointer, renderControl: pointer): ptr cQQuickView {.importc: "QQuickView_new_source_renderControl".}
proc fcQQuickView_new6(vtbl: pointer, vdata: csize_t, parent: pointer): ptr cQQuickView {.importc: "QQuickView_new_parent".}
proc fcQQuickView_new7(vtbl: pointer, vdata: csize_t, source: pointer, parent: pointer): ptr cQQuickView {.importc: "QQuickView_new_source_parent".}
proc fcQQuickView_new8(vtbl: pointer, vdata: csize_t, uri: struct_seaqt_string, typeName: struct_seaqt_string, parent: pointer): ptr cQQuickView {.importc: "QQuickView_new_uri_typeName_parent".}
...
proc loadFromModule*(self: gen_qquickview_types.QQuickView, uri: openArray[char], typeName: openArray[char]): void =
  fcQQuickView_loadFromModule(self.h, struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))))
...
proc setInitialProperties*(self: gen_qquickview_types.QQuickView, initialProperties: Table[string,gen_qvariant_types.QVariant]): void =
  ...
  fcQQuickView_setInitialProperties(self.h, struct_seaqt_map(len: csize_t(len(initialProperties)),keys: if len(initialProperties) == 0: nil else: addr(initialProperties_Keys_CArray[0]), values: if len(initialProperties) == 0: nil else: addr(initialProperties_Values_CArray[0]),))
...
proc QQuickViewnativeEvent*(self: gen_qquickview_types.QQuickView, eventType: openArray[byte], message: pointer, resultVal: ptr uint): bool =
  fcQQuickView_virtualbase_nativeEvent(self.h, struct_seaqt_string(data: if len(eventType) > 0: addr eventType[0] else: nil, len: csize_t(len(eventType))), message, resultVal)
...
proc fcQQuickView_vtable_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let vtbl = cast[ptr QQuickViewVTable](fcQQuickView_vdata(self)[])
  let self = QQuickView(h: self)
...
proc fcQQuickView_method_callback_nativeEvent(self: pointer, eventType: struct_seaqt_string, message: pointer, resultVal: ptr uint): bool {.cdecl.} =
  let inst = cast[VirtualQQuickView](fcQQuickView_vdata(self)[])
  var veventType_bytearray = eventType
  var veventTypex_ret = @(toOpenArray(cast[ptr UncheckedArray[byte]](veventType_bytearray.data), 0, int(veventType_bytearray.len)-1))
  c_free(veventType_bytearray.data)
...
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new(addr(vtbl[].vtbl), csize_t(sizeof(pointer))), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new2(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), engine.h, parent.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new3(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), source.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new4(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName)))), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new5(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), source.h, renderControl.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new6(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), parent.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new7(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), source.h, parent.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
proc create*(T: type gen_qquickview_types.QQuickView,
    uri: openArray[char], typeName: openArray[char], parent: gen_qwindow_types.QWindow,
    vtbl: ref QQuickViewVTable = nil): gen_qquickview_types.QQuickView =
  let vtbl = if vtbl == nil: new QQuickViewVTable else: vtbl
  GC_ref(vtbl)
  vtbl[].vtbl.destructor = proc(self: pointer) {.cdecl.} =
  ...
  if not isNil(vtbl[].disconnectNotify):
    vtbl[].vtbl.disconnectNotify = fcQQuickView_vtable_callback_disconnectNotify
  let tmp = gen_qquickview_types.QQuickView(h: fcQQuickView_new8(addr(vtbl[].vtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))), parent.h), owned: true)
  fcQQuickView_vdata(tmp.h)[] = addr(vtbl[])
  tmp
...
proc create*(T: type gen_qquickview_types.QQuickView,
    ...
    inst: VirtualQQuickView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQuickView_new4(addr(cQQuickView_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))))
  fcQQuickView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qquickview_types.QQuickView,
    ...
    inst: VirtualQQuickView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQuickView_new5(addr(cQQuickView_mvtbl), csize_t(sizeof(pointer)), source.h, renderControl.h)
  fcQQuickView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qquickview_types.QQuickView,
    ...
    inst: VirtualQQuickView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQuickView_new6(addr(cQQuickView_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQQuickView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qquickview_types.QQuickView,
    ...
    inst: VirtualQQuickView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQuickView_new7(addr(cQQuickView_mvtbl), csize_t(sizeof(pointer)), source.h, parent.h)
  fcQQuickView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qquickview_types.QQuickView,
    ...
    inst: VirtualQQuickView) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQQuickView_new8(addr(cQQuickView_mvtbl), csize_t(sizeof(pointer)), struct_seaqt_string(data: if len(uri) > 0: addr uri[0] else: nil, len: csize_t(len(uri))), struct_seaqt_string(data: if len(typeName) > 0: addr typeName[0] else: nil, len: csize_t(len(typeName))), parent.h)
  fcQQuickView_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
