...
{.compile("gen_qsgengine.cpp", QtQuickCFlags).}
...
proc fcQSGEngine_vtable_callback_metaObject(self: pointer): pointer {.cdecl.} =
...
proc fcQSGEngine_vtable_callback_metacall(self: pointer, param1: cint, param2: cint, param3: pointer): cint {.cdecl.} =
  let vtbl = cast[ptr QSGEngineVTable](fcQSGEngine_vdata(self)[])
  let self = QSGEngine(h: self)
...
proc fcQSGEngine_method_callback_metacall(self: pointer, param1: cint, param2: cint, param3: pointer): cint {.cdecl.} =
  let inst = cast[VirtualQSGEngine](fcQSGEngine_vdata(self)[])
  let slotval1 = cint(param1)
  let slotval2 = param2
  let slotval3 = param3
...
proc create*(T: type gen_qsgengine_types.QSGEngine,
  ...
  if not isNil(vtbl[].metaObject):
    vtbl[].vtbl.metaObject = fcQSGEngine_vtable_callback_metaObject
  if not isNil(vtbl[].metacast):
    vtbl[].vtbl.metacast = fcQSGEngine_vtable_callback_metacast
  if not isNil(vtbl[].metacall):
    vtbl[].vtbl.metacall = fcQSGEngine_vtable_callback_metacall
  if not isNil(vtbl[].event):
    vtbl[].vtbl.event = fcQSGEngine_vtable_callback_event
  if not isNil(vtbl[].eventFilter):
    vtbl[].vtbl.eventFilter = fcQSGEngine_vtable_callback_eventFilter
  if not isNil(vtbl[].timerEvent):
    vtbl[].vtbl.timerEvent = fcQSGEngine_vtable_callback_timerEvent
  if not isNil(vtbl[].childEvent):
    vtbl[].vtbl.childEvent = fcQSGEngine_vtable_callback_childEvent
  if not isNil(vtbl[].customEvent):
    vtbl[].vtbl.customEvent = fcQSGEngine_vtable_callback_customEvent
...
const cQSGEngine_mvtbl = cQSGEngineVTable(
  ...
  metacall: fcQSGEngine_method_callback_metacall,
  event: fcQSGEngine_method_callback_event,
  eventFilter: fcQSGEngine_method_callback_eventFilter,
  timerEvent: fcQSGEngine_method_callback_timerEvent,
  childEvent: fcQSGEngine_method_callback_childEvent,
  customEvent: fcQSGEngine_method_callback_customEvent,
  connectNotify: fcQSGEngine_method_callback_connectNotify,
  disconnectNotify: fcQSGEngine_method_callback_disconnectNotify,
)
proc create*(T: type gen_qsgengine_types.QSGEngine,
    inst: VirtualQSGEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSGEngine_new(addr(cQSGEngine_mvtbl), csize_t(sizeof(pointer)))
  fcQSGEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
...
proc create*(T: type gen_qsgengine_types.QSGEngine,
    ...
    inst: VirtualQSGEngine) =
  if inst[].h != nil: delete(move(inst[]))
  inst[].h = fcQSGEngine_new2(addr(cQSGEngine_mvtbl), csize_t(sizeof(pointer)), parent.h)
  fcQSGEngine_vdata(inst[].h)[] = addr inst[]
  inst[].owned = true
